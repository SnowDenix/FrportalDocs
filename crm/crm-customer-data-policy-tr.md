# CRM Müşteri Verileri Politikası (KVKK Uyumlu)

> **Son Güncelleme:** 01 Aralık 2025 | **Versiyon:** 1.0  
> **Hazırlayan:** Aydınlı Grup

---

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Yasal Dayanak](#yasal-dayanak)
5. [Sorumluluklar](#sorumluluklar)
6. [Veri Toplama İlkeleri](#veri-toplama-ilkeleri)
7. [İzin Yönetimi](#izin-yönetimi)
8. [Veri Saklama ve İmha](#veri-saklama-ve-imha)
9. [Müşteri Hakları](#müşteri-hakları)
10. [Süreç Adımları](#süreç-adımları)
11. [Ekran Görüntüleri](#ekran-görüntüleri)
12. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
13. [Mağaza İpuçları](#mağaza-ipuçları)
14. [Revizyon Geçmişi](#revizyon-geçmişi)

---

## Amaç

Bu politika, Aydınlı Grup franchise mağazalarında (U.S. Polo Assn., Pierre Cardin, Cacharel) müşteri kişisel verilerinin toplanması, işlenmesi, saklanması ve korunmasına ilişkin kuralları 6698 sayılı Kişisel Verilerin Korunması Kanunu (KVKK) çerçevesinde tanımlar.

### Politikanın Hedefleri

- KVKK'ya tam uyum sağlamak
- Müşteri verilerini güvenli bir şekilde yönetmek
- Veri işleme süreçlerini standartlaştırmak
- Müşteri haklarını korumak ve saygı göstermek
- Veri ihlali risklerini minimize etmek

---

## Kapsam

### Bu Politika Kimleri Kapsar?

| Rol | Sorumluluk |
|-----|------------|
| Tüm Mağaza Personeli | Veri toplama ve işleme kurallarına uymak |
| Mağaza Müdürü | Politika uyumunu denetlemek |
| Bölge Müdürü | Bölge genelinde uyumu sağlamak |
| CRM Merkez | Veri yönetimi ve güvenliği |
| Veri Koruma Sorumlusu (DPO) | KVKK uyumluluk denetimi |

### Kapsanan Veriler

| Veri Kategorisi | Örnekler | Hassasiyet |
|-----------------|----------|------------|
| **Kimlik Bilgileri** | Ad, soyad, T.C. kimlik no | Yüksek |
| **İletişim Bilgileri** | Telefon, e-posta, adres | Orta |
| **Satın Alma Verileri** | Satış geçmişi, tercihler | Düşük |
| **Finansal Veriler** | Ödeme yöntemi (maskelenmiş) | Yüksek |
| **Demografik Veriler** | Doğum tarihi, cinsiyet | Orta |
| **İletişim Tercihleri** | SMS/E-posta izinleri | Düşük |

---

## Tanımlar

### KVKK Terimleri

| Terim | Tanım |
|-------|-------|
| **Kişisel Veri** | Kimliği belirli veya belirlenebilir gerçek kişiye ilişkin her türlü bilgi |
| **Özel Nitelikli Kişisel Veri** | Irk, sağlık, biyometrik veri gibi hassas veriler |
| **Veri Sorumlusu** | Verilerin işleme amaçlarını ve vasıtalarını belirleyen kişi (Aydınlı Grup) |
| **Veri İşleyen** | Veri sorumlusunun verdiği yetkiye dayanarak veri işleyen kişi |
| **Açık Rıza** | Belirli bir konuya ilişkin, bilgilendirilmeye dayanan ve özgür iradeyle açıklanan rıza |
| **Aydınlatma Metni** | Müşteriye verilerinin nasıl işleneceğini açıklayan bilgilendirme |
| **İmha** | Kişisel verilerin silinmesi, yok edilmesi veya anonim hale getirilmesi |
| **Veri İhlali** | Kişisel verilerin yetkisiz erişim, kayıp veya ifşaya maruz kalması |

### İzin Tipleri

| İzin Türü | Açıklama | Dayanak |
|-----------|----------|---------|
| **Veri İşleme İzni** | Kişisel verilerin işlenmesine onay | KVKK m.5 |
| **Ticari Elektronik İleti İzni** | SMS/E-posta pazarlama | 6563 sayılı Kanun |
| **Profilleme İzni** | Müşteri davranış analizi | KVKK m.5 |
| **Üçüncü Taraf Paylaşım İzni** | Verilerin iş ortaklarıyla paylaşımı | KVKK m.8 |

---

## Yasal Dayanak

### İlgili Mevzuat

| Mevzuat | Konu |
|---------|------|
| 6698 sayılı KVKK | Kişisel verilerin korunması |
| 6563 sayılı Kanun | Elektronik ticaret ve ticari iletişim |
| KVKK Yönetmelikleri | Uygulama detayları |
| Kişisel Verileri Koruma Kurulu Kararları | Emsal kararlar |

### Temel İlkeler (KVKK m.4)

1. **Hukuka ve dürüstlük kurallarına uygun olma**
2. **Doğru ve gerektiğinde güncel olma**
3. **Belirli, açık ve meşru amaçlar için işlenme**
4. **İşlendikleri amaçla bağlantılı, sınırlı ve ölçülü olma**
5. **İlgili mevzuatta öngörülen veya işlendikleri amaç için gerekli olan süre kadar muhafaza edilme**

---

## Sorumluluklar

### Mağaza Personeli

| Görev | Detay |
|-------|-------|
| Aydınlatma | Müşteriye veri işleme hakkında bilgi vermek |
| İzin alma | Açık rızayı belgelemek |
| Veri girişi | Doğru ve eksiksiz veri girmek |
| Gizlilik | Müşteri verilerini gizli tutmak |
| Raporlama | Şüpheli durumları bildirmek |

### Mağaza Müdürü

| Görev | Detay |
|-------|-------|
| Eğitim | Personeli KVKK konusunda eğitmek |
| Denetim | Veri işleme süreçlerini denetlemek |
| İzin yönetimi | İzin formlarını saklamak |
| İhlal bildirimi | Veri ihlallerini raporlamak |

### CRM Merkez / DPO

| Görev | Detay |
|-------|-------|
| Politika | Veri koruma politikalarını oluşturmak |
| Uyumluluk | KVKK uyumluluğunu sağlamak |
| Eğitim | Kurumsal eğitim programları düzenlemek |
| İhlal yönetimi | Veri ihlallerini yönetmek |
| Kurul iletişimi | KVKK Kurulu ile ilişkileri yönetmek |

---

## Veri Toplama İlkeleri

### Toplanabilecek Veriler

| Veri | Amaç | Zorunluluk |
|------|------|------------|
| Ad, Soyad | Müşteri tanımlama | Zorunlu |
| Cep Telefonu | İletişim, müşteri doğrulama | Zorunlu |
| E-posta | İletişim, dijital hizmetler | Önerilen |
| Doğum Tarihi | Doğum günü kampanyaları | İsteğe bağlı |
| Cinsiyet | Kişiselleştirilmiş hizmet | İsteğe bağlı |
| Adres | Teslimat hizmetleri | İsteğe bağlı |

### Toplanamayacak Veriler

Aşağıdaki veriler **kesinlikle toplanamaz**:

| Veri Türü | Açıklama |
|-----------|----------|
| Sağlık bilgileri | Hastalık, ilaç kullanımı vb. |
| Dini inanç | Din, mezhep bilgisi |
| Siyasi görüş | Parti üyeliği vb. |
| Irk/Etnik köken | Etnik köken bilgisi |
| Biyometrik veri | Parmak izi, yüz tanıma |
| Ceza mahkumiyeti | Sabıka kaydı |
| Cinsel hayat | Cinsel tercih bilgisi |

> ⚠️ **UYARI:** Bu verileri sormak, kaydetmek veya işlemek yasaktır ve ciddi yaptırımlara tabidir.

### Veri Toplama Kuralları

1. **Sadece gerekli verileri toplayın:** Amaç dışı veri toplamayın
2. **Aydınlatma yapın:** Verinin neden toplandığını açıklayın
3. **İzin alın:** Açık rızayı belgeleyin
4. **Doğrulayın:** Girilen verilerin doğruluğunu teyit edin
5. **Güvende tutun:** Verileri yetkisiz erişimden koruyun

---

## İzin Yönetimi

### Aydınlatma Metni Sunumu

**Ne zaman:** Her yeni müşteri kaydında

**Nasıl:**
1. Dijital ekrandan okutun
2. Veya basılı metni verin
3. Müşterinin okuduğunu/anladığını teyit edin

**Aydınlatma Metni İçeriği:**
- Veri sorumlusunun kimliği
- Verilerin hangi amaçla işleneceği
- Verilerin kimlere aktarılabileceği
- Veri toplama yöntemi ve hukuki sebebi
- Müşterinin KVKK kapsamındaki hakları

### İzin Alma Süreci

#### Veri İşleme İzni (Zorunlu)

```
┌─────────────────────────────────────────────────────────────────┐
│  AYDINLATMA VE AÇIK RIZA METNİ                                  │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  6698 sayılı Kişisel Verilerin Korunması Kanunu kapsamında      │
│  kişisel verilerimin işlenmesine ilişkin aydınlatma metnini     │
│  okudum ve anladım.                                             │
│                                                                 │
│  [✓] Kişisel verilerimin işlenmesini KABUL EDİYORUM (Zorunlu)  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### Ticari İletişim İzinleri (İsteğe Bağlı)

```
┌─────────────────────────────────────────────────────────────────┐
│  TİCARİ ELEKTRONİK İLETİ İZNİ                                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Aydınlı Grup markalarından (U.S. Polo Assn., Pierre Cardin,    │
│  Cacharel) kampanya, promosyon ve bilgilendirme amaçlı          │
│  ticari elektronik ileti almak istiyorum:                       │
│                                                                 │
│  [ ] SMS ile                                                    │
│  [ ] E-posta ile                                                │
│  [ ] Telefon ile aranarak                                       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### İzin Formu Saklama

| Format | Saklama Süresi | Saklama Yeri |
|--------|----------------|--------------|
| Dijital onay | 10 yıl | CRM sistemi |
| Fiziksel form | 10 yıl | Mağaza arşivi |
| Ses kaydı (call center) | 10 yıl | Merkez arşiv |

---

## Veri Saklama ve İmha

### Saklama Süreleri

| Veri Kategorisi | Saklama Süresi | Dayanak |
|-----------------|----------------|---------|
| Müşteri kimlik bilgileri | Müşteri ilişkisi + 10 yıl | Ticaret Kanunu |
| Satın alma geçmişi | 10 yıl | Vergi mevzuatı |
| İletişim izinleri | İzin iptali + 10 yıl | KVKK |
| Şikayet kayıtları | Çözüm + 10 yıl | TTK |
| Loyalty işlemleri | Son işlem + 5 yıl | İç politika |

### İmha Yöntemleri

| Yöntem | Açıklama | Kullanım Alanı |
|--------|----------|----------------|
| **Silme** | Verinin geri getirilemeyecek şekilde silinmesi | Dijital kayıtlar |
| **Yok Etme** | Fiziksel imha (öğütme, yakma) | Basılı belgeler |
| **Anonimleştirme** | Verinin kişiyle ilişkilendirilemez hale getirilmesi | Analitik amaçlı |

### Periyodik İmha

- **Sıklık:** Her 6 ayda bir
- **Sorumlu:** CRM Merkez
- **Raporlama:** İmha tutanağı düzenlenir

---

## Müşteri Hakları

### KVKK m.11 Kapsamında Haklar

| Hak | Açıklama | Süre |
|-----|----------|------|
| **Bilgi Edinme** | Verilerin işlenip işlenmediğini öğrenme | 30 gün |
| **Erişim** | İşlenen verilere erişim talep etme | 30 gün |
| **Düzeltme** | Eksik/yanlış verilerin düzeltilmesini isteme | 30 gün |
| **Silme** | Verilerin silinmesini isteme (unutulma hakkı) | 30 gün |
| **Aktarım Bilgisi** | Verilerin aktarıldığı üçüncü tarafları öğrenme | 30 gün |
| **İtiraz** | Otomatik işleme sonuçlarına itiraz | 30 gün |
| **Tazminat** | Zarara uğranılması halinde tazminat talep etme | Dava süreci |

### Hak Kullanım Süreci

```
┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│  Müşteri    │────▶│   Mağaza    │────▶│    CRM      │────▶│   Müşteri   │
│   Talebi    │     │   Kaydı     │     │   Değerlen. │     │   Yanıtı    │
└─────────────┘     └─────────────┘     └─────────────┘     └─────────────┘
       │                   │                   │                   │
       ▼                   ▼                   ▼                   ▼
  Yazılı başvuru      Ticket açma         İnceleme           30 gün içinde
  veya e-posta        Belge toplama       Karar              yazılı yanıt
```

---

## Süreç Adımları

### 1. Yeni Müşteri Kaydında Veri Toplama

**Adım 1:** Müşteriye aydınlatma metnini sunun

**Adım 2:** Metnin okunduğunu/anlaşıldığını teyit alın

**Adım 3:** Veri işleme iznini alın (zorunlu)

**Adım 4:** Ticari iletişim izinlerini sorun (isteğe bağlı)

**Adım 5:** İzinleri CRM'e kaydedin

**Adım 6:** Fiziksel form varsa imzalatın ve arşivleyin

**Adım 7:** Müşteriye bilgilendirme yapın

> ✅ **Doğru:** "KVKK kapsamında verilerinizi işleyebilmemiz için onayınıza ihtiyacımız var. İsterseniz kampanyalardan haberdar olmak için SMS/E-posta izni de verebilirsiniz."

> ❌ **Yanlış:** "Şuraya imza atın" (açıklama yapmadan)

---

### 2. Müşteri Bilgi Güncelleme

**Adım 1:** Müşteri kimliğini doğrulayın

**Adım 2:** Güncellenecek bilgileri alın

**Adım 3:** Değişikliği CRM'e kaydedin

**Adım 4:** Müşteriye güncelleme onayı verin

---

### 3. Silme/Unutulma Hakkı Talebi

**Adım 1:** Müşteriden yazılı başvuru alın

**Adım 2:** Ticket açın (Kategori: KVKK Talebi)

**Adım 3:** Müşteri kimliğini doğrulayın

**Adım 4:** CRM Merkez'e yönlendirin

**Adım 5:** Yasal saklama yükümlülüklerini kontrol edin

**Adım 6:** Silinebilecek verileri silin

**Adım 7:** 30 gün içinde müşteriye yazılı yanıt verin

> ⚠️ **Dikkat:** Bazı veriler yasal zorunluluk nedeniyle silinemez (örn: fatura bilgileri 10 yıl saklanmalı).

---

### 4. Veri İhlali Durumunda

**Adım 1:** İhlali tespit edin ve durdurun

**Adım 2:** Mağaza müdürünü bilgilendirin

**Adım 3:** CRM Merkez'e acil bildirim yapın

**Adım 4:** İhlal detaylarını belgeleyin:
- Ne zaman?
- Hangi veriler?
- Kaç müşteri etkilendi?
- Nasıl gerçekleşti?

**Adım 5:** CRM Merkez KVKK Kurulu'na bildirim yapar (72 saat içinde)

**Adım 6:** Etkilenen müşteriler bilgilendirilir

---

## Ekran Görüntüleri

### İzin Yönetimi Ekranı

![İzin Yönetimi - Placeholder](screenshots/crm-consent-management.png)

*Ekran görüntüsü eklenecektir.*

### KVKK Talep Formu

![KVKK Talep - Placeholder](screenshots/crm-kvkk-request.png)

*Ekran görüntüsü eklenecektir.*

---

## Sık Karşılaşılan Sorunlar ve Çözümler

### Sorun 1: Müşteri KVKK İznini Geri Çekmek İstiyor

**Çözüm:**
1. Talebi yazılı olarak alın
2. Veri işleme izni geri çekilirse kayıt silinemez, sadece pasife alınır
3. Ticari iletişim izni anında iptal edilir
4. 30 gün içinde yazılı yanıt verin

---

### Sorun 2: Müşteri Verilerini Sorgulamak İstiyor

**Çözüm:**
1. Müşteri kimliğini doğrulayın
2. CRM'den müşteri verilerini çıkarın
3. Yazılı olarak müşteriye teslim edin
4. Teslim tutanağı alın

---

### Sorun 3: Müşteri Yanlış Veri Şikayetinde Bulunuyor

**Çözüm:**
1. Yanlışlığı doğrulayın
2. Doğru bilgiyi müşteriden alın
3. CRM'de düzeltin
4. Müşteriye düzeltme onayı verin

---

### Sorun 4: Veri İhlali Şüphesi

**Çözüm:**
1. Panik yapmayın ama hızlı hareket edin
2. Yetkisiz erişimi engelleyin
3. Mağaza müdürünü bilgilendirin
4. CRM Merkez'i arayın
5. Hiçbir şeyi silmeyin/değiştirmeyin (delil)

---

## Mağaza İpuçları

### Doğru Uygulamalar

| Yapın | Açıklama |
|-------|----------|
| ✅ Her zaman aydınlatma yapın | İzin almadan veri işlemeyin |
| ✅ İzinleri belgeleyin | Dijital veya fiziksel kayıt tutun |
| ✅ Ekranı kilitleyin | Masanızdan ayrılırken |
| ✅ Şifrenizi koruyun | Kimseyle paylaşmayın |
| ✅ Şüpheli durumları bildirin | Hemen müdüre haber verin |

### Yanlış Uygulamalar

| Yapmayın | Risk |
|----------|------|
| ❌ İzinsiz veri işleme | KVKK cezası |
| ❌ Hassas veri sorma | Yasal yaptırım |
| ❌ Veriyi başkasıyla paylaşma | Veri ihlali |
| ❌ Müşteri ekranını açık bırakma | Yetkisiz erişim |
| ❌ Kağıda müşteri bilgisi yazma | Veri güvenliği riski |

### Müşteriye Açıklama Örnekleri

**İzin isterken:**
> "Sayın müşterimiz, size daha iyi hizmet verebilmek için bazı bilgilerinizi kaydetmemiz gerekiyor. Bu bilgiler KVKK kapsamında korunacak ve sadece size hizmet vermek amacıyla kullanılacaktır. İsterseniz kampanyalarımızdan da haberdar olabilirsiniz."

**İzin vermek istemezse:**
> "Tabii ki, bu tamamen sizin tercihiniz. Bu durumda sadece fiş karşılığı satış yapabiliriz, müşteri kartı avantajlarından yararlanamazsınız."

**Bilgi sorgulamak isterse:**
> "Elbette, KVKK kapsamında bu hakkınız var. Yazılı başvurunuzu aldıktan sonra 30 gün içinde bilgilerinizi size ileteceğiz."

---

## Revizyon Geçmişi

| Versiyon | Tarih | Değişiklik | Hazırlayan |
|----------|-------|------------|------------|
| 1.0 | 01.12.2025 | İlk sürüm oluşturuldu | Aydınlı Grup |

---

*Bu doküman Aydınlı Grup CRM Operasyonları tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*

