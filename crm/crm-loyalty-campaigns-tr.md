---
layout: default
title: "CRM Loyalty Programı ve Kampanya Yönetimi"
---

# CRM Loyalty Programı ve Kampanya Yönetimi

> **Son Güncelleme:** 01 Aralık 2025 | **Versiyon:** 1.0  
> **Hazırlayan:** Aydınlı Grup

---

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Loyalty Programı Yapısı](#loyalty-programı-yapısı)
6. [Puan Sistemi](#puan-sistemi)
7. [Kampanya Türleri](#kampanya-türleri)
8. [Kupon Yönetimi](#kupon-yönetimi)
9. [Süreç Adımları](#süreç-adımları)
10. [Ekran Görüntüleri](#ekran-görüntüleri)
11. [KPI'lar](#kpılar)
12. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
13. [Mağaza İpuçları](#mağaza-ipuçları)
14. [Revizyon Geçmişi](#revizyon-geçmişi)

---

## Amaç

Bu doküman, Aydınlı Grup franchise mağazalarında (U.S. Polo Assn., Pierre Cardin, Cacharel) uygulanan Loyalty Programı ve kampanya yönetimi süreçlerini tanımlar.

### Loyalty Programının Hedefleri

- Müşteri sadakatini artırmak
- Tekrar satın alma oranını yükseltmek
- Müşteri yaşam boyu değerini (CLV) artırmak
- Kişiselleştirilmiş deneyim sunmak
- Müşteri verisi toplama kapasitesini güçlendirmek

---

## Kapsam

### Bu Doküman Kimleri Kapsar?

| Rol | Sorumluluk |
|-----|------------|
| Mağaza Personeli | Loyalty işlemlerini yürütmek |
| Mağaza Müdürü | Kampanya uygulamalarını denetlemek |
| Bölge Müdürü | Bölgesel performansı takip etmek |
| CRM Merkez | Kampanya oluşturma ve yönetimi |
| Pazarlama Ekibi | Kampanya stratejisi belirleme |

### Kapsanan Süreçler

1. Loyalty programına üyelik
2. Puan kazanma ve kullanma
3. Kupon oluşturma ve kullanma
4. Kampanya uygulama
5. Üyelik yönetimi
6. Raporlama ve analiz

---

## Tanımlar

### Loyalty Terimleri

| Terim | Tanım |
|-------|-------|
| **Loyalty Programı** | Müşteri sadakatini ödüllendiren, puan/kupon tabanlı sistem |
| **Loyalty Kartı** | Üyeliği temsil eden fiziksel veya dijital kart |
| **Puan** | Alışveriş karşılığı kazanılan ödül birimi |
| **Kupon** | Belirli koşullarda indirim sağlayan dijital belge |
| **Kampanya** | Belirli süre/koşullarda geçerli promosyon |
| **Welcome Bonus** | Yeni üyelere verilen hoş geldin puanı/kuponu |
| **Birthday Bonus** | Doğum gününde verilen özel avantaj |
| **Redemption** | Puan veya kuponun kullanılması |

### Kampanya Terimleri

| Terim | Tanım |
|-------|-------|
| **Threshold** | Kampanyanın geçerli olması için minimum tutar |
| **Cap** | Maksimum indirim tutarı |
| **Stacking** | Birden fazla kampanyanın birlikte kullanılması |
| **Exclusion** | Kampanya dışı ürün/kategori |
| **Validity Period** | Kampanya geçerlilik süresi |
| **Target Segment** | Kampanyanın hedef müşteri grubu |

---

## Sorumluluklar

### Mağaza Personeli

| Görev | Detay |
|-------|-------|
| Üyelik kaydı | Yeni müşterileri loyalty programına kaydetmek |
| Puan işleme | Satışlarda puan kazandırmak/kullandırmak |
| Kupon uygulama | Geçerli kuponları satışa uygulamak |
| Bilgilendirme | Müşteriyi kampanyalar hakkında bilgilendirmek |
| Sorun çözümü | Loyalty ile ilgili sorunları çözmek/yönlendirmek |

### Mağaza Müdürü

| Görev | Detay |
|-------|-------|
| Kampanya takibi | Aktif kampanyaların uygulanmasını denetlemek |
| Performans izleme | Mağaza loyalty KPI'larını izlemek |
| Eğitim | Personeli kampanyalar konusunda eğitmek |
| Raporlama | Kampanya performansını raporlamak |

### CRM Merkez

| Görev | Detay |
|-------|-------|
| Kampanya tasarımı | Kampanyaları oluşturmak ve yapılandırmak |
| Kupon üretimi | Toplu kupon oluşturmak |
| Sistem yönetimi | Loyalty sisteminin teknik yönetimi |
| Analiz | Kampanya etkinliğini analiz etmek |

---

## Loyalty Programı Yapısı

### Program Özellikleri

Aydınlı Grup Loyalty Programı, tek seviyeli (flat) bir yapıya sahiptir:

| Özellik | Detay |
|---------|-------|
| Program Adı | Aydınlı Club |
| Yapı | Tek seviye (tüm üyeler eşit haklara sahip) |
| Katılım | Ücretsiz |
| Geçerli Markalar | U.S. Polo Assn., Pierre Cardin, Cacharel |
| Geçerli Kanallar | Mağaza, Online |

### Üyelik Avantajları

| Avantaj | Açıklama |
|---------|----------|
| Puan Kazanma | Her alışverişte puan kazanma |
| Özel Kampanyalar | Sadece üyelere özel indirimler |
| Doğum Günü Bonusu | Doğum gününde özel kupon |
| Erken Erişim | Yeni koleksiyonlara erken erişim |
| Özel Davetler | VIP etkinliklere davet |
| Kişisel Teklifler | Alışveriş alışkanlıklarına göre özel teklifler |

### Üyelik Kartı

| Tip | Açıklama |
|-----|----------|
| Dijital Kart | CRM sisteminde kayıtlı, telefon numarası ile erişim |
| Fiziksel Kart | Plastik kart (talep üzerine, opsiyonel) |
| Mobil Uygulama | Dijital cüzdan entegrasyonu (gelecek geliştirme) |

---

## Puan Sistemi

### Puan Kazanma

| Koşul | Puan Oranı | Örnek |
|-------|------------|-------|
| Standart Alışveriş | 1 TL = 1 Puan | 500 TL = 500 Puan |
| Çarpanlı Kampanya (2x) | 1 TL = 2 Puan | 500 TL = 1.000 Puan |
| Çarpanlı Kampanya (3x) | 1 TL = 3 Puan | 500 TL = 1.500 Puan |
| Doğum Günü Alışverişi | 1 TL = 2 Puan | Doğum günü haftasında |

### Puan Kazanma Kuralları

| Kural | Açıklama |
|-------|----------|
| Eşleştirme zorunlu | Puan kazanmak için satış müşteriye eşleştirilmeli |
| İndirimli ürünler | İndirimli satış tutarı üzerinden puan hesaplanır |
| İade durumu | İade edilen ürünün puanı düşürülür |
| Geçerlilik | Puanlar 12 ay geçerlidir |

### Puan Kullanma

| Koşul | Değer | Kural |
|-------|-------|-------|
| Minimum Kullanım | 100 Puan | En az 100 puan kullanılabilir |
| Puan Değeri | 100 Puan = 10 TL | Her 100 puan 10 TL değerinde |
| Maksimum Kullanım | Sepet tutarının %50'si | Tek seferde en fazla sepet tutarının yarısı kadar |
| Nakit İadesi | Yok | Puanlar nakde çevrilemez |

### Puan Geçerliliği

```
┌─────────────────────────────────────────────────────────────────┐
│                    PUAN GEÇERLİLİK TAKVİMİ                      │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Kazanım: Ocak 2025           →    Son Kullanım: Ocak 2026     │
│  Kazanım: Haziran 2025        →    Son Kullanım: Haziran 2026  │
│  Kazanım: Aralık 2025         →    Son Kullanım: Aralık 2026   │
│                                                                 │
│  ⚠️ Süresi dolan puanlar otomatik silinir                       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## Kampanya Türleri

### 1. İndirim Kampanyaları

| Kampanya Tipi | Açıklama | Örnek |
|---------------|----------|-------|
| Yüzde İndirim | Belirli oranda indirim | %20 indirim |
| Tutar İndirim | Sabit tutar indirim | 100 TL indirim |
| X Al Y Öde | Çoklu alım indirimi | 3 Al 2 Öde |
| İkinci Üründe İndirim | İkinci üründe indirim | 2. üründe %50 |

### 2. Puan Kampanyaları

| Kampanya Tipi | Açıklama | Örnek |
|---------------|----------|-------|
| Çarpanlı Puan | Puan kazanım çarpanı | 2x, 3x puan |
| Bonus Puan | Ek sabit puan | 500 bonus puan |
| Kategori Puanı | Belirli kategoride ekstra puan | Gömlek alımında 3x puan |

### 3. Kupon Kampanyaları

| Kampanya Tipi | Açıklama | Örnek |
|---------------|----------|-------|
| Welcome Kupon | Yeni üye kuponu | 50 TL hoş geldin kuponu |
| Birthday Kupon | Doğum günü kuponu | %15 doğum günü kuponu |
| Winback Kupon | Pasif müşteri kuponu | 100 TL geri dön kuponu |
| Segment Kupon | Segmente özel kupon | VIP müşterilere özel |

### 4. Ücretsiz Ürün/Hizmet

| Kampanya Tipi | Açıklama | Örnek |
|---------------|----------|-------|
| Hediye Ürün | Belirli alışverişte hediye | 1.000 TL'ye çorap hediye |
| Ücretsiz Kargo | Online siparişlerde | 500 TL üzeri ücretsiz kargo |
| Ücretsiz Tadilat | Ürün tadilatı | Takım elbisede ücretsiz tadilat |

---

## Kupon Yönetimi

### Kupon Özellikleri

| Özellik | Açıklama |
|---------|----------|
| Kupon Kodu | Benzersiz alfanumerik kod |
| İndirim Tipi | Yüzde (%) veya Tutar (TL) |
| Minimum Sepet | Kuponun geçerli olması için minimum tutar |
| Maksimum İndirim | İndirim üst limiti (yüzde kuponlarda) |
| Geçerlilik | Başlangıç - Bitiş tarihi |
| Kullanım Limiti | Tek kullanım / Çoklu kullanım |
| Geçerli Ürünler | Tüm ürünler / Belirli kategoriler |
| Hariç Ürünler | Kampanya dışı ürünler |

### Kupon Durumları

| Durum | Açıklama |
|-------|----------|
| Aktif | Kullanıma hazır |
| Kullanıldı | Zaten kullanılmış |
| Süresi Doldu | Geçerlilik tarihi geçmiş |
| İptal Edildi | Manuel olarak iptal edilmiş |
| Kilitli | Belirli koşul sağlanana kadar kilitli |

### Kupon Kullanım Kuralları

| Kural | Açıklama |
|-------|----------|
| Tek kupon | Bir satışta tek kupon kullanılabilir |
| Puan + Kupon | Aynı satışta hem puan hem kupon kullanılabilir |
| İndirimli ürün | İndirimli ürünlerde kupon geçerli olmayabilir |
| İade durumu | Kuponlu satış iadesinde kupon iade edilmez |

---

## Süreç Adımları

### 1. Loyalty Üyelik Kaydı

**Adım 1:** Müşteriye programı tanıtın

> "Aydınlı Club üyeliğimiz tamamen ücretsiz. Her alışverişte puan kazanır, özel kampanyalardan yararlanırsınız."

**Adım 2:** CRM'de yeni müşteri kaydı oluşturun

**Adım 3:** KVKK onaylarını alın

**Adım 4:** Loyalty programına kayıt kutusunu işaretleyin

**Adım 5:** Welcome bonus tanımlandıysa müşteriye bildirin

**Adım 6:** Müşteriye üyelik bilgilerini verin

---

### 2. Satışta Puan Kazandırma

**Adım 1:** Satış işlemini başlatın

**Adım 2:** Müşteriyi telefon numarası ile arayın

**Adım 3:** Müşteriyi satışa eşleştirin

**Adım 4:** Satışı tamamlayın

**Adım 5:** Sistem otomatik puan hesaplar

**Adım 6:** Kazanılan puan fişte görünür

```
┌─────────────────────────────────────────┐
│           SATIŞ FİŞİ                    │
├─────────────────────────────────────────┤
│  Ürün: Polo Tişört         350,00 TL   │
│  Ürün: Keten Pantolon      650,00 TL   │
│  ─────────────────────────────────────  │
│  TOPLAM:                  1.000,00 TL   │
│  ─────────────────────────────────────  │
│  Kazanılan Puan:              1.000    │
│  Toplam Puan Bakiyesi:        3.500    │
└─────────────────────────────────────────┘
```

---

### 3. Puan Kullanımı

**Adım 1:** Müşteriyi satışa eşleştirin

**Adım 2:** "Puan Kullan" seçeneğine tıklayın

**Adım 3:** Kullanılacak puan miktarını girin

| Seçenek | Açıklama |
|---------|----------|
| Tümünü Kullan | Müsait tüm puanları kullan |
| Tutar Gir | Belirli TL değerinde puan kullan |
| Puan Gir | Belirli puan miktarı gir |

**Adım 4:** İndirimli tutarı onaylayın

**Adım 5:** Ödemeyi alın

```
┌─────────────────────────────────────────┐
│           SATIŞ FİŞİ                    │
├─────────────────────────────────────────┤
│  TOPLAM:                    500,00 TL   │
│  Kullanılan Puan: 2.000    -200,00 TL   │
│  ─────────────────────────────────────  │
│  ÖDENECEK:                  300,00 TL   │
└─────────────────────────────────────────┘
```

---

### 4. Kupon Kullanımı

**Adım 1:** Müşteriyi satışa eşleştirin

**Adım 2:** "Kuponlar" sekmesini açın

**Adım 3:** Müşterinin aktif kuponlarını görüntüleyin

**Adım 4:** Uygun kuponu seçin

**Adım 5:** Kupon koşullarını kontrol edin:
- [ ] Minimum sepet tutarı sağlandı mı?
- [ ] Ürünler kupon kapsamında mı?
- [ ] Kupon süresi geçerli mi?

**Adım 6:** "Kuponu Uygula" butonuna tıklayın

**Adım 7:** İndirimli tutarı doğrulayın

**Adım 8:** Ödemeyi alın

---

### 5. Kampanya Uygulama

**Adım 1:** Güncel kampanyaları kontrol edin

**Adım 2:** Müşterinin kampanyaya uygunluğunu değerlendirin

**Adım 3:** Uygun kampanyayı müşteriye bildirin

**Adım 4:** Sepet koşullarını sağlayın

**Adım 5:** Sistem otomatik kampanyayı uygular

**Adım 6:** Kampanya indirimi fişte görünür

---

## Ekran Görüntüleri

### Loyalty Dashboard

![Loyalty Dashboard - Placeholder](screenshots/crm-loyalty-dashboard.png)

*Ekran görüntüsü eklenecektir.*

### Puan İşlem Ekranı

![Puan İşlem - Placeholder](screenshots/crm-loyalty-points.png)

*Ekran görüntüsü eklenecektir.*

### Kupon Listesi

![Kupon Listesi - Placeholder](screenshots/crm-loyalty-coupons.png)

*Ekran görüntüsü eklenecektir.*

---

## KPI'lar

### Loyalty Program KPI'ları

| KPI | Tanım | Formül | Hedef |
|-----|-------|--------|-------|
| **Loyalty Penetration** | Loyalty üyesi satış oranı | (Üye Satış Adedi / Toplam Satış Adedi) × 100 | >70% |
| **Enrollment Rate** | Yeni üye kayıt oranı | (Yeni Üye / Yeni Müşteri) × 100 | >50% |
| **Active Member Rate** | Aktif üye oranı | (12 Ay İçinde Alışveriş Yapan / Toplam Üye) × 100 | >40% |
| **Redemption Rate** | Puan kullanım oranı | (Kullanılan Puan / Kazanılan Puan) × 100 | >30% |
| **Points Liability** | Kullanılmamış puan değeri | Toplam Açık Puan × Puan Değeri | Takip |

### Kampanya KPI'ları

| KPI | Tanım | Formül | Hedef |
|-----|-------|--------|-------|
| **Campaign Response Rate** | Kampanya katılım oranı | (Kampanyaya Katılan / Hedef Kitle) × 100 | >15% |
| **Coupon Redemption Rate** | Kupon kullanım oranı | (Kullanılan Kupon / Dağıtılan Kupon) × 100 | >20% |
| **Incremental Sales** | Kampanyanın yarattığı ek satış | Kampanya Satışı - Baz Satış | Pozitif |
| **ROI** | Kampanya yatırım getirisi | (Ek Gelir - Kampanya Maliyeti) / Kampanya Maliyeti | >2x |

### Hesaplama Örnekleri

**Loyalty Penetration:**
```
Üyeli satış adedi: 700
Toplam satış adedi: 1.000
Loyalty Penetration = (700 / 1.000) × 100 = %70
```

**Redemption Rate:**
```
Bu ay kullanılan puan: 500.000
Bu ay kazanılan puan: 1.500.000
Redemption Rate = (500.000 / 1.500.000) × 100 = %33
```

---

## Sık Karşılaşılan Sorunlar ve Çözümler

### Sorun 1: Kupon Çalışmıyor

**Kontrol Listesi:**
- [ ] Kupon süresi dolmuş mu?
- [ ] Minimum sepet tutarı sağlandı mı?
- [ ] Ürünler kupon kapsamında mı?
- [ ] Kupon daha önce kullanılmış mı?
- [ ] Müşteri eşleştirmesi yapıldı mı?

**Çözüm:** Koşulları kontrol edin, sağlanmıyorsa müşteriye açıklayın.

---

### Sorun 2: Puanlar Görünmüyor

**Olası Nedenler:**
- Satış müşteriye eşleştirilmemiş
- Sistem senkronizasyonu gecikmeli
- Müşteri farklı profilden bakıyor

**Çözüm:**
1. Satış geçmişinden eşleştirmeyi kontrol edin
2. 15-30 dakika bekleyin
3. Sorun devam ederse ticket açın

---

### Sorun 3: Müşteri Yanlış Puan Kullandı

**Çözüm:**
1. İşlemi iptal edin (mağaza müdürü onayı gerekir)
2. Yeni işlem oluşturun
3. Doğru puan miktarını kullandırın
4. Durumu ticket ile belgeleyin

---

### Sorun 4: Kampanya Uygulanmıyor

**Kontrol Listesi:**
- [ ] Kampanya tarihleri geçerli mi?
- [ ] Müşteri hedef segmentte mi?
- [ ] Sepet koşulları sağlanıyor mu?
- [ ] Ürünler kampanya kapsamında mı?

**Çözüm:** Kampanya koşullarını CRM'den kontrol edin.

---

## Mağaza İpuçları

### Loyalty Satışı Artırma

1. **Her müşteriye sorun:** "Aydınlı Club üyemiz misiniz?"
2. **Avantajları anlatın:** "Şu an 500 puan kazanabilirsiniz"
3. **Puanları hatırlatın:** "3.500 puanınız var, bugün kullanmak ister misiniz?"
4. **Kampanyaları duyurun:** "Üyelere özel %20 indirim var"

### Üyelik Kaydını Teşvik

| Yaklaşım | Örnek |
|----------|-------|
| Anlık avantaj | "Hemen üye olun, 50 TL'lik hoş geldin kuponu kazanın" |
| Gelecek avantaj | "Bir sonraki alışverişinizde kullanacağınız puanlar kazanın" |
| Özel gün | "Doğum gününüzde özel indirim kuponu gönderelim" |
| Erken erişim | "Yeni koleksiyonları herkesten önce görün" |

### Kampanya Uygulama Kuralları

| Yapın | Yapmayın |
|-------|----------|
| ✅ Kampanya koşullarını açıklayın | ❌ Koşulları gizlemeyin |
| ✅ Müşteriye en avantajlı seçeneği önerin | ❌ Müşteriyi yanıltmayın |
| ✅ Kupon/puan kombinasyonunu kontrol edin | ❌ Sistem dışı indirim uygulamayın |
| ✅ İşlem sonrası kazanımları bildirin | ❌ Geçersiz kupon kabul etmeyin |

### Sorunlu Durumlar

| Durum | Yaklaşım |
|-------|----------|
| Müşteri kampanyayı kaçırmış | Gelecek kampanyadan haberdar edin |
| Puan süresi dolmuş | Anlayışla karşılayın, yeni kazanım teşvik edin |
| Kupon çalışmıyor | Koşulları sabırla açıklayın |
| Sistem hatası | Özür dileyin, ticket açın, müşteriyi takip için bilgilendirin |

---

## Revizyon Geçmişi

| Versiyon | Tarih | Değişiklik | Hazırlayan |
|----------|-------|------------|------------|
| 1.0 | 01.12.2025 | İlk sürüm oluşturuldu | Aydınlı Grup |

---

*Bu doküman Aydınlı Grup CRM Operasyonları tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*


