---
layout: default
title: "Operasyonlar (TR)"
parent: Consolidated Documentation
nav_order: 4
---

# Operasyonlar - Tam Dokümantasyon (Türkçe)

> **Konsolide Doküman**  
> **Son Güncelleme:** 8 Aralık 2025  
> **Hazırlayan:** Aydınlı Grup
>
> **Kaynak Dosyalar:** store-operations, supply-chain klasörlerinden 13 dosya

---

## Doküman Genel Bakış

Bu kapsamlı operasyon dokümantasyonu, Aydınlı Grup franchise mağazaları için mağaza operasyonları ve tedarik zinciri yönetiminin tüm yönlerini kapsamaktadır. Doküman, genel bakıştan spesifik prosedürlere mantıksal bir hiyerarşi izleyerek 13 ana bölümden oluşmaktadır:

**Bölüm 1: Mağaza Operasyonları Genel Bakış** - Mağaza yönetimi ve personel koordinasyonunu yöneten tam operasyonel çerçeveyi, organizasyon yapısını, temel sorumlulukları ve günlük iş akışı süreçlerini sunar.

**Bölüm 2: Tedarik Zinciri Genel Bakış** - Tedarikçi ilişkileri, dağıtım ağı, envanter akışı ve mağazalar, depolar ve merkez arasındaki koordinasyon dahil tedarik zinciri ekosisteminin kapsamlı görünümünü sağlar.

**Bölüm 3: Açılış ve Kapanış Prosedürleri** - Sistem kontrolleri, kasa kurulumu, güvenlik protokolleri ve gün sonu mutabakat süreçleri dahil günlük mağaza açılış ve kapanış operasyonları için adım adım kontrol listeleri sağlar.

**Bölüm 4: Nakit Yönetimi Kılavuzu** - Kasa operasyonları, kasa yönetimi, banka yatırmaları, bozuk para sipariş etme, fark yönetimi ve dolandırıcılık önleme protokolleri dahil tüm nakit kullanım prosedürlerini detaylandırır.

**Bölüm 5: Müşteri Hizmetleri Standartları** - Karşılama protokolleri, şikayet yönetimi, hizmet telafisi prosedürleri ve müşteri etkileşim yönergeleri dahil hizmet mükemmelliği standartlarını belirler.

**Bölüm 6: Envanter Kontrol Kılavuzu** - Stok sayım metodları, fire kontrolü, döngüsel sayım prosedürleri, stok doğruluk hedefleri ve optimal stok seviyelerini korumak için envanter raporlama gerekliliklerini kapsar.

**Bölüm 7: Zayi Önleme Kılavuzu** - Güvenlik önlemlerini, hırsızlık önleme stratejilerini, çalışan farkındalık protokollerini, gözetim sistemi kullanımını ve fireyi minimize etmek ve mağaza varlıklarını korumak için olay raporlama prosedürlerini özetler.

**Bölüm 8: İade ve Değişim Prosedürleri** - Onay gereksinimleri, dokümantasyon ihtiyaçları ve iade edilen ürünler için kalite kontrol kontrolleri dahil müşteri iadelerini, değişimlerini ve geri ödemelerini yönetmek için politika ve süreçleri detaylandırır.

**Bölüm 9: Sipariş Yönetim Kılavuzu** - Sipariş oluşturma prosedürleri, takip sistemleri, iletişim protokolleri ve sipariş sorun çözümü dahil oluşturmadan karşılamaya kadar tam sipariş yaşam döngüsünü kapsar.

**Bölüm 10: Teslimat ve Kabul Prosedürleri** - Teslimat kabulü, kalite denetimi, dokümantasyon gereksinimleri, tutarsızlık çözümü ve teslim alınan malların uygun depolanması dahil gelen sevkiyat işlemlerini açıklar.

**Bölüm 11: Stok Transfer Prosedürleri** - Transfer talep prosedürleri, onay iş akışları, dokümantasyon gereksinimleri ve transfer edilen mallar için envanter mutabakatı dahil mağazalar arası ve depo transfer süreçlerini tanımlar.

**Bölüm 12: Sevkiyat ve Lojistik Kılavuzu** - Paketleme standartları, taşıyıcı seçimi, sevkiyat dokümantasyonu, takip prosedürleri ve dağıtım merkezleri ile lojistik koordinasyon dahil giden sevkiyat süreçlerini detaylandırır.

**Bölüm 13: Depo Koordinasyon Kılavuzu** - Stok talep prosedürleri, teslimat planlaması, acil stok süreçleri ve depo sistemi kullanımı dahil merkezi depolar ile iletişim ve koordinasyon protokollerini açıklar.

---

## SECTION 1: STORE-OPERATIONS-OVERVIEW-TR

﻿---
layout: default
title: "Mağaza Operasyonları Genel Bakış – TR"
parent: Store Operations

## İçindekiler

1. [Hızlı Bakış (EN/TR Toggle)](#hızlı-bakış-entr-toggle)
2. [Amaç](#amaç)
3. [Kapsam](#kapsam)
4. [Tanımlar](#tanımlar)
5. [Sorumluluklar](#sorumluluklar)
6. [Süreç Adımları](#süreç-adımları)
7. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
8. [Kontrol Listeleri](#kontrol-listeleri)
9. [Örnekler / Senaryolar](#örnekler--senaryolar)
10. [KPI'lar](#kpilar)
11. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
12. [Revizyon Geçmişi](#revizyon-geçmişi)


## Amaç

Bu doküman, tüm Aydınlı Grup franchise mağazaları için mağaza operasyonları standartları, prosedürleri ve en iyi uygulamalarına kapsamlı bir genel bakış sunmaktadır. Etkili mağaza operasyonları, tüm lokasyonlarda tutarlı müşteri deneyimleri, operasyonel verimlilik ve iş karlılığını sağlar.

### Operasyon Misyonu

Şunlar aracılığıyla olağanüstü müşteri deneyimleri sunmak:
- **Tutarlılık:** Tüm mağazalarda standartlaştırılmış operasyonlar
- **Verimlilik:** Optimize edilmiş süreçler ve kaynak kullanımı
- **Uyum:** Marka ve düzenleyici standartlara bağlılık
- **Mükemmellik:** Hizmet sunumunda sürekli iyileştirme


## Tanımlar

| Terim (EN) | Terim (TR) | Tanım |
|------------|------------|-------|
| SOP | Standart İşletim Prosedürü | Standart Operasyon Prosedürü |
| POS | Satış Noktası | Satış Noktası sistemi |
| KPI | Anahtar Performans Göstergesi | Temel Performans Göstergesi |
| UPT | İşlem Başına Ürün | İşlem Başına Ürün Adedi |
| ATV | Ortalama İşlem Değeri | Ortalama İşlem Değeri |
| Conversion Rate | Dönüşüm Oranı | Satın alma yapan ziyaretçiler |
| Shrinkage | Fire | Hırsızlık veya hatadan kaynaklanan envanter kaybı |
| Planogram | Planogram | Görsel ürün yerleşim kılavuzu |
| Float | Kasa Açılış Parası | Kasada başlangıç nakit |
| Z-Report | Z Raporu | Gün sonu kasa raporu |


## Süreç Adımları

### Günlük Operasyon Döngüsü

```
06:00-09:00  AÇILIŞ ÖNCESİ
├── Güvenlik kontrolü ve alarm devre dışı bırakma
├── Işık ve sistem aktivasyonu
├── Açılış nakit doğrulaması
├── Mağaza turu ve sunum kontrolü
└── Ekip brifingi ve görev ataması

09:00-12:00  SABAH OPERASYONLARI
├── Mağaza müşterilere açılıyor
├── Müşteri hizmeti odağı
├── Teslimatlar alınıyor ve işleniyor
├── VM bakımı ve ürün yenileme
└── Sabah satış takibi

12:00-14:00  ÖĞLEN GEÇİŞİ
├── Öğle arası rotasyonu
├── Kasa mutabakatı (gerekirse)
├── Hızlı mağaza reset
└── Öğleden sonra ekip brifingi

14:00-18:00  ÖĞLEDEN SONRA YOĞUN SAAT
├── Tam ekip satış alanında
├── Yoğun trafik yönetimi
├── Aktif satış ve hizmet
├── Müşteriler arasında VM bakımı
└── Anlık satış izleme

18:00-21:00  AKŞAM OPERASYONLARI
├── Akşam müşteri akışı yönetimi
├── Kapanış hazırlığına başla
├── Kabin temizleme
└── Son satış hamleleri

21:00-22:00  KAPANIŞ
├── Son müşteri hizmeti
├── Mağaza güvenliği
├── Kasa kapanışı ve Z-raporu
├── Son mağaza turu
└── Alarm aktivasyonu ve çıkış
```

### Haftalık Operasyon Ritmi

| Gün | Odak Alanı | Ana Aktiviteler |
|-----|------------|-----------------|
| **Pazartesi** | Hafta Başlangıcı | KPI inceleme, ekip toplantısı, görsel reset |
| **Salı** | Operasyonlar | Teslimat işleme, envanter görevleri |
| **Çarşamba** | Hafta Ortası | Eğitim, derin temizlik, idari görevler |
| **Perşembe** | Hazırlık | Hafta sonu hazırlığı, stok kontrolü, VM yenileme |
| **Cuma** | Hafta Sonu Başlangıcı | Yoğunluk hazırlığı, tam ekip brifingi |
| **Cumartesi** | Yoğun Ticaret | Maksimum hizmet, tüm ekip sahada |
| **Pazar** | Hafta Kapanışı | Özet, gelecek hafta planlaması, derin reset |


## Kontrol Listeleri

### Günlük Açılış Kontrol Listesi

```
GÜVENLİK VE SİSTEMLER
□ Yetkili kodla alarmı devre dışı bırak
□ Zorlanmış giriş veya hasar izleri kontrol et
□ Tüm ışıkları aç (satış alanı, depo)
□ POS sistemlerini aç
□ Gerekli tüm sistemlere giriş yap

NAKİT VE BANKA
□ Kasadan açılış nakitini al
□ Açılış tutarını say ve doğrula
□ Açılış nakitini kasaya yerleştir
□ Açılış nakitini kasaya kaydet

MAĞAZA SUNUMU
□ Tam mağaza turu
□ Tüm sergilerin bütünlüğünü kontrol et
□ Manken styling'ini doğrula
□ Kabinleri kontrol et (temiz, boş)
□ Müziğin doğru seviyede çaldığından emin ol

EKİP VE İLETİŞİM
□ Önceki günün performansını incele
□ HQ iletişimlerini kontrol et
□ Ekibe bölge ve görev ata
□ Ekibe günlük hedefleri ve odağı brifingle
```

### Günlük Kapanış Kontrol Listesi

```
MÜŞTERİ VE SATIŞ ALANI
□ Son müşterilere yardım et
□ Kapanıştan 15 dk önce kabinleri kapat
□ Son kabin kontrolü ve temizliği
□ Kapanış saatinde girişi kilitle

NAKİT VE RAPORLAMA
□ Bekleyen tüm işlemleri tamamla
□ Z-raporunu çıkart
□ Nakiti say ve mutabakat yap
□ Banka yatırımını hazırla
□ Nakiti kasada güvenceye al

MAĞAZA GÜVENLİĞİ
□ Tam mağaza turu
□ Tüm pencere ve kapıları kontrol et
□ Gereksiz ışıkları kapat
□ POS'u kapat (yerel protokole göre)
□ Alarmı ayarla
□ Tüm kilitleri tekrar kontrol et

ERTESI GÜN HAZIRLIĞI
□ Sonraki vardiya için sorunları not et
□ Personel programını kontrol et
□ Özel görev listesi hazırla
```


## KPI'lar

### Mağaza Performans Metrikleri

| KPI | Formül | Hedef | Sıklık |
|-----|--------|-------|--------|
| **Satışlar** | Günlük/Haftalık/Aylık | Bütçe | Günlük |
| **Dönüşüm Oranı** | İşlemler / Trafik | ≥%25 | Günlük |
| **UPT** | Ürün / İşlem | ≥2.0 | Günlük |
| **ATV** | Satışlar / İşlem | Marka benchmark | Günlük |
| **Fire** | Kayıp envanter / Satışlar | <%1 | Yıllık |
| **Müşteri Memnuniyeti** | Anket puanı | ≥%85 | Aylık |
| **Personel Verimliliği** | Satışlar / Personel saatleri | Benchmark | Haftalık |

### KPI Panosu Örneği

```
┌─────────────────────────────────────────────────────────────┐
│              GÜNLÜK MAĞAZA PERFORMANSI                      │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Hedefe Karşı Satış   [██████████████████░░░░] %82         │
│  Dönüşüm Oranı        [████████████████████░░] %27         │
│  UPT                  [████████████████████████] 2.3        │
│  ATV                  [███████████████████░░░░] $68         │
│                                                             │
│  Trafik: 245 ziyaretçi                                      │
│  İşlemler: 66                                               │
│  Toplam Satış: $4,488                                       │
│                                                             │
│  Durum: YOLUNDA - Öğleden sonra dönüşüme odaklan           │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```


### Sorun 2: Nakit Tutarsızlıkları

**Problem:** Kapanışta kasa fazla/eksik

**Çözümler:**
- Şahit ile yeniden say
- İşlem geçmişini incele
- İptal edilen işlemleri kontrol et
- Tüm ödeme tiplerini doğrula
- Politikaya göre belgele ve raporla


## Revizyon Geçmişi

| Versiyon | Tarih | Değişiklik | Hazırlayan |
|----------|-------|------------|------------|
| 1.0 | 2025-12-01 | İlk versiyon oluşturuldu | Aydınlı Grup |


---

## SECTION 2: SUPPLY-CHAIN-OVERVIEW-TR

﻿---
layout: default
title: "Tedarik Zinciri ve Lojistik Genel Bakış – TR"
parent: Supply Chain

## İçindekiler

1. [Hızlı Bakış (EN/TR Toggle)](#hızlı-bakış-entr-toggle)
2. [Amaç](#amaç)
3. [Kapsam](#kapsam)
4. [Tanımlar](#tanımlar)
5. [Sorumluluklar](#sorumluluklar)
6. [Süreç Adımları](#süreç-adımları)
7. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
8. [Kontrol Listeleri](#kontrol-listeleri)
9. [Örnekler / Senaryolar](#örnekler--senaryolar)
10. [KPI'lar](#kpilar)
11. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
12. [Revizyon Geçmişi](#revizyon-geçmişi)

## Amaç

Bu doküman, Aydınlı Grup franchise ortakları için tedarik zinciri ve lojistik operasyonlarına kapsamlı bir genel bakış sunmaktadır. Verimli tedarik zinciri yönetimi, tüm perakende lokasyonlarında ürün bulunabilirliği, envanter optimizasyonu ve müşteri memnuniyetini sağlar.

### Tedarik Zinciri Misyonu

"Doğru ürünü, doğru yere, doğru zamanda, doğru miktarda, doğru maliyetle teslim etmek."

### Temel Hedefler

| Hedef | Etki |
|-------|------|
| **Ürün Bulunabilirliği** | Stoksuzluğu minimize et |
| **Envanter Optimizasyonu** | Fazla stoğu azalt |
| **Pazara Hız** | Hızlı yenileme |
| **Maliyet Verimliliği** | Lojistik maliyetleri minimize et |
| **Doğruluk** | Hatasız teslimatlar |


## Tanımlar

| Terim (EN) | Terim (TR) | Tanım |
|------------|------------|-------|
| DC | Dağıtım Merkezi | Distribution Center |
| Lead Time | Tedarik Süresi | Siparişten teslimata süre |
| SKU | Stok Kodu | Stok Tutma Birimi |
| MOQ | Minimum Sipariş | Minimum Sipariş Miktarı |
| PO | Satınalma Siparişi | Purchase Order |
| ASN | Ön Sevk Bildirimi | Önceden Sevkiyat Bildirimi |
| BOL | Konşimento | Bill of Lading |
| FIFO | İlk Giren İlk Çıkar | İlk Giren, İlk Çıkar |
| Safety Stock | Güvenlik Stoğu | Tampon envanter |
| Reorder Point | Yeniden Sipariş Noktası | Yeniden siparişi tetikleyen envanter seviyesi |


## Süreç Adımları

### Sipariş Döngüsü

```
GÜN 1: SİPARİŞ GÖNDERİMİ
├── Mağaza envanteri inceler
├── Yenileme ihtiyaçlarını belirler
├── B2B portalı üzerinden sipariş gönderir
└── Sipariş son tarihi: 18:00 yerel saat

GÜN 2: SİPARİŞ İŞLEME
├── HQ siparişi inceler
├── Envanter durumunu kontrol eder
├── Siparişi onaylar veya düzeltir
└── Sipariş onayı gönderir

GÜN 3-4: TOPLAMA VE PAKETLEME
├── Depo siparişi toplar
├── Kalite kontrolü
├── Standartlara göre paketleme
└── Sevkiyat belgelerini oluşturur

GÜN 4-5: SEVKİYAT
├── Taşıyıcı teslim alımı
├── Transit takibi
└── Mağazaya ETA bildirimi

GÜN 5-7: TESLİMAT VE KABUL
├── Mağazaya teslimat
├── Mağaza kabul süreci
├── Envanter güncelleme
└── Tutarsızlık raporlama
```

### Sezonluk Planlama Döngüsü

| Aşama | Zamanlama | Aktiviteler |
|-------|-----------|-------------|
| **Planlama** | T-6 ay | Tahmin, bütçeleme |
| **Sipariş** | T-4 ay | Üretim siparişleri |
| **Üretim** | T-3 ay | İmalat |
| **Sevkiyat** | T-2 ay | DC'ye lojistik |
| **Dağıtım** | T-1 ay | Mağaza tahsisleri |
| **Lansman** | T-Günü | Sezon başlar |


## Kontrol Listeleri

### Haftalık Sipariş Kontrol Listesi

```
HAFTALIK SİPARİŞ HAZIRLIĞI
Tarih: _____________ Mağaza: _____________

ENVANTER İNCELEME
□ Mevcut stok seviyeleri kontrol edildi
□ Satış hızı analiz edildi
□ Düşük stok ürünleri belirlendi
□ Fazla stok ürünleri not edildi

SİPARİŞ PLANLAMA
□ Gerekli miktarlar hesaplandı
□ Beden oranları değerlendirildi
□ Kampanya ihtiyaçları dahil edildi
□ Yaklaşan etkinlikler değerlendirildi

SİPARİŞ GÖNDERİMİ
□ Sipariş B2B portalına girildi
□ Miktarlar doğrulandı
□ Teslimat adresi onaylandı
□ Sipariş son tarihine kadar gönderildi

ONAY
□ Sipariş onayı alındı
□ Onaylanan miktarlar eşleşiyor
□ Beklenen teslimat tarihi not edildi
□ Düzeltmeler kabul edildi

Sipariş Numarası: _____________
Beklenen Teslimat: _____________
```

### Teslimat Kabul Kontrol Listesi

```
TESLİMAT KABUL KONTROL LİSTESİ
Tarih: _____________ Teslimat #: _____________

TESLİMAT ÖNCESİ
□ Kabul alanı hazırlandı
□ Personel atandı
□ Beklenen ürün listesi hazır
□ Tarayıcı/ekipman hazır

VARIŞ KONTROLÜ
□ Taşıyıcı ve araç doğrulandı
□ Mühür numarası ASN ile eşleşiyor
□ Dış hasar kontrolü
□ Kutu sayısı eşleşiyor

KABUL SÜRECİ
□ Her kutu açıldı ve kontrol edildi
□ Ürünler sisteme tarandı
□ Miktarlar doğrulandı
□ Hasarlı ürünler ayrıldı
□ Eksik ürünler belgelendi
□ Fazla ürünler belgelendi

TAMAMLAMA
□ Teslimat notlarla imzalandı
□ Tutarsızlık raporu dosyalandı (varsa)
□ Ürünler stoğa taşındı
□ Envanter sistemi güncellendi
□ Evraklar dosyalandı

Beklenen Toplam Adet: _____
Alınan Toplam Adet: _____
Hasarlı Adet: _____
Fark: _____

Kabul Eden: _____________
Müdür: _____________
```


## KPI'lar

### Tedarik Zinciri Metrikleri

| KPI | Formül | Hedef | Sıklık |
|-----|--------|-------|--------|
| **Sipariş Doğruluğu** | Doğru ürün / Toplam ürün | ≥%99 | Teslimat başına |
| **Zamanında Teslimat** | Zamanında teslimat / Toplam | ≥%95 | Aylık |
| **Doldurma Oranı** | Sevk edilen / Sipariş edilen | ≥%97 | Aylık |
| **Envanter Devir Hızı** | SMM / Ort envanter | Yılda 4-6x | Üç aylık |
| **Stoksuzluk Oranı** | Stoksuz SKU / Toplam | <%3 | Haftalık |
| **Kabul Doğruluğu** | Doğru kabuller / Toplam | %100 | Teslimat başına |

### Tedarik Zinciri Panosu

```
┌─────────────────────────────────────────────────────────────┐
│              AYLIK TEDARİK ZİNCİRİ PERFORMANSI              │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Zamanında Teslimat    [████████████████████░░] %96        │
│  Hedef: ≥%95           ✓ KARŞILIYOR                         │
│                                                             │
│  Sipariş Doldurma Oranı[████████████████████░░] %98        │
│  Hedef: ≥%97           ✓ AŞIYOR                             │
│                                                             │
│  Kabul Doğruluğu       [████████████████████████] %100     │
│  Hedef: %100           ✓ KARŞILIYOR                         │
│                                                             │
│  Stoksuzluk Oranı      [██░░░░░░░░░░░░░░░░░░░░] %2.1       │
│  Hedef: <%3            ✓ HEDEF İÇİNDE                       │
│                                                             │
│  Bu Ay En Çok Sorunlar:                                     │
│  1. Taşıyıcı gecikmesi (2 sevkiyat)                        │
│  2. DC stok eksikliği (3 SKU)                              │
│  3. Adres hatası (1 teslimat)                              │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```


### Sorun 2: Teslimat Gecikmeleri

**Problem:** Sevkiyatlar geç varıyor

**Çözümler:**
- Sevkiyatları proaktif izle
- Taşıyıcıyla iletişim kur
- Tampon süre planla
- Alternatif taşıyıcıları düşün
- Tekrarlayan sorunları eskalasyon yap


## Revizyon Geçmişi

| Versiyon | Tarih | Değişiklik | Hazırlayan |
|----------|-------|------------|------------|
| 1.0 | 2025-12-01 | İlk versiyon oluşturuldu | Aydınlı Grup Tedarik Zinciri |


---

## SECTION 3: OPENING-CLOSING-PROCEDURES-TR

﻿---
layout: default
title: "Açılış ve Kapanış Prosedürleri – TR"
parent: Store Operations

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Süreç Adımları](#süreç-adımları)
6. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
7. [Kontrol Listeleri](#kontrol-listeleri)
8. [Örnekler / Senaryolar](#örnekler--senaryolar)
9. [KPI'lar](#kpilar)
10. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
11. [Revizyon Geçmişi](#revizyon-geçmişi)


## Kapsam

### Uygulama

Bu prosedürler şunlara uygulanır:
- Tüm mağaza formatları (Flagship, Standart, Kompakt, Shop-in-Shop, Outlet)
- Tüm coğrafi bölgeler
- Özel ticaret saatleri dahil tüm işletim günleri

### Çalışma Saatleri

| Gün | Standart Saatler | AVM Saatleri | Tatil Saatleri |
|-----|------------------|--------------|----------------|
| Pazartesi-Cuma | 10:00-21:00 | AVM'ye göre | Değişken |
| Cumartesi | 10:00-22:00 | AVM'ye göre | Değişken |
| Pazar | 11:00-20:00 | AVM'ye göre | Değişken |

*Not: Saatler lokasyon ve yerel düzenlemelere göre değişir*


## Sorumluluklar

### Anahtar Sorumlusu (Açılış)

| Görev | Zaman Dilimi | Hesap Verebilirlik |
|-------|--------------|-------------------|
| Açılıştan önce varış | -45 dakika | Dakiklik |
| Alarmı devre dışı bırak | Hemen | Güvenlik |
| Güvenlik turu | İlk 10 dk | Emniyet |
| Nakit kurulumu | Açılış öncesi | Doğruluk |
| Sistem başlatma | Açılış öncesi | Hazırlık |
| Ekip koordinasyonu | Açılış öncesi | Liderlik |

### Anahtar Sorumlusu (Kapanış)

| Görev | Zaman Dilimi | Hesap Verebilirlik |
|-------|--------------|-------------------|
| Müşteri çıkışı | Kapanış saatinde | Hizmet |
| Nakit mutabakatı | Son satıştan sonra | Doğruluk |
| Güvenlik turu | Ayrılmadan önce | Emniyet |
| Sistem kapatma | Protokole göre | Uyum |
| Alarm aktivasyonu | Son işlem | Güvenlik |
| Güvenli çıkış | Hemen ardından | Emniyet |


## Standartlar ve Kurallar

### Anahtar Güvenlik Standartları

| Standart | Gereksinim | İhlal Sonucu |
|----------|------------|--------------|
| Alarm yanıt süresi | 30 saniye | Alarm tetiklenir |
| İki kişi kuralı (yüksek değer) | Kasa erişimi için zorunlu | Disiplin işlemi |
| Anahtar hesap verebilirliği | Kişisel sorumluluk | İstihdam incelemesi |
| Alarm kodu gizliliği | Asla paylaşılmaz | Derhal fesih |
| Açılış/kapanış arkadaşı | Önerilir | Yerel politikaya göre |

### Nakit Açılış Tutarı Standartları

| Mağaza Boyutu | Standart Açılış | Aralık |
|---------------|-----------------|--------|
| Küçük | 200 $ | 150-250 $ |
| Orta | 300 $ | 250-400 $ |
| Büyük | 500 $ | 400-600 $ |
| Flagship | 750 $ | 600-1000 $ |

### Zamanlama Standartları

| Aktivite | Maksimum Süre | Notlar |
|----------|---------------|--------|
| Açılış prosedürü | 45 dakika | Varıştan açılışa |
| Kapanış prosedürü | 30 dakika | Kapanıştan çıkışa |
| Nakit sayımı (açılış) | 10 dakika | Kasa başına |
| Nakit sayımı (kapanış) | 15 dakika | Mutabakat dahil |
| Z-raporu tutarsızlığı | Aynı gün | Çözülmeli |


## Örnekler / Senaryolar

### Senaryo 1: Açılışta Alarm Tetikleniyor

**Durum:** Mağazaya girerken alarm çalıyor

**Yanıt Protokolü:**
1. Mağaza içine ilerleme
2. Hemen güvenli mesafeye çık
3. Alarm izleme şirketini ara
4. Tavsiye edilirse polisi ara
5. Mağaza Müdürü / Bölge Müdürünü ara
6. İzin verilene kadar tekrar girme
7. Olayı tam belgele

### Senaryo 2: Kapanışta Müşteri Ayrılmayı Reddediyor

**Durum:** Müşteri kapanıştan 10 dakika sonra hâlâ alışveriş yapıyor

**Yanıt Protokolü:**
1. Müşteriye kibar bir şekilde yaklaş
2. "Üzgünüm, artık kapandık. Satın alımınızı tamamlamanıza yardımcı olabilir miyim?"
3. Gerekirse ürünleri ayırma teklif et
4. Kasaya eşlik et
5. Reddederse müdürü dahil et
6. Asla müşteriyi fiziksel olarak zorla
7. Önemli gecikme varsa belgele

### Senaryo 3: Kapanışta Nakit Farkı

**Durum:** Kasa çekmecesinde 15$ eksik

**Yanıt Protokolü:**
1. Tüm çekmeceyi yeniden say
2. Yanlış sayılmış banknotları kontrol et
3. İptal/iade işlemlerini incele
4. Birbirine yapışmış banknotları kontrol et
5. Farkı formda belgele
6. Müdüre hemen raporla
7. Olay raporu tamamla
8. Yetki olmadan düzeltme yapma


## Sık Karşılaşılan Sorunlar ve Çözümler

### Sorun 1: Alarm Devre Dışı Kalmıyor

**Çözüm:**
1. Kodu dikkatle tekrar dene
2. Doğru kodu kontrol et
3. Alarm şirketini hemen ara
4. Doğrulama için kimlik hazır bulundur
5. Manuel olarak devre dışı bırakma

### Sorun 2: POS Sistemi Başlamıyor

**Çözüm:**
1. Güç bağlantılarını kontrol et
2. Yeniden başlatmayı dene
3. IT desteğini ara
4. Yedek prosedürleri kullan
5. Gerekirse manuel işlem kaydı

### Sorun 3: Nakit Açılış Tutarı Yanlış

**Çözüm:**
1. Şahitle yeniden say
2. Önceki günün kapanışını kontrol et
3. Müdürle iletişime geç
4. Tutarsızlığı belgele
5. Onay olmadan düzeltme yapma


*Bu doküman Aydınlı Grup Mağaza Operasyonları Ekibi tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*




---

## SECTION 4: CASH-MANAGEMENT-GUIDE-TR

﻿---
layout: default
title: "Nakit Yönetimi Kılavuzu – TR"
parent: Store Operations

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Süreç Adımları](#süreç-adımları)
6. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
7. [Kontrol Listeleri](#kontrol-listeleri)
8. [Örnekler / Senaryolar](#örnekler--senaryolar)
9. [KPI'lar](#kpilar)
10. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
11. [Revizyon Geçmişi](#revizyon-geçmişi)


## Kapsam

Bu kılavuz şunları kapsar:
- Kasa operasyonları
- Kasa yönetimi
- Banka yatırımları
- Bozukluk siparişi
- Nakit fark yönetimi
- Dolandırıcılık önleme


## Sorumluluklar

### Satış Personeli

| Görev | Standart | Sıklık |
|-------|----------|--------|
| Nakit işlemleri işleme | Doğru sayım | İşlem başına |
| Alınan banknotları doğrulama | Sahte kontrolü | İşlem başına |
| Çekmece güvenliğini koruma | Kullanılmadığında kapalı | Sürekli |
| Tutarsızlıkları raporlama | Hemen | Gerçekleştiğinde |

### Vardiya Sorumlusu

| Görev | Standart | Sıklık |
|-------|----------|--------|
| Çekmece seviyelerini izleme | Gerektiğinde kasaya bırakma | Sürekli |
| İptal/iadeleri onaylama | Yetki gerekli | Her olayda |
| Gün ortası mutabakatı | Gerekirse | Vardiya başına |
| Nakit çekimleri | Protokole göre | Gerektiğinde |

### Mağaza Müdürü

| Görev | Standart | Sıklık |
|-------|----------|--------|
| Günlük nakit mutabakatı | Sıfır fark | Günlük |
| Banka yatırımları | Aynı/sonraki iş günü | Günlük |
| Kasa envanteri | Doğru sayım | Haftalık |
| Fark araştırması | Tam dokümantasyon | Her olayda |
| Nakit yönetimi eğitimi | Standarda göre | Yeni personel + yenileme |


## Standartlar ve Kurallar

### Nakit İşlem Standartları

| Standart | Gereksinim | Sonuç |
|----------|------------|-------|
| Çekmece erişimi | Sadece atanan kullanıcı | Disiplin işlemi |
| Nakit görünürlüğü | Her zaman görünür şekilde say | Yeniden eğitim |
| Çekmece durumu | Kullanılmadığında kapalı | Sözlü uyarı |
| Büyük banknotlar | Dedektör kalemiyle doğrula | Sahte kabul |
| Fark toleransı | Günlük ±5$ | Araştırma gerekli |

### Mağazaya Göre Açılış Nakiti Standartları

| Mağaza Tipi | Açılış Nakiti | Kasaya Bırakma Eşiği |
|-------------|---------------|----------------------|
| Küçük | 200$ | 300$ |
| Orta | 300$ | 500$ |
| Büyük | 500$ | 750$ |
| Flagship | 750$ | 1.000$ |

### Para Birimi Gereksinimleri

| Para Birimi | Açılış Miktarı | Notlar |
|-------------|----------------|--------|
| 200 TL | 0 | Kabul et ama tutma |
| 100 TL | 5 (500 TL) | Birincil banknot |
| 50 TL | 10 (500 TL) | Para üstü verme |
| 20 TL | 10 (200 TL) | Para üstü verme |
| 10 TL | 10 (100 TL) | Para üstü verme |
| 5 TL | 20 (100 TL) | Para üstü verme |
| 1 TL | 50 (50 TL) | Bozukluk |
| Kuruşlar | 50 TL değerinde | Bozukluk |

### Yasaklı Uygulamalar

- ❌ Çekmeceyi açık ve gözetimsiz bırakma
- ❌ Çekmece erişim kodlarını paylaşma
- ❌ Görev sırasında kişisel işlemler
- ❌ Çekmeceden borç verme/borç alma
- ❌ Nakit avans
- ❌ İşlem olmadan para bozma
- ❌ Müşterilerin tutarları görebileceği yerde nakit sayma
- ❌ Nakit tutarlarını alenen tartışma


## Örnekler / Senaryolar

### Senaryo 1: Nakit Fark Araştırması

**Durum:** Kapanışta 20$ eksik

**Araştırma Adımları:**
```
1. YENİDEN SAYIM
   ├── Tüm nakiti çekmeceden çıkar
   ├── Para birimine göre say
   ├── Tanığın doğrulamasını sağla
   └── Hâlâ eksikse devam et

2. İŞLEMLERİ İNCELE
   ├── Detaylı Z-raporu yazdır
   ├── Tüm iptalleri incele
   ├── Tüm iadeleri incele
   ├── Eksik işlemleri kontrol et
   └── Yaygın hataları ara

3. FİZİKSEL KONTROL
   ├── Çekmece altını kontrol et
   ├── Kasa çevresini kontrol et
   ├── Çöpü dikkatle kontrol et
   └── Cepleri kontrol et (personel gönüllü)

4. BELGELE
   ├── Fark raporunu tamamla
   ├── Müdür incelemesi ve imzası
   ├── Bölge Müdürüne ilet
   └── Kayıtlar için dosyala

5. TAKİP
   ├── Mevcutsa CCTV incele
   ├── Gerekirse personelle görüş
   └── Önleyici tedbirler uygula
```

### Senaryo 2: Şüpheli Sahte Banknot

**Durum:** Müşteri şüpheli 100$ ile ödeme yapıyor

**Yanıt Protokolü:**
1. Gizlice dedektör kalemiyle test et
2. Güvenlik özelliklerini kontrol et
3. Şüpheliyse:
   - Banknotu iade etME
   - Kibarca alternatif ödeme iste
   - "Üzgünüm, sistemimiz bu banknotu kabul etmiyor"
   - Müdürle iletişime geç
   - Sahte raporunu tamamla
   - Gerekirse yetkililere bildir

### Senaryo 3: Müşteri Para Üstünü İtiraz Ediyor

**Durum:** Müşteri yanlış para üstü aldığını iddia ediyor

**Yanıt Protokolü:**
1. Sakin ve profesyonel kal
2. Karışıklık için özür dile
3. Müdürün tanık olmasını iste
4. Çekmeceyi saymayı teklif et (kapanış gerektirir)
5. Hata bulunursa hemen düzelt
6. Hata yoksa kibarca açıkla
7. Olayı belgele


## Sık Karşılaşılan Sorunlar ve Çözümler

### Sorun 1: Sık Küçük Farklar

**Problem:** Düzenli olarak 1-5$ farklar oluşuyor

**Çözümler:**
- İşlem yönetimini incele
- Para üstü sayma konusunda yeniden eğit
- Hesaplama hatalarını kontrol et
- POS'un doğru hesapladığını doğrula
- Yüksek sesle sayma uygula


### Sorun 3: Sahte Banknotlar Kabul Edilmiş

**Problem:** Yatırım sırasında sahte para bulundu

**Çözümler:**
- Tüm personeli tespit konusunda eğit
- Tespit kalemleri/cihazları sağla
- 20$ ve üzeri tüm banknotları doğrula
- Güvenlik özellikleri referansı as
- Anında raporlama protokolü


*Bu doküman Aydınlı Grup Mağaza Operasyonları Ekibi tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*




---

## SECTION 5: CUSTOMER-SERVICE-STANDARDS-TR

﻿---
layout: default
title: "Müşteri Hizmetleri Standartları – TR"
parent: Store Operations

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Süreç Adımları](#süreç-adımları)
6. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
7. [Kontrol Listeleri](#kontrol-listeleri)
8. [Örnekler / Senaryolar](#örnekler--senaryolar)
9. [KPI'lar](#kpilar)
10. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
11. [Revizyon Geçmişi](#revizyon-geçmişi)


## Kapsam

Bu standartlar şunlara uygulanır:
- Tüm müşteri etkileşimleri (mağaza içi, telefon, dijital)
- Tüm personel seviyeleri (satış danışmanlarından müdürlere)
- Tüm mağaza formatları ve lokasyonları
- Tüm markalar (U.S. Polo Assn., Pierre Cardin, Cacharel)


## Sorumluluklar

### Satış Danışmanları

| Sorumluluk | Standart |
|------------|----------|
| Her müşteriyi karşılama | 30 saniye içinde |
| Yardım teklifi | Samimi, baskıcı olmayan |
| Ürün bilgisi | Soruları güvenle yanıtlama |
| Kabin hizmeti | İlgili, yardımsever |
| Kasa deneyimi | Hızlı, doğru, samimi |
| Veda | Teşekkür et ve geri dönmeye davet et |

### Mağaza Müdürü

| Sorumluluk | Standart |
|------------|----------|
| Hizmet kültürü | Örnek olarak liderlik et |
| Eğitim sunumu | Sürekli koçluk |
| Müşteri eskalasyonları | Kişisel olarak ele al |
| Hizmet izleme | Gerçek zamanlı gözlem |
| Geri bildirim analizi | Haftalık inceleme |
| Takdir | Mükemmel hizmeti kabul et |


## Standartlar ve Kurallar

### Yanıt Süresi Standartları

| Etkileşim | Maksimum Süre | Hedef |
|-----------|---------------|-------|
| İlk karşılama | 30 saniye | 15 saniye |
| Yardım talebi | 60 saniye | 30 saniye |
| Kabin kontrolü | 5 dakika | 3 dakika |
| Kasa bekleme | 3 dakika | 2 dakika altı |
| Telefona cevap | 3 çalış | 2 çalış |
| Müşteri şikayeti | Hemen | Hemen |

### Hizmet Davranışları

#### Gerekli Davranışlar ✓

- ✓ Gülümse ve göz teması kur
- ✓ Biliniyorsa müşterinin adını kullan
- ✓ Birden fazla temas noktasında müşteriye teşekkür et
- ✓ Ürünleri taşımaya yardım teklif et
- ✓ Ürüne yürüyerek götür, işaret etme
- ✓ Kabine eşlik et
- ✓ Proaktif olarak alternatif beden teklif et
- ✓ Kabinde takip et
- ✓ Tam kombinler öner
- ✓ Kasada çanta teklif et

#### Yasaklı Davranışlar ✗

- ✗ Müşterileri görmezden gelme
- ✗ Müşteriler varken meslektaşlarla sohbet
- ✗ Satış alanında cep telefonu kullanma
- ✗ Satış alanında yeme içme
- ✗ Olumsuz beden dili (çapraz kollar, yaslanma)
- ✗ Müşterilerle tartışma
- ✗ Müşteriler hakkında varsayımda bulunma
- ✗ Müşterileri baskılama veya takip etme
- ✗ Kişisel konuları tartışma
- ✗ Rakipleri eleştirme

### İletişim Standartları

| Boyut | Standart |
|-------|----------|
| **Ton** | Sıcak, profesyonel, samimi |
| **Dil** | Açık, jargonsuz, saygılı |
| **Ses** | Ortama uygun |
| **Hız** | Orta, işlemeye izin veren |
| **Beden Dili** | Açık, ilgili, dikkatli |


## Örnekler / Senaryolar

### Senaryo 1: Yoğun Mağaza, Bekleyen Müşteri

**Durum:** Mağaza yoğun, müşteri yardım için bekliyor

**Yanıt:**
```
1. Hemen kabul et (göz teması + baş sallama)
2. Söyle: "Hemen sizinle ilgileneceğim!"
3. Mevcut etkileşimi verimli bitir
4. Bekleyen müşteriye hızlıca yaklaş
5. Bekleme için özür dile
6. Tam dikkatini ver
```

### Senaryo 2: Kararsız Müşteri

**Durum:** Müşteri iki ürün arasında karar veremiyor

**Yanıt:**
```
1. Tereddütü anla
   "Kararı zorlaştıran ne?"
   
2. Farkları vurgula
   "Bu farklı vesileler için daha çok yönlü,
    bu ise [belirli kullanım] için mükemmel."
    
3. Kullanım durumunu sor
   "Bunu en çok nerede giyeceğinizi düşünüyorsunuz?"
   
4. Güvence ver
   "İkisi de harika seçimler. İade politikamız
    gerekirse evde karar verme zamanı verir."
    
5. Hâlâ kararsızsa
   "İkisini de alıp uymayanı iade etmek ister misiniz?"
```

### Senaryo 3: Müşteri Şikayeti

**Durum:** Müşteri önceki satın alımından memnun değil

**Yanıt:**
```
1. DİNLE
   - Müşterinin tam açıklamasına izin ver
   - Sözünü kesme
   - Empati göster
   
2. KABUL ET
   "Hayal kırıklığınızı tamamen anlıyorum.
    Bu yaşamanızı istediğimiz deneyim değil."
    
3. ÖZÜR DİLE
   "Bunun olduğu için üzgünüm. Düzeltmeme izin verin."
   
4. ÇÖZ
   - Yetki dahilinde çözüm sun
   - Yetkiyi aşıyorsa müdürü dahil et
   - Hızlı hareket et
   
5. TEŞEKKÜR ET
   "Bunu düzeltme şansı verdiğiniz için teşekkürler.
    Geri bildiriminize gerçekten değer veriyoruz."
```


## Sık Karşılaşılan Sorunlar ve Çözümler

### Sorun 1: Müşteriler Yardımsız Ayrılıyor

**Problem:** Müşteriler personel ilgisi olmadan ayrılıyor

**Çözümler:**
- Saha kapsamını iyileştir
- Personeli kilit bölgelere konumlandır
- Satın alma sinyallerini okuma eğitimi ver
- Karşılama hesap verebilirliği uygula
- Kapı zili/sayaç farkındalığı kullan


### Sorun 3: Bekleme Süresi Şikayetleri

**Problem:** Müşteriler beklemekten rahatsız

**Çözümler:**
- Bekleyen müşterileri hemen kabul et
- Kuyruk yönetimi uygula
- Kasa yedeklemesi için çapraz eğitim ver
- Yoğun saatleri uygun şekilde planla
- Beklenen bekleme süresini ilet


*Bu doküman Aydınlı Grup Mağaza Operasyonları Ekibi tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*




---

## SECTION 6: INVENTORY-CONTROL-GUIDE-TR

﻿---
layout: default
title: "Envanter Kontrol Kılavuzu – TR"
parent: Store Operations

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Süreç Adımları](#süreç-adımları)
6. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
7. [Kontrol Listeleri](#kontrol-listeleri)
8. [Örnekler / Senaryolar](#örnekler--senaryolar)
9. [KPI'lar](#kpilar)
10. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
11. [Revizyon Geçmişi](#revizyon-geçmişi)


## Kapsam

Bu kılavuz şunları kapsar:
- Teslimat alma ve işleme
- Stok organizasyonu ve yenileme
- Döngüsel sayım prosedürleri
- Yıllık envanter sayımları
- Envanter düzeltme süreçleri
- Mağazalar arası transferler


## Sorumluluklar

### Satış Personeli

| Görev | Sıklık |
|-------|--------|
| Satış alanı stoğunu koruma | Sürekli |
| Düşük stok ürünlerini raporlama | Fark edildiğinde |
| Döngüsel sayımlara katılım | Atandığında |
| Hasarlı ürünleri raporlama | Hemen |

### Depo Personeli

| Görev | Sıklık |
|-------|--------|
| Teslimatları alma ve işleme | Teslimat başına |
| Arka stoğu organize etme | Günlük |
| Satış alanını yenileme | Günde birkaç kez |
| Döngüsel sayım yapma | Programa göre |

### Mağaza Müdürü

| Görev | Sıklık |
|-------|--------|
| Envanter doğruluğunu denetleme | Günlük |
| Envanter düzeltmelerini onaylama | Gerektiğinde |
| Teslimat programlarını yönetme | Haftalık |
| Yıllık envanteri yönetme | Yıllık |
| Farkları araştırma | Gerçekleştiğinde |


## Standartlar ve Kurallar

### Envanter Doğruluk Standartları

| Metrik | Hedef | Aksiyon Eşiği |
|--------|-------|---------------|
| SKU Doğruluğu | ≥%98 | <%95 araştırma tetikler |
| Adet Doğruluğu | ≥%99 | <%97 araştırma tetikler |
| Fire Oranı | Satışların <%1'i | >%1.5 aksiyon planı tetikler |
| Mal Kabul Doğruluğu | %100 | Her hata rapor gerektirir |

### Depo Organizasyonu

```
DEPO ALAN DÜZENİ

BÖLGE A: MAL KABUL
├── Gelen sevkiyatlar
├── İşleme alanı
└── Güvenlik etiketleme istasyonu

BÖLGE B: AKTİF STOK
├── Güncel sezon ürünleri
├── Yüksek hareket hızlı ürünler
└── Kategori/stile göre organize

BÖLGE C: YEDEK STOK
├── Taşan envanter
├── Gelecek sezon (varsa)
└── Toplu depolama

BÖLGE D: İADELER/HASARLAR
├── İşlenecek ürünler
├── Hasarlı ürünler
└── Transfer hazırlık alanı
```

### FIFO Prensipleri

| Prensip | Uygulama |
|---------|----------|
| **İlk Giren, İlk Çıkar** | Eski stok önce satılır |
| **Yeni Teslimatlar** | Mevcut stoğun arkasına yerleştir |
| **Saha Stoğu** | Arka stokla rotasyon yap |
| **Tarihli Ürünler** | Tarihleri kontrol et, en eskiyi önce sat |


## Örnekler / Senaryolar

### Senaryo 1: Eksik Teslimat

**Durum:** 100 adet teslimat, paket fişi 120 gösteriyor

**Süreç:**
```
1. Eksikliği hemen belgele
2. Teslimat makbuzuna not: "100 alındı, 120 bekleniyordu"
3. Sürücüye değiştirilmiş makbuzu imzalat
4. Tutarsızlık raporunu tamamla
5. Sistemde sadece 100 kabul et
6. Detaylarla HQ'ya bildir
7. Tüm evrakları dosyala
8. Çözüm için takip et
```

### Senaryo 2: Döngüsel Sayımda Fark Bulundu

**Durum:** Sistem 15 gösteriyor, gerçek sayım 12

**Süreç:**
```
1. Doğrulamak için yeniden say
2. Kabinleri kontrol et
3. Fikstür altlarını kontrol et
4. Arka stoğu kontrol et
5. Son satışları incele
6. Yanlış taramaları kontrol et
7. Fark onaylanırsa:
   - Bulguları belgele
   - Müdür incelemesi
   - Onaylanırsa envanteri düzelt
   - Neden kodu not et
8. Desen bulunursa daha fazla araştır
```

### Senaryo 3: Hasarlı Ürün Teslim Alındı

**Durum:** Teslimattta 5 ürün kusurlu

**Süreç:**
```
1. Hasarlı ürünleri ayır
2. Fotoğraflarla belgele
3. Mal kabul evraklarına not et
4. Envantere KOYMA
5. Hasarlı ürün formunu tamamla
6. HQ'ya gönder
7. Politikaya göre ürünleri tut:
   - Satıcıya iade, VEYA
   - İndirimle sat, VEYA
   - Dokümantasyonla imha et
8. Envanteri buna göre güncelle
```


## Sık Karşılaşılan Sorunlar ve Çözümler

### Sorun 1: Kronik Stoksuzluk

**Problem:** Aynı ürünler sürekli stoksuz kalıyor

**Çözümler:**
- Yeniden sipariş noktalarını incele
- Satış hızını analiz et
- Sipariş miktarlarını artır
- Öncelikli tahsis talep et
- Mağaza transferi düşün


### Sorun 3: Düzensiz Arka Stok

**Problem:** Gerektiğinde ürünler bulunamıyor

**Çözümler:**
- Bölge sistemi uygula
- Günlük organizasyon zamanı
- Net etiketleme
- Eski ürünlerin düzenli temizlenmesi
- Personel hesap verebilirliği


*Bu doküman Aydınlı Grup Mağaza Operasyonları Ekibi tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*




---

## SECTION 7: LOSS-PREVENTION-GUIDE-TR

﻿---
layout: default
title: "Kayıp Önleme Kılavuzu – TR"
parent: Store Operations

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Süreç Adımları](#süreç-adımları)
6. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
7. [Kontrol Listeleri](#kontrol-listeleri)
8. [Örnekler / Senaryolar](#örnekler--senaryolar)
9. [KPI'lar](#kpilar)
10. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
11. [Revizyon Geçmişi](#revizyon-geçmişi)


## Kapsam

Bu kılavuz şunları kapsar:
- Dış hırsızlık (mağaza hırsızlığı)
- İç hırsızlık (çalışan)
- İdari hatalar
- Satıcı dolandırıcılığı
- İade dolandırıcılığı
- Fiziksel güvenlik önlemleri


## Sorumluluklar

### Tüm Personel

| Sorumluluk | Standart |
|------------|----------|
| Müşteri kabulü | Tüm müşterileri karşıla |
| Saha varlığı | Görünür ve dikkatli |
| Şüpheli davranış | Hemen raporla |
| Güvenlik prosedürleri | Tutarlı uygula |

### Mağaza Müdürü

| Sorumluluk | Standart |
|------------|----------|
| LP eğitimi | Tüm personel eğitimli |
| Güvenlik ekipmanı | Her zaman çalışır durumda |
| Olay yönetimi | Politikaya göre |
| Fire takibi | Aylık inceleme |
| Soruşturmalar | İç soruşturmaları yönet |


## Standartlar ve Kurallar

### Güvenlik Ekipmanı Standartları

| Ekipman | Gereksinim | Kontrol Sıklığı |
|---------|------------|-----------------|
| EAS Kapıları | Çalışır durumda | Günlük |
| CCTV Sistemi | Kayıtta | Günlük |
| Güvenlik Etiketleri | Yeterli stok | Haftalık |
| Aynalar | Temiz, konumlandırılmış | Günlük |
| Kilitler | İşlevsel | Günlük |
| Kasa | Güvenli | Her kullanımda |

### Kabin Prosedürleri

| Adım | Gereksinim |
|------|------------|
| **Ürün Sayımı** | Giren tüm ürünleri say |
| **Limit** | Maksimum 6 ürün (ayarlanabilir) |
| **Numara Etiketi** | Müşteriye numaralı etiket ver |
| **Kontrol** | Kullanım sırasında periyodik kontroller |
| **Çıkış Sayımı** | Çıkan tüm ürünleri say |
| **Tutarsızlık** | Hemen raporla |

### Yüksek Riskli Ürün Koruması

| Risk Seviyesi | Ürünler | Koruma |
|---------------|---------|--------|
| **Yüksek** | Deri, kaşmir, premium | Birden fazla etiket, kilitli vitrin |
| **Orta** | Dış giyim, aksesuarlar | Tek etiket, görünür yerleşim |
| **Standart** | Temel giyim | Tek etiket |


## Örnekler / Senaryolar

### Senaryo 1: Gözlemlenen Hırsızlık

**Durum:** Personel müşterinin ürünü çantasına sakladığını görüyor

**Müdahale:**
```
YAP:
✓ Sakin kal
✓ Görsel teması koru
✓ İş arkadaşına/müdüre işaret ver
✓ Müşteri hizmeti yaklaşımına devam et
✓ Tarif detaylarını not et
✓ Müdürün halletmesine izin ver

YAPMA:
✗ Müşteriyi suçlama
✗ Fiziksel yüzleşme
✗ Çıkışı engelleme
✗ Mağaza dışına kovalama
✗ Kendini riske atma

MÜŞTERİ ÇIKARSA:
1. Dışarı takip ETME
2. Gidiş yönünü not et
3. Hemen belgele
4. CCTV görüntüsünü kaydet
5. Yüksek değerse polisi ara
```

### Senaryo 2: Kabin Tutarsızlığı

**Durum:** Müşteri 4 ürünle girdi, 2 ile çıkıyor

**Müdahale:**
```
1. "Affedersiniz, kabinde bazı ürünler bırakmış 
    olabilirsiniz. Sizin için kontrol edeyim."

2. Kabini birlikte kontrol et

3. Ürünler bulunursa:
   - Ürünleri müşteriye ver
   - "İşte denediğiniz diğer ürünler."

4. Ürünler BULUNAMAZSA:
   - "Özür dilerim, yanlış saymış olmalıyım."
   - Suçlama YAPMA
   - Müşterinin gitmesine izin ver
   - CCTV'yi hemen incele
   - Olayı belgele
```

### Senaryo 3: Şüpheli Çalışan Hırsızlığı

**Durum:** Nakit eksiklikleri veya envanter farkları

**Müdahale:**
```
ÖNEMLİ: Gizli olarak ele al

1. Tüm gerçekleri belgele
2. CCTV görüntülerini incele
3. Desenleri analiz et (tarihler, vardiyalar)
4. Bölge Müdürüyle iletişime geç
5. Çalışanla yüzleşME
6. İK/LP ekibinin soruşturmasına izin ver
7. Normal operasyonları sürdür
8. Delilleri koru
```


## Sık Karşılaşılan Sorunlar ve Çözümler

### Sorun 1: Yüksek Kabin Hırsızlığı

**Problem:** Kabinlerden ürünler kayboluyor

**Çözümler:**
- Daha sıkı ürün sayımı
- Daha düşük ürün limitleri
- Daha sık oda kontrolü
- Boş askıları kaldır
- Görevli pozisyonu düşün


### Sorun 3: Yüksek İç Hırsızlık

**Problem:** Çalışan hırsızlığı şüphesi

**Çözümler:**
- Nakit yönetimi prosedürlerini güçlendir
- Çanta kontrolleri uygula
- CCTV'yi düzenli incele
- Yeni işe alımlarda özgeçmiş kontrolü
- Anonim ihbar hattı


*Bu doküman Aydınlı Grup Mağaza Operasyonları Ekibi tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*




---

## SECTION 8: RETURN-EXCHANGE-PROCEDURES-TR

﻿---
layout: default
title: "İade ve Değişim Prosedürleri – TR"
parent: Store Operations

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Süreç Adımları](#süreç-adımları)
6. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
7. [Kontrol Listeleri](#kontrol-listeleri)
8. [Örnekler / Senaryolar](#örnekler--senaryolar)
9. [KPI'lar](#kpilar)
10. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
11. [Revizyon Geçmişi](#revizyon-geçmişi)


## Kapsam

Bu prosedür şunları kapsar:
- Standart iadeler (para iadesi)
- Değişimler (aynı ürün, farklı beden/renk)
- Kusurlu ürün iadeleri
- Hediye iadeleri (fişsiz)
- Online alışveriş mağazada iade


## Sorumluluklar

### Satış Personeli

| Görev | Standart |
|-------|----------|
| İade müşterilerini sıcak karşılama | Tüm müşterilerle aynı |
| İade uygunluğunu doğrulama | Politikaya göre |
| İade ürünlerini inceleme | Detaylı kontrol |
| İşlemi gerçekleştirme | Doğru, verimli |
| Gerektiğinde sorumluyu dahil etme | Karmaşık durumlar |

### Sorumlu/Müdür

| Görev | Standart |
|-------|----------|
| İstisnaları yetkilendirme | Yetki seviyesine göre |
| Eskalasyonları ele alma | Müşteri memnuniyeti odağı |
| Yüksek tutarlı iadeleri onaylama | Eşiğe göre |
| Şüpheli iadeleri araştırma | Dokümantasyon gerekli |


## Standartlar ve Kurallar

### Uygunluk Kriterleri

| Kriter | Uygun | Uygun Değil |
|--------|-------|-------------|
| **Zaman Çerçevesi** | 30 gün içinde | 30 gün sonra |
| **Etiketler** | Takılı | Çıkarılmış |
| **Durum** | Giyilmemiş, temiz | Giyilmiş, kirli, hasarlı |
| **Fiş** | Mevcut | Eksik (sadece para iadesi) |
| **Kategori** | Normal ürünler | İç giyim, mayo, küpe |
| **İndirimli Ürünler** | Politikaya göre | Son satış ürünleri |
| **Tadilat** | Yok | Paça kısaltılmış, değiştirilmiş |

### İade Yöntemleri

| Orijinal Ödeme | İade Yöntemi | Süre |
|----------------|--------------|------|
| Nakit | Nakit | Anında |
| Kredi Kartı | Aynı kart | 3-5 iş günü |
| Banka Kartı | Aynı kart | 3-5 iş günü |
| Hediye Kartı | Hediye kartı | Anında |
| Mağaza Kredisi | Mağaza kredisi | Anında |

### İstisna Yönetimi

| Senaryo | Yetki | Aksiyon |
|---------|-------|---------|
| Süreden 1-7 gün sonra | Sorumlu | Onaylayabilir |
| Süreden 8-14 gün sonra | Müdür | Onaylayabilir |
| Süreden 15+ gün sonra | Bölge Müdürü | Onay gerekli |
| Fişsiz, yüksek değer | Müdür | Satın alma geçmişini doğrula |
| Şüpheli dolandırıcılık | Müdür | Reddet, belgele |


## Örnekler / Senaryolar

### Senaryo 1: Fişli Standart İade

**Durum:** Müşteri geçen hafta aldığı gömleği iade ediyor

**Süreç:**
```
1. Sıcak karşıla, nedenini sor
2. Fiş iste
3. Satın alma tarihini doğrula (30 gün içinde) ✓
4. Ürünü incele:
   - Etiketler takılı ✓
   - Giyilmemiş ✓
   - Hasar yok ✓
5. Orijinal karta iade işle
6. İade onayı ver
7. Müşteriye teşekkür et ve geri dönmeye davet et
```

### Senaryo 2: Fişsiz İade

**Durum:** Müşteri hediyeyi iade etmek istiyor, fiş yok

**Süreç:**
```
1. Sıcak karşıla, durumu kabul et
2. Politikayı açıkla: "Fiş olmadan, güncel satış 
   fiyatı üzerinden değişim veya mağaza kredisi sunabiliriz"
3. Müşteri kabul ederse:
   - Sistemde ürünü ara
   - Güncel fiyat için mağaza kredisi ver
   - Mağaza kredisi iadesi olarak işle
4. Müşteri kabul etmezse:
   - Hediyeyi verenin fiş sağlayıp sağlayamayacağını kontrol etmeyi teklif et
   - Politikada samimi ama kararlı kal
```

### Senaryo 3: İade Süresi Geçmiş

**Durum:** Müşteri satın almadan 35 gün sonra ürün iade ediyor

**Süreç:**
```
1. Sıcak karşıla, fişi kontrol et
2. Satın alma tarihinin 30 günü geçtiğini not et
3. Açıkla: "Politikamız 30 gündür, ama ne 
   yapabileceğime bakayım"
4. Ürün durumunu kontrol et (iyi)
5. 1-7 gün geçmişse:
   - Sorumlu onaylayabilir
6. Uzlaşma olarak değişim veya mağaza kredisi teklif et
7. Onaylanırsa istisnayı belgele
```

### Senaryo 4: Şüpheli Dolandırıcı İade

**Kırmızı Bayraklar:**
- Fiş yok, yüksek değerli ürün
- Kullanım/giyim belirtileri
- Aynı kişiden birden fazla iade
- Ürün çok eski görünüyor
- Fiş değiştirilmiş görünüyor

**Süreç:**
```
1. Sakin ve profesyonel kal
2. Detaylı inceleme yap
3. Şüpheliyse, gizlice müdürü çağır
4. Müdür konuşmayı yönetir
5. İadeyi kibarca reddedebilir:
   "Maalesef, bu ürün kullanım belirtileri gösterdiği 
    için iade kriterlerimizi karşılamıyor"
6. Olayı belgele
7. Mümkünse müşteri detaylarını not et
```


## Sık Karşılaşılan Sorunlar ve Çözümler

### Sorun 1: Müşteri Politika Dışı İadede Israr Ediyor

**Çözüm:**
- Sakin ve empatik kal
- Politikayı açıkça açıkla
- Alternatifler sun (değişim, mağaza kredisi)
- Müşteri eskalasyon yaparsa müdürü dahil et
- Müdür gerekçeliyse istisnayı onaylayabilir
- İstisnaları belgele


### Sorun 3: Şüpheli İade Dolandırıcılığı Deseni

**Çözüm:**
- Müşteriye göre iade desenlerini izle
- Sık iade yapanları işaretle
- Fişsiz iadeler için kimlik iste
- Şüpheli iadeler için müdür incelemesi
- Desenleri Kayıp Önleme'ye raporla


*Bu doküman Aydınlı Grup Mağaza Operasyonları Ekibi tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*




---

## SECTION 9: ORDER-MANAGEMENT-GUIDE-TR

﻿---
layout: default
title: "Sipariş Yönetimi Kılavuzu – TR"
parent: Supply Chain

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Süreç Adımları](#süreç-adımları)
6. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
7. [Kontrol Listeleri](#kontrol-listeleri)
8. [Örnekler / Senaryolar](#örnekler--senaryolar)
9. [KPI'lar](#kpilar)
10. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
11. [Revizyon Geçmişi](#revizyon-geçmişi)


## Kapsam

### Sipariş Türleri

| Sipariş Türü | Açıklama | Zamanlama |
|--------------|----------|-----------|
| **Sezon Siparişi** | Sezon öncesi toplu sipariş | 4-6 ay önceden |
| **Yenileme Siparişi** | Düzenli stok tazeleme | Haftalık/İki haftalık |
| **Özel Sipariş** | Müşteriye özel talepler | Gerektiğinde |
| **Transfer Siparişi** | Mağazalar arası hareketler | Gerektiğinde |
| **Acil Sipariş** | Acil stok ihtiyaçları | Aynı gün |

### Sistem Erişimi

| Sistem | Amaç | Erişim |
|--------|------|--------|
| **B2B Portalı** | Sipariş gönderimi | Tüm mağazalar |
| **ERP Sistemi** | Envanter takibi | Müdür seviyesi |
| **WMS** | Depo durumu | Sadece görüntüleme |
| **Takip Portalı** | Sevkiyat takibi | Tüm mağazalar |


## Sorumluluklar

### Mağaza Personeli

| Sorumluluk | Standart |
|------------|----------|
| Stok seviyelerini izle | Günlük |
| Düşük stok ürünlerini raporla | Hemen |
| Sipariş önerileri hazırla | Haftalık |
| Alınan siparişleri doğrula | Aynı gün |
| Envanter sistemini güncelle | Gerçek zamanlı |

### Mağaza Müdürü

| Sorumluluk | Standart |
|------------|----------|
| Sipariş gönderimlerini onayla | Son tarihten önce |
| Sipariş bütçesini yönet | Tahsis dahilinde |
| Sipariş geçmişini incele | Haftalık |
| Bölge ile koordine ol | Gerektiğinde |
| Sipariş doğruluğunu sağla | Gönderim başına |

### Bölge Müdürü

| Sorumluluk | Standart |
|------------|----------|
| Bölgesel siparişleri denetle | Haftalık inceleme |
| Bölgesel envanteri dengele | Aylık |
| Özel siparişleri onayla | 48 saat içinde |
| Transferleri koordine et | Gerektiğinde |
| Bölgesel performansı izle | Aylık |

### Merkez Tedarik Zinciri

| Sorumluluk | Standart |
|------------|----------|
| Siparişleri işle | 24 saat dönüş |
| Tahsisleri yönet | Politikaya göre |
| Tedarikçilerle koordine ol | Sürekli |
| Sistem bakımı | Sürekli |
| Performans raporlaması | Aylık |


## Standartlar ve Kurallar

### Sipariş Gönderim Standartları

| Standart | Gereksinim |
|----------|------------|
| **Sipariş Son Tarihi** | Haftalık siparişler için Salı 18:00 |
| **Minimum Sipariş** | Marka/kategori MOQ'ya göre |
| **Sipariş Sıklığı** | Haftalık (standart) |
| **Sistem Erişimi** | Sadece yetkili kullanıcılar |
| **Dokümantasyon** | Tüm siparişler belgelenir |

### Sipariş Doğruluk Gereksinimleri

| Metrik | Hedef |
|--------|-------|
| Ürün Seçimi | %100 doğru SKU |
| Miktar Doğruluğu | İhtiyacın %5'i dahilinde |
| Beden Oranı | Marka kılavuzlarına göre |
| Adres Doğruluğu | %100 doğru |
| Özel Talimatlar | Tam ve net |

### Bütçe Yönetimi

| Kural | Açıklama |
|-------|----------|
| **Satın Alma Limiti** | Tahsisi aşma |
| **Onay Gerekli** | Bütçeyi aşan siparişler için |
| **Öncelikli Ürünler** | Hızlı satanlara odaklan |
| **Sezonluk Düzeltme** | Yoğun dönemler için ayarla |
| **İndirim Dengesi** | İndirimli ürünleri dahil et |


## Örnekler / Senaryolar

### Senaryo 1: Düzenli Haftalık Sipariş

**Durum:** Salı haftalık sipariş gönderimi

**Süreç:**
```
1. Pazartesi: Envanter raporu çalıştır
2. Pazartesi: Satış hızını analiz et
3. Pazartesi: Yenileme ihtiyaçlarını belirle
4. Pazartesi: Sipariş listesini hazırla
5. Salı Sabah: Müdür incelemesi ve onayı
6. Salı Öğleden Sonra: 18:00'dan önce sipariş gönder
7. Salı: Onayı kaydet
8. Çarşamba: Sipariş onayının alındığını doğrula
```

### Senaryo 2: Acil Stok Talebi

**Durum:** Kilit ürün beklenmedik şekilde tükendi

**Süreç:**
```
1. Gerçek stoğu doğrula (fiziksel sayım)
2. Bölgesel envanteri kontrol et (diğer mağazalar)
3. Mevcutsa: Mağaza transferi talep et
4. Mevcut değilse: Bölge Müdürüyle iletişime geç
5. Acil sipariş talebi gönder
6. Gerekçe sun (satış verileri)
7. Onay durumunu takip et
8. Hızlandırılmış teslimatı koordine et
```

### Senaryo 3: Sezon Sipariş Planlaması

**Durum:** Sezon öncesi toplu sipariş

**Süreç:**
```
1. Önceki sezon performansını incele
2. Kategoriye göre satış oranlarını analiz et
3. En çok satan stilleri/renkleri belirle
4. HQ önerilerini incele
5. Yerel pazar tercihlerini değerlendir
6. Beden oranlarını hesapla
7. Bütçe dahilinde sipariş hazırla
8. Son tarihe kadar gönder
9. HQ tahsisini incele
10. Gerekirse düzeltme talep et
```


## Sık Karşılaşılan Sorunlar ve Çözümler

### Sorun 1: Kaçırılan Sipariş Son Tarihi

**Problem:** Sipariş son tarihe kadar gönderilmedi

**Çözümler:**
- Takvim hatırlatıcıları ayarla
- Siparişleri önceden hazırla
- Yedek yetki ata
- Kaçırıldıysa hemen HQ'ya ulaş
- Son tarih uzatması talep et (sadece bir kez)


### Sorun 3: Sistem Erişim Sorunları

**Problem:** B2B portalına erişilemiyor

**Çözümler:**
- İnternet bağlantısını kontrol et
- Tarayıcı önbelleğini temizle
- Farklı tarayıcı dene
- Gerekirse şifre sıfırla
- Hemen IT desteğine ulaş
- Yedek kullanıcının sipariş göndermesini sağla


## B2B Portal Kılavuzu

### Giriş Süreci

```
B2B PORTAL ERİŞİMİ

1. portal.aydinli.com adresine git
2. Kullanıcı adını gir (mağaza e-postası)
3. Şifreyi gir
4. İki faktörlü doğrulamayı tamamla
5. Sipariş panosuna eriş

NAVİGASYON
- Pano → Sipariş özeti
- Yeni Sipariş → Yenileme siparişi oluştur
- Sipariş Geçmişi → Geçmiş siparişleri görüntüle
- Takip → Sevkiyatları takip et
- Raporlar → Envanter raporlarını indir
- Destek → Yardım masasına ulaş
```

### Sipariş Giriş Adımları

```
YENİ SİPARİŞ OLUŞTURMA

1. "Yeni Sipariş"e tıkla
2. Marka seç (USPA/PC/Cacharel)
3. Kategori seç (Erkek/Kadın/Çocuk/Aksesuar)
4. Ürünlere göz at veya ara
5. Beden başına miktar gir
6. Sepete ekle
7. Tüm ürünler için tekrarla
8. Sepeti incele
9. Teslimat adresini doğrula
10. Özel talimatlar ekle
11. Siparişi gönder
12. Onayı kaydet
```


*Bu doküman Aydınlı Grup Tedarik Zinciri Ekibi tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*




---

## SECTION 10: DELIVERY-RECEIVING-PROCEDURES-TR

﻿---
layout: default
title: "Teslimat ve Kabul Prosedürleri – TR"
parent: Supply Chain

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Süreç Adımları](#süreç-adımları)
6. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
7. [Kontrol Listeleri](#kontrol-listeleri)
8. [Örnekler / Senaryolar](#örnekler--senaryolar)
9. [KPI'lar](#kpilar)
10. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
11. [Revizyon Geçmişi](#revizyon-geçmişi)


## Kapsam

### Teslimat Türleri

| Teslimat Türü | Kaynak | Sıklık |
|---------------|--------|--------|
| **Düzenli Yenileme** | Ana DC | Haftalık |
| **Sezon Başlangıç** | Ana DC | Sezon öncesi |
| **Transfer** | Diğer mağazalar | Gerektiğinde |
| **İade/Onarım** | Onarım merkezi | Uygulanabildiğinde |
| **Özel/Acil** | DC veya tedarikçi | Acil durum |

### Kabul Alanları

| Alan | Amaç | Gereksinimler |
|------|------|---------------|
| **Arka Oda** | Birincil kabul | Güvenli, düzenli |
| **Hazırlık Alanı** | Geçici bekletme | Temiz alan |
| **İşleme Alanı** | Etiketleme/Sıralama | Çalışma istasyonu |
| **Depolama** | Envanter tutma | Uygun raflama |


## Sorumluluklar

### Kabul Personeli

| Sorumluluk | Standart |
|------------|----------|
| Kabul alanını hazırla | Teslimat öncesi |
| Teslimat belgelerini doğrula | Varışta |
| Tüm ürünleri incele | Kabul sırasında |
| Tutarsızlıkları belgele | Hemen |
| Envanter sistemini güncelle | Aynı gün |

### Mağaza Müdürü

| Sorumluluk | Standart |
|------------|----------|
| Kabul pencerelerini planla | Önceden |
| Yeterli personel ata | Teslimat boyutuna göre |
| Tutarsızlık raporlarını incele | Aynı gün |
| Envanter güncellemelerini onayla | Aynı gün |
| Sorunları takip et | 24 saat içinde |

### Bölge Müdürü

| Sorumluluk | Standart |
|------------|----------|
| Kabul KPI'larını izle | Haftalık |
| Tekrarlayan sorunları ele al | Gerektiğinde |
| Lojistikle koordine ol | Gerektiğinde |
| Talep gönderimlerini onayla | 48 saat içinde |


## Standartlar ve Kurallar

### Kabul Standartları

| Standart | Gereksinim |
|----------|------------|
| **Kabul Penceresi** | Teslimat sonrası 24 saat içinde |
| **Doğrulama** | Ürünlerin %100'ü kontrol edilir |
| **Dokümantasyon** | Her teslimat için tam |
| **Sistem Güncelleme** | Kabul ile aynı gün |
| **Tutarsızlık Son Tarihi** | 24 saat içinde raporla |

### Kalite Standartları

| Husus | Standart |
|-------|----------|
| **Ürün Durumu** | Görünür kusur yok |
| **Ambalaj** | Sağlam ve koruyucu |
| **Etiketleme** | Tüm etiketler mevcut |
| **Barkodlar** | Taranabilir |
| **Askılar** | Uygun ve hasarsız |

### Güvenlik Standartları

| Kural | Açıklama |
|-------|----------|
| **Denetimli Kabul** | Her zaman 2+ personel |
| **Güvenli Alan** | Kabul sadece arka odada |
| **Müşteri Erişimi Yok** | Kabul sırasında |
| **Hemen İşleme** | Gece bırakılan ürün yok |
| **Dokümantasyon** | İmza gerekli |


## Örnekler / Senaryolar

### Senaryo 1: Normal Teslimat

**Durum:** Düzenli haftalık yenileme geldi

**Süreç:**
```
1. Taşıyıcı ve mühür numarasını doğrula
2. Kutuları say (12 beklenen, 12 alınan)
3. BOL'u imzala
4. Tüm ürünleri aç ve tara
5. Tüm ürünler paket listesiyle eşleşiyor
6. Envanter sistemini güncelle
7. Ürünleri satış alanına işle
8. Dokümantasyonu dosyala
```

**Sonuç:** Temiz kabul, tutarsızlık yok


### Senaryo 3: Dış Hasarlı Teslimat

**Durum:** Kutu görünür şekilde hasarlı geldi

**Süreç:**
```
1. İmzalamadan önce hasarlı kutuyu fotoğrafla
2. BOL'a "HASARLI ALINDI" not et
3. Taşıyıcının kabul etmesini sağla
4. Hasarlı kutuyu dikkatli aç
5. İçindeki tüm ürünleri incele
6. Ürün hasarlarını belgele
7. Hasarlı ürünleri ayır
8. Hasar talep formunu tamamla
9. Talebi fotoğraflarla gönder
10. Hasarlı ürünleri inceleme için tut
```

**Sonuç:** Hasar belgelendi, taşıyıcı bilgilendirildi


## KPI'lar

### Kabul Performans Metrikleri

| KPI | Formül | Hedef | Sıklık |
|-----|--------|-------|--------|
| **Kabul Doğruluğu** | Doğru kabuller / Toplam | %100 | Teslimat başına |
| **Zamanında Kabul** | 24 saat içinde / Toplam | %100 | Haftalık |
| **Tutarsızlık Oranı** | Sorunlu teslimatlar / Toplam | <%2 | Aylık |
| **İşleme Süresi** | Teslimatdan satış alanına saat | <4 saat | Teslimat başına |
| **Talep Gönderimi** | Zamanında talepler / Toplam talep | %100 | Aylık |

### Performans Panosu

```
┌─────────────────────────────────────────────────────────────┐
│                 KABUL PERFORMANS PANOSU                     │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Kabul Doğruluğu       [████████████████████████] %100     │
│  Hedef: %100           ✓ KARŞILIYOR                         │
│                                                             │
│  Zamanında Kabul       [████████████████████████] %100     │
│  Hedef: %100           ✓ KARŞILIYOR                         │
│                                                             │
│  Tutarsızlık Oranı     [█░░░░░░░░░░░░░░░░░░░░░░] %1.5      │
│  Hedef: <%2            ✓ HEDEF İÇİNDE                       │
│                                                             │
│  Ort İşleme Süresi     [████████████████░░░░░░░░] 3.2 saat │
│  Hedef: <4 saat        ✓ KARŞILIYOR                         │
│                                                             │
│  Bu Ayın Teslimatları:                                      │
│  - Toplam teslimat: 8                                       │
│  - Temiz teslimatlar: 7                                     │
│  - Tutarsızlıklı: 1                                         │
│  - Dosyalanan talepler: 1                                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```


### Sorun 2: Tarayıcı Arızası

**Problem:** Kabul sırasında ürünler taranamıyor

**Çözümler:**
- Yedek tarayıcı bulundur
- Alternatif olarak manuel sayım
- Manuel belgele ve sonra güncelle
- IT desteğine ulaş
- Kabul tamamlanmasını asla geciktirme


### Sorun 4: Depolama Kısıtlamaları

**Problem:** Teslimat için yeterli alan yok

**Çözümler:**
- Teslimat öncesi alan temizle
- Ürünleri hemen satış alanına işle
- Daha küçük, daha sık teslimat talep et
- Arka oda organizasyonunu optimize et
- Kapasite limitlerini HQ'ya ilet


## Revizyon Geçmişi

| Versiyon | Tarih | Değişiklik | Hazırlayan |
|----------|-------|------------|------------|
| 1.0 | 2025-12-01 | İlk versiyon oluşturuldu | Aydınlı Grup Tedarik Zinciri |


---

## SECTION 11: STOCK-TRANSFER-PROCEDURES-TR

﻿---
layout: default
title: "Stok Transfer Prosedürleri – TR"
parent: Supply Chain

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Süreç Adımları](#süreç-adımları)
6. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
7. [Kontrol Listeleri](#kontrol-listeleri)
8. [Örnekler / Senaryolar](#örnekler--senaryolar)
9. [KPI'lar](#kpilar)
10. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
11. [Revizyon Geçmişi](#revizyon-geçmişi)


## Kapsam

### Transfer Türleri

| Transfer Türü | Açıklama | Onay |
|---------------|----------|------|
| **Mağazadan Mağazaya** | Mağazalar arası doğrudan transfer | Bölge Müdürü |
| **Mağazadan DC'ye** | Dağıtım merkezine iade | HQ Onayı |
| **DC'den Mağazaya** | Acil yenileme | HQ Başlatır |
| **Bölgesel Dengeleme** | Çoklu mağaza ayarlamaları | Bölge Müdürü |
| **Sezon Sonu** | Sezon sonu konsolidasyonu | HQ Onayı |

### Transfer Nedenleri

| Neden | Öncelik | Tipik Hacim |
|-------|---------|-------------|
| **Stoksuzluk Önleme** | Yüksek | 10-50 adet |
| **Beden/Renk Dengeleme** | Orta | 20-100 adet |
| **Yavaş Satan Taşıma** | Orta | 50-200 adet |
| **Mağaza Açılış Desteği** | Yüksek | Değişken |
| **Mağaza Kapanış Transferi** | Yüksek | Tüm envanter |
| **Etkinlik/Promosyon Desteği** | Yüksek | Gerektiği kadar |


## Sorumluluklar

### Gönderen Mağaza

| Sorumluluk | Standart |
|------------|----------|
| Transfer talebini hazırla | Prosedüre göre |
| Ürünleri topla ve paketle | 24 saat içinde |
| Transfer dokümantasyonunu oluştur | Sevkiyat öncesi |
| Envanter sistemini güncelle | Sevkiyatta |
| Toplama/teslimatı koordine et | Programa göre |

### Alan Mağaza

| Sorumluluk | Standart |
|------------|----------|
| Transfer ihtiyacını onayla | Talepte |
| Kabul alanını hazırla | Varış öncesi |
| Alınan ürünleri doğrula | Aynı gün |
| Envanter sistemini güncelle | Aynı gün |
| Tutarsızlıkları raporla | 24 saat içinde |

### Bölge Müdürü

| Sorumluluk | Standart |
|------------|----------|
| Transferleri yetkilendir | 48 saat içinde |
| Bölgesel envanteri dengele | Haftalık inceleme |
| Transfer anlaşmazlıklarını çöz | 48 saat içinde |
| Transfer KPI'larını izle | Aylık |


## Standartlar ve Kurallar

### Transfer Standartları

| Standart | Gereksinim |
|----------|------------|
| **Minimum Transfer** | 10 adet (mağazadan mağazaya) |
| **Talep Son Tarihi** | İhtiyaçtan 48 saat önce |
| **Uygulama Süresi** | Onaydan sonra 48 saat içinde |
| **Dokümantasyon** | Tüm transferler için tam |
| **Durum** | Sadece satılabilir ürünler |

### Paketleme Standartları

| Standart | Gereksinim |
|----------|------------|
| **Koruma** | Ürünler hasardan korunur |
| **Organizasyon** | Ürünler SKU'ya göre düzenlenir |
| **Etiketleme** | Tüm kutular net etiketlenir |
| **Mühür** | Kutular mühürlü ve güvenli |
| **Paket Listesi** | Kutunun içinde ve dışında |

### Dokümantasyon Gereksinimleri

| Belge | Amaç | Saklama |
|-------|------|---------|
| Transfer Talebi | Süreci başlat | 1 yıl |
| Onay Kaydı | Yetkilendirme | 1 yıl |
| Paket Listesi | İçerik doğrulama | 1 yıl |
| Sevkiyat Belgesi | Transit kaydı | 1 yıl |
| Kabul Onayı | Tamamlanma kaydı | 1 yıl |


## Örnekler / Senaryolar

### Senaryo 1: Acil Stoksuzluk Önleme

**Durum:** Mağaza A en çok satanda azalıyor, Mağaza B'de fazlalık var

**Süreç:**
```
Gün 1 - Sabah:
1. Mağaza A stoksuzluk riskini belirler
2. Bölgesel envanter sistemini kontrol eder
3. Mağaza B'de 30 adet fazlalık görür
4. Acil transfer talebi gönderir (15 adet)

Gün 1 - Öğleden Sonra:
5. Bölge Müdürü onaylar (acil)
6. Mağaza B ürünleri toplar ve paketler
7. Mağaza B envanteri günceller (çıkış)
8. Aynı gün kurye ayarlar

Gün 2 - Sabah:
9. Mağaza A transferi alır
10. 15 adet alındığını doğrular
11. Envanteri günceller (giriş)
12. Transfer tamamlanmasını onaylar
```

**Sonuç:** Stoksuzluk önlendi, satışlar korundu


### Senaryo 3: Tutarsızlıklı Transfer

**Durum:** Alan mağaza beklenenden az ürün alıyor

**Süreç:**
```
1. 50 adetlik transfer gönderildi
2. Alan mağaza 47 adet sayar
3. Tutarsızlık hemen belgelenir
4. Alınan ürünlerin fotoğrafı çekilir
5. Gönderen mağazayla iletişim kurulur
6. Gönderen mağaza kayıtlarını kontrol eder
7. Araştırma 3 ürünün yanlış yerleştirildiğini belirler
8. Ek 3 adet gönderilir
9. Transfer tamamlanır
10. Olay inceleme için kaydedilir
```

**Sonuç:** Tutarsızlık çözüldü, süreç iyileştirildi


## KPI'lar

### Transfer Performans Metrikleri

| KPI | Formül | Hedef | Sıklık |
|-----|--------|-------|--------|
| **Transfer Doğruluğu** | Doğru transferler / Toplam | ≥%98 | Aylık |
| **Zamanında Uygulama** | Zamanında / Toplam | ≥%95 | Aylık |
| **Transfer Farkı** | Tutarsızlıklar / Toplam | <%1 | Aylık |
| **Talepten Teslimata** | Ort gün talepten | <3 gün | Aylık |
| **Transfer Hacmi** | Transfer edilen adet | Takip | Aylık |

### Performans Panosu

```
┌─────────────────────────────────────────────────────────────┐
│               TRANSFER PERFORMANS PANOSU                    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Transfer Doğruluğu    [████████████████████░░] %98        │
│  Hedef: ≥%98           ✓ KARŞILIYOR                         │
│                                                             │
│  Zamanında Uygulama    [██████████████████████░░] %96      │
│  Hedef: ≥%95           ✓ AŞIYOR                             │
│                                                             │
│  Transfer Farkı        [░░░░░░░░░░░░░░░░░░░░░░░░] %0.5     │
│  Hedef: <%1            ✓ HEDEF İÇİNDE                       │
│                                                             │
│  Ort Talepten Teslimata: 2.3 gün                           │
│  Hedef: <3 gün         ✓ HEDEF İÇİNDE                       │
│                                                             │
│  Bu Ayın Aktivitesi:                                        │
│  - Transfer talepleri: 15                                   │
│  - Onaylanan: 14                                            │
│  - Tamamlanan: 13                                           │
│  - Devam eden: 1                                            │
│  - Reddedilen: 1                                            │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```


### Sorun 2: Transfer Farkları

**Problem:** Transit sırasında eksik veya hasarlı ürünler

**Çözümler:**
- Sevkiyattan önce paketlemeyi iki kez kontrol et
- Güvenli ambalaj kullan
- Gönderim ve alımda fotoğraf çek
- Tüm ürünleri dikkatli belgele
- Güvenilir taşıyıcılar seç


### Sorun 4: Geçerli Taleplerin Reddi

**Problem:** Geçerli transfer talepleri reddediliyor

**Çözümler:**
- Net gerekçe sun
- Destekleyici veri ekle (satış hızı)
- Aciliyeti net ilet
- Bölge Müdürü ile takip et
- Ek bilgiyle yeniden değerlendirme talep et


## Revizyon Geçmişi

| Versiyon | Tarih | Değişiklik | Hazırlayan |
|----------|-------|------------|------------|
| 1.0 | 2025-12-01 | İlk versiyon oluşturuldu | Aydınlı Grup Tedarik Zinciri |


---

## SECTION 12: SHIPPING-LOGISTICS-GUIDE-TR

﻿---
layout: default
title: "Sevkiyat ve Lojistik Kılavuzu – TR"
parent: Supply Chain

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Süreç Adımları](#süreç-adımları)
6. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
7. [Kontrol Listeleri](#kontrol-listeleri)
8. [Örnekler / Senaryolar](#örnekler--senaryolar)
9. [KPI'lar](#kpilar)
10. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
11. [Revizyon Geçmişi](#revizyon-geçmişi)


## Kapsam

### Sevkiyat Yöntemleri

| Yöntem | Açıklama | Tedarik Süresi | Maliyet |
|--------|----------|----------------|---------|
| **Standart Kara** | Düzenli karayolu taşımacılığı | 3-7 gün | Düşük |
| **Ekspres Kara** | Öncelikli karayolu taşımacılığı | 1-3 gün | Orta |
| **Hava Kargo** | Hava taşımacılığı | 1-2 gün | Yüksek |
| **Deniz Taşımacılığı** | Okyanus konteyneri | 2-6 hafta | En Düşük |
| **Kurye** | Kapıdan kapıya ekspres | Aynı-Ertesi gün | En Yüksek |


## Sorumluluklar

### Mağaza Ekibi

| Sorumluluk | Standart |
|------------|----------|
| Gelen sevkiyatları takip et | Günlük |
| Teslimat kabulünü koordine et | Programa göre |
| Sevkiyat sorunlarını raporla | Hemen |
| Teslimat kabulünü onayla | Aynı gün |
| Sevkiyat kayıtlarını tut | Tam |

### Bölge Müdürü

| Sorumluluk | Standart |
|------------|----------|
| Bölgesel lojistiği denetle | Haftalık |
| Sevkiyat sorunlarını eskalasyon yap | Gerektiğinde |
| Teslimat rotalarını optimize et | Üç aylık |
| Taşıyıcı performansını incele | Aylık |

### Merkez Lojistik Ekibi

| Sorumluluk | Standart |
|------------|----------|
| Taşıyıcı ilişkilerini yönet | Sürekli |
| Sevkiyat fiyatlarını pazarlık et | Yıllık |
| Tüm sevkiyatları takip et | Gerçek zamanlı |
| Sevkiyat sorunlarını çöz | Öncelik bazında |
| Lojistik KPI'larını raporla | Aylık |


## Standartlar ve Kurallar

### Teslimat Standartları

| Standart | Hedef |
|----------|-------|
| **Zamanında Teslimat** | ≥%95 |
| **Hasarsız Teslimat** | ≥%99 |
| **Tam Sevkiyatlar** | ≥%98 |
| **POD Yakalama** | %100 |
| **Sorun Çözümü** | 48 saat içinde |

### Ambalaj Standartları

| Standart | Gereksinim |
|----------|------------|
| **Kutu Kalitesi** | Minimum çift duvarlı oluklu |
| **Dolgu Malzemesi** | Uygun boşluk doldurma |
| **Etiketleme** | Net, dayanıklı etiketler |
| **Ağırlık Limitleri** | Taşıyıcı kılavuzlarına göre |
| **Mühürleme** | Güvenli bant kapatma |

### Dokümantasyon Gereksinimleri

| Belge | Amaç | Saklama |
|-------|------|---------|
| Konşimento | Sevkiyat sözleşmesi | 3 yıl |
| Paket Listesi | İçerik detayı | 1 yıl |
| Ticari Fatura | Gümrük/değer | 3 yıl |
| POD | Teslimat onayı | 3 yıl |
| Sigorta Sertifikası | Kapsam kanıtı | 3 yıl |


## Örnekler / Senaryolar

### Senaryo 1: Standart Yurt İçi Sevkiyat Takibi

**Durum:** İstanbul DC'den haftalık yenileme

**Süreç:**
```
Gün 1 (Pazartesi):
1. Takip bilgili ASN e-postası al
2. Takip numarasını not et: TRK123456789
3. Beklenen teslimat: Perşembe

Gün 2-3:
4. Takibi günlük kontrol et
5. Durum: "Transit - Ankara Hub"
6. Programda

Gün 4 (Perşembe):
7. Durum: "Teslimata Çıktı"
8. Kabul alanını hazırla
9. Sevkiyatı 14:30'da al
10. 15 kutuyu beklenen şekilde doğrula
11. POD'u imzala
12. Sistemde onayla
```


### Senaryo 3: Uluslararası Gümrük İşlemleri

**Durum:** Sevkiyat gümrükte bekletildi

**Süreç:**
```
1. Takip "Gümrük Beklemesi" gösteriyor
2. Taşıma acentasıyla iletişim kur
3. Sorunu belirle: Eksik belge
4. HQ gerekli sertifikayı sağlar
5. Gümrük müşavirine gönder
6. Gümrük sevkiyatı serbest bırakır
7. Hedefe devam et
8. Gelecek referans için sorunu belgele
```


## KPI'lar

### Lojistik Performans Metrikleri

| KPI | Formül | Hedef | Sıklık |
|-----|--------|-------|--------|
| **Zamanında Teslimat** | Zamanında / Toplam | ≥%95 | Haftalık |
| **Hasar Oranı** | Hasarlı sevkiyat / Toplam | <%1 | Aylık |
| **Mükemmel Sipariş Oranı** | Tam, zamanında, hasarsız / Toplam | ≥%90 | Aylık |
| **Transit Süresi** | Sevkten teslimata ort gün | Rotaya göre | Aylık |
| **Birim Başına Maliyet** | Toplam sevkiyat maliyeti / Sevk edilen adet | Takip | Aylık |

### Performans Panosu

```
┌─────────────────────────────────────────────────────────────┐
│              SEVKİYAT VE LOJİSTİK PANOSU                    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Zamanında Teslimat    [██████████████████████░░] %96      │
│  Hedef: ≥%95           ✓ AŞIYOR                             │
│                                                             │
│  Hasar Oranı           [░░░░░░░░░░░░░░░░░░░░░░░░] %0.5     │
│  Hedef: <%1            ✓ HEDEF İÇİNDE                       │
│                                                             │
│  Mükemmel Sipariş Oranı[██████████████████████░░] %92      │
│  Hedef: ≥%90           ✓ AŞIYOR                             │
│                                                             │
│  Ort Transit Süresi:   3.2 gün (yurt içi)                  │
│  Önceki Ay:            3.5 gün                              │
│  Trend:                ✓ İYİLEŞİYOR                         │
│                                                             │
│  Bu Ayın Aktivitesi:                                        │
│  - Toplam sevkiyat: 24                                      │
│  - Zamanında: 23                                            │
│  - Gecikmeli: 1                                             │
│  - Hasarlı: 0                                               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```


### Sorun 2: Sık Hasar Talepleri

**Problem:** Ürünler hasarlı geliyor

**Çözümler:**
- Ambalaj standartlarını incele
- Gelişmiş ambalaj talep et
- Kalıpları belgele ve raporla
- Gerekirse taşıyıcı değiştir
- Hasar önleme tedbirleri uygula


### Sorun 4: Gümrük Gecikmeleri

**Problem:** Uluslararası sevkiyatlar gümrükte takılı

**Çözümler:**
- Dokümantasyonun tam olduğundan emin ol
- Deneyimli müşavirle çalış
- Mümkünse evrakları önceden temizle
- Hedef gereksinimleri anla
- Uyum kayıtlarını sürdür


## En İyi Uygulamalar

### Verimli Lojistik Yönetimi

1. **Proaktif Takip**
   - Durumu günlük kontrol et
   - Uyarılar ayarla
   - İstisnalara hemen aksiyon al

2. **İletişim**
   - Ekibi teslimatlar konusunda bilgilendir
   - Sorunları derhal raporla
   - Kabulü aynı gün onayla

3. **Dokümantasyon**
   - Tüm sevkiyat belgelerini tut
   - Hasarı hemen fotoğrafla
   - Talepleri son tarihler içinde dosyala

4. **Sürekli İyileştirme**
   - Gecikmelerdeki kalıpları takip et
   - Taşıyıcı geri bildirimi sağla
   - Süreç iyileştirmeleri öner


*Bu doküman Aydınlı Grup Tedarik Zinciri Ekibi tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*




---

## SECTION 13: WAREHOUSE-COORDINATION-GUIDE-TR

﻿---
layout: default
title: "Depo Koordinasyon Kılavuzu – TR"
parent: Supply Chain

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Süreç Adımları](#süreç-adımları)
6. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
7. [Kontrol Listeleri](#kontrol-listeleri)
8. [Örnekler / Senaryolar](#örnekler--senaryolar)
9. [KPI'lar](#kpilar)
10. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
11. [Revizyon Geçmişi](#revizyon-geçmişi)


## Kapsam

### Dağıtım Merkezi Ağı

| DC Lokasyonu | Kapsam | Fonksiyon |
|--------------|--------|-----------|
| **İstanbul (Ana)** | Türkiye, Avrupa, MENA | Birincil DC |
| **Bölgesel Hublar** | Yerel pazarlar | İkincil dağıtım |
| **Partner DC** | Amerika, APAC | Üçüncü parti lojistik |

### Depo Hizmetleri

| Hizmet | Açıklama | Uygunluk |
|--------|----------|----------|
| **Sipariş Karşılama** | Düzenli yenileme | Hafta içi |
| **Acil Siparişler** | Hızlı sevkiyatlar | Talep üzerine 7/24 |
| **İade İşleme** | İade ürün işleme | Hafta içi |
| **Onarım/Düzeltme** | Ürün onarımları | Randevuyla |
| **Envanter Sorguları** | Stok mevcudiyet kontrolleri | Hafta içi |


## Sorumluluklar

### Mağaza Ekibi

| Sorumluluk | Standart |
|------------|----------|
| Doğru siparişler gönder | Programa göre |
| Teslimat tercihlerini ilet | Önceden |
| Kabul sorunlarını raporla | 24 saat içinde |
| Envanter geri bildirimi sağla | Gerektiğinde |
| İadeleri koordine et | Prosedüre göre |

### Bölge Müdürü

| Sorumluluk | Standart |
|------------|----------|
| Bölgesel koordinasyonu denetle | Sürekli |
| Depo sorunlarını eskalasyon yap | Gerektiğinde |
| Bölgesel tahsisleri optimize et | Aylık |
| Depo performansını incele | Aylık |

### Merkez Depo Ekibi

| Sorumluluk | Standart |
|------------|----------|
| Siparişleri doğru işle | 24-48 saat |
| İletişim kanallarını sürdür | İş saatleri |
| Mağaza sorgularını çöz | Aynı gün |
| Karşılama KPI'larını izle | Günlük |
| Taşıyıcılarla koordine ol | Sürekli |


## Standartlar ve Kurallar

### İletişim Standartları

| Standart | Gereksinim |
|----------|------------|
| **Yanıt Süresi** | Standart: 24 saat, Acil: 2 saat |
| **İletişim Kanalı** | Standart için B2B Portal, Acil için Telefon |
| **Dokümantasyon** | Tüm sorgular kaydedilir |
| **Takip** | Çözüme kadar |
| **Eskalasyon** | Tanımlı süreçe göre |

### Sipariş İşleme Standartları

| Standart | Hedef |
|----------|-------|
| **Sipariş İşleme** | 24-48 saat içinde |
| **Toplama Doğruluğu** | ≥%99.5 |
| **Paketleme Kalitesi** | Standarda %100 |
| **Zamanında Sevk** | ≥%98 |
| **Dokümantasyon** | %100 doğru |

### Depo Çalışma Saatleri

| Gün | Saatler | Hizmetler |
|-----|---------|-----------|
| Pazartesi-Cuma | 08:00-18:00 | Tam hizmet |
| Cumartesi | 08:00-13:00 | Sınırlı hizmet |
| Pazar | Kapalı | Sadece acil |


## Örnekler / Senaryolar

### Senaryo 1: Standart Sipariş Sorgusu

**Durum:** Mağaza sipariş durumunu kontrol etmek istiyor

**Süreç:**
```
1. B2B Portalına giriş yap
2. Sipariş Geçmişine git
3. Sipariş numarasını bul
4. Durumu kontrol et:
   - Beklemede → İşleme sırasında
   - İşleniyor → Toplanıyor/paketleniyor
   - Sevk edildi → Transit (takip için tıkla)
   - Teslim edildi → Kabulü onayla
5. Soru varsa bilet gönder
6. Yanıt bekle (24 saat)
```


### Senaryo 3: Kalite Sorunu Raporlama

**Durum:** Kalite kusurlu ürünler alındı

**Süreç:**
```
1. Kusurlu ürünleri ayır
2. Tüm kusurları fotoğrafla
3. Belgele:
   - Sipariş/teslimat numarası
   - Etkilenen SKU'lar
   - Kusurlu miktar
   - Kusur açıklaması
4. B2B Portalı üzerinden kalite raporu gönder
5. Fotoğrafları ekle
6. Depo yanıtını bekle
7. Şunlar için talimatları izle:
   - DC'ye iade
   - Yerel imha
   - Kredi/değiştirme
```


## KPI'lar

### Depo Performans Metrikleri

| KPI | Formül | Hedef | Sıklık |
|-----|--------|-------|--------|
| **Sipariş Doğruluğu** | Doğru siparişler / Toplam | ≥%99 | Haftalık |
| **Zamanında Sevkiyat** | Zamanında / Toplam | ≥%98 | Haftalık |
| **Sorgu Yanıt Süresi** | Ort yanıt saati | <24 saat | Haftalık |
| **Sorun Çözümü** | SLA içinde çözülen / Toplam | ≥%95 | Aylık |
| **Toplama Doğruluğu** | Doğru toplamalar / Toplam | ≥%99.5 | Günlük |

### Performans Panosu

```
┌─────────────────────────────────────────────────────────────┐
│              DEPO KOORDİNASYON PANOSU                       │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Sipariş Doğruluğu     [████████████████████░░] %99        │
│  Hedef: ≥%99           ✓ KARŞILIYOR                         │
│                                                             │
│  Zamanında Sevkiyat    [████████████████████░░] %98        │
│  Hedef: ≥%98           ✓ KARŞILIYOR                         │
│                                                             │
│  Ort Yanıt Süresi      [████████████████░░░░░░] 18 saat    │
│  Hedef: <24 saat       ✓ HEDEF İÇİNDE                       │
│                                                             │
│  Sorun Çözümü          [██████████████████████░░] %96      │
│  Hedef: ≥%95           ✓ AŞIYOR                             │
│                                                             │
│  Bu Haftanın Aktivitesi:                                    │
│  - İşlenen siparişler: 45                                   │
│  - İşlenen sorgular: 12                                     │
│  - Çözülen sorunlar: 8                                      │
│  - Bekleyen sorunlar: 2                                     │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```


### Sorun 2: Stok Tahsis Sorunları

**Problem:** Beklenen tahsisi almıyor

**Çözümler:**
- Tahsis kriterlerini incele
- Bölge Müdürüyle iletişim kur
- Gerekçeyle tahsis itirazı gönder
- Alternatif kaynakları değerlendir
- Diğer mağazalardan transfer talep et


### Sorun 4: Tekrarlayan Kalite Sorunları

**Problem:** Tutarlı ürün kalite problemleri

**Çözümler:**
- Tüm sorunları sistematik belgele
- Trend raporu derle
- Resmi kalite şikayeti gönder
- Tedarikçi araştırması talep et
- Gelen ürün incelemesi uygula


## En İyi Uygulamalar

### Etkili İletişim

1. **Net ve Spesifik Ol**
   - Sipariş/referans numaralarını sağla
   - Tüm ilgili detayları dahil et
   - Destekleyici dokümantasyon ekle

2. **Doğru Kanalı Kullan**
   - Standart: B2B Portalı
   - Acil: Telefon
   - Karmaşık: Takipli e-posta

3. **Her Şeyi Belgele**
   - Bilet numaralarını kaydet
   - Konuşma detaylarını not et
   - Tüm yazışmaları dosyala

4. **Tutarlı Takip Et**
   - Durumu düzenli kontrol et
   - Gerektiğinde eskalasyon yap
   - Çözümü onayla


*Bu doküman Aydınlı Grup Tedarik Zinciri Ekibi tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*




---
