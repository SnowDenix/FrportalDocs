---
layout: default
title: "Sosyal Medya Lokalizasyon Rehberi"
parent: Marketing
nav_order: 18
---

# Sosyal Medya Lokalizasyon Rehberi

> **Son Güncelleme:** 01 Aralık 2025 | **Versiyon:** 1.0  
> **Hazırlayan:** Aydınlı Grup  
> **Markalar:** U.S. Polo Assn., Pierre Cardin, Cacharel

---

## İçindekiler

1. [Amaç](#amaç)
2. [Kapsam](#kapsam)
3. [Tanımlar](#tanımlar)
4. [Sorumluluklar](#sorumluluklar)
5. [Bölgeye Göre Platform Stratejisi](#bölgeye-göre-platform-stratejisi)
6. [İçerik Lokalizasyonu](#içerik-lokalizasyonu)
7. [Dil Kılavuzları](#dil-kılavuzları)
8. [Kültürel Değerlendirmeler](#kültürel-değerlendirmeler)
9. [Hashtag Stratejisi](#hashtag-stratejisi)
10. [Topluluk Yönetimi](#topluluk-yönetimi)
11. [Süreç Adımları](#süreç-adımları)
12. [Standartlar ve Kurallar](#standartlar-ve-kurallar)
13. [KPI'lar](#kpilar)
14. [Sık Karşılaşılan Sorunlar ve Çözümler](#sık-karşılaşılan-sorunlar-ve-çözümler)
15. [Revizyon Geçmişi](#revizyon-geçmişi)

---

## Amaç

Bu belge, marka tutarlılığını korurken farklı pazarlarda sosyal medya içeriğinin lokalize edilmesi için kapsamlı kılavuzlar sunar. Kültürel nüansları ve bölgesel tercihleri göz önünde bulundurarak yerel kitlelerle etkili iletişim kurulmasını sağlar.

### Lokalizasyon Hedefleri

| Hedef | Açıklama |
|-------|----------|
| **Alaka** | Yerel kitlelerle rezonans kuran içerik oluşturmak |
| **Tutarlılık** | Tüm pazarlarda marka kimliğini korumak |
| **Etkileşim** | Lokalize içerikle etkileşimi maksimize etmek |
| **Uyumluluk** | Yerel düzenlemelere ve kültürel normlara uymak |
| **Verimlilik** | Lokalizasyon süreçlerini düzenleştirmek |

---

## Kapsam

### Coğrafi Kapsam

| Bölge | Diller | Birincil Platformlar |
|-------|--------|---------------------|
| **KSA** | Arapça, İngilizce | Instagram, Snapchat, TikTok, X (Twitter) |
| **Fas** | Arapça, Fransızca | Instagram, Facebook, TikTok |
| **Balkanlar** | Yerel diller, İngilizce | Instagram, Facebook, TikTok |

### Kapsanan Markalar

- U.S. Polo Assn.
- Pierre Cardin
- Cacharel

---

## Tanımlar

| Terim | Tanım |
|-------|-------|
| **Lokalizasyon** | İçeriği belirli bölgesel pazarlar için uyarlama |
| **Transkreasyon** | Niyeti korurken içeriğin yaratıcı adaptasyonu |
| **RTL (Sağdan Sola)** | Arapça ve benzeri diller için metin yönü |
| **Kültürel Adaptasyon** | İçeriği yerel gelenek ve tercihlere uyacak şekilde değiştirme |
| **Coğrafi Hedefleme** | Coğrafi konuma göre kullanıcılara içerik sunma |
| **UGC** | Kullanıcı Tarafından Oluşturulan İçerik |
| **Influencer Pazarlama** | Marka tanıtımı için yerel kişiliklerle ortaklık |

---

## Sorumluluklar

### Mağaza Müdürü

| Görev | Açıklama | Sıklık |
|-------|----------|--------|
| Yerel İzleme | Mağaza etiketli içeriği izlemek | Günlük |
| İçerik Desteği | Yerel içerik oluşturma taleplerini desteklemek | Gerektiğinde |
| Geri Bildirim | Yerel etkileşim trendlerini raporlamak | Haftalık |

### Bölge Pazarlama Müdürü

| Görev | Açıklama | Sıklık |
|-------|----------|--------|
| İçerik Lokalizasyonu | Global içeriği bölge için uyarlamak | Kampanya başına |
| Topluluk Yönetimi | Bölgesel sosyal hesapları denetlemek | Günlük |
| Influencer Koordinasyonu | Bölgesel influencer ortaklıklarını yönetmek | Sürekli |
| Performans Analizi | Bölgesel sosyal metrikleri analiz etmek | Haftalık |

### Merkez Sosyal Medya Ekibi

| Görev | Açıklama | Sıklık |
|-------|----------|--------|
| Global Strateji | Global sosyal medya yönünü belirlemek | Üç ayda bir |
| İçerik Oluşturma | Global içerik şablonları oluşturmak | Kampanya başına |
| Marka Kılavuzları | Sosyal marka kılavuzlarını sürdürmek | Sürekli |
| Performans İncelemesi | Global sosyal performansı incelemek | Aylık |

---

## Bölgeye Göre Platform Stratejisi

### KSA (Suudi Arabistan)

| Platform | Öncelik | Kitle | İçerik Odağı |
|----------|---------|-------|--------------|
| **Instagram** | Birincil | 18-45, tüm cinsiyetler | Yaşam tarzı, ürün vitrinleri |
| **Snapchat** | Birincil | 18-35, tüm cinsiyetler | Kamera arkası, özel içerik |
| **TikTok** | Büyüyen | 16-30, tüm cinsiyetler | Trendler, eğlence |
| **X (Twitter)** | İkincil | 25-45, erkek ağırlıklı | Haberler, duyurular |

**KSA'ya Özgü Değerlendirmeler:**
- Tesettür standartları için içerik moderasyonu
- Birincil dil olarak Arapça
- İslami değerlere ve tatillere saygı
- Akşam saatlerinde zirve etkileşim
- Ramazan içerik stratejisi önemli

### Fas

| Platform | Öncelik | Kitle | İçerik Odağı |
|----------|---------|-------|--------------|
| **Instagram** | Birincil | 18-40, tüm cinsiyetler | Moda, yaşam tarzı |
| **Facebook** | Birincil | 25-55, tüm cinsiyetler | Topluluk, teklifler |
| **TikTok** | Büyüyen | 16-30, tüm cinsiyetler | Trendler, eğlence |

**Fas'a Özgü Değerlendirmeler:**
- İki dilli içerik (Arapça/Fransızca)
- Fransız moda terminolojisi kabul edilebilir
- İslami değerlere saygı
- Güçlü topluluk etkileşimi tercihi
- Turizm sezonu içerik fırsatları

### Balkanlar

| Platform | Öncelik | Kitle | İçerik Odağı |
|----------|---------|-------|--------------|
| **Instagram** | Birincil | 18-40, tüm cinsiyetler | Moda, yaşam tarzı |
| **Facebook** | Birincil | 25-55, tüm cinsiyetler | Topluluk, etkinlikler |
| **TikTok** | Büyüyen | 16-30, tüm cinsiyetler | Trendler, yerel kültür |

**Balkanlara Özgü Değerlendirmeler:**
- Birden fazla dil (Sırpça, Arnavutça, Makedonca, vb.)
- İngilizce yaygın olarak anlaşılır
- Avrupa moda duyarlılığı
- Gerektiğinde ülkeye özgü içerik
- Dini çeşitlilik farkındalığı

---

## İçerik Lokalizasyonu

### Lokalizasyon Çerçevesi

```
┌─────────────────────────────────────────────────────────────────┐
│                 İÇERİK LOKALİZASYON SÜRECİ                       │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  GLOBAL İÇERİK                                                   │
│  ├── Temel mesaj ve görseller                                    │
│  ├── Marka kılavuzları uyumluluğu                                │
│  └── Kampanya hedefleri                                          │
│           ↓                                                      │
│  BÖLGESEL ADAPTASYON                                             │
│  ├── Dil çevirisi/transkreasyon                                  │
│  ├── Kültürel değişiklikler                                      │
│  ├── Yerel referans eklemeleri                                   │
│  └── Zamanlama ayarlamaları                                      │
│           ↓                                                      │
│  YEREL UYGULAMA                                                  │
│  ├── Platforma özgü formatlama                                   │
│  ├── Yerel hashtagler                                            │
│  ├── Topluluk etkileşimi                                         │
│  └── Performans takibi                                           │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### İçerik Türleri ve Lokalizasyon Seviyesi

| İçerik Türü | Lokalizasyon Seviyesi | Onay Gerekli |
|-------------|----------------------|--------------|
| Marka kampanyası | Çeviri + kültürel inceleme | Merkez |
| Ürün lansmanı | Çeviri + yerel fiyatlandırma | Bölgesel |
| Sezonluk içerik | Tam lokalizasyon | Bölgesel |
| Promosyonel teklifler | Çeviri + yasal uyumluluk | Bölgesel |
| UGC yeniden paylaşımları | Açıklama lokalizasyonu | Mağaza/Bölgesel |
| Stories/Reels | Şablon adaptasyonu | Bölgesel |

### Görsel Lokalizasyon

| Unsur | Lokalizasyon Yaklaşımı |
|-------|------------------------|
| **Ürün Görselleri** | Yerel tercihlere göre ayarlama (stil) |
| **Yaşam Tarzı Görselleri** | Kültürel olarak uygun ortamlar kullan |
| **Modeller** | Temsilin pazarla eşleştiğinden emin ol |
| **Metin Bindirmeleri** | Çevir ve gerekirse RTL için ayarla |
| **Renkler** | Kültürel renk çağrışımlarını kontrol et |

---

## Dil Kılavuzları

### Çeviri vs. Transkreasyon

| Yaklaşım | Ne Zaman Kullanılır | Örnek |
|----------|---------------------|-------|
| **Doğrudan Çeviri** | Gerçek bilgiler, fiyatlar | "%50 İNDİRİM" → "خصم 50%" |
| **Transkreasyon** | Sloganlar, duygusal içerik | "Tarzını Yaşa" → yerel olarak rezonans kuran ifade |
| **İngilizce Koru** | Marka adları, hashtagler, teknoloji terimleri | "U.S. Polo Assn." aynı kalır |

### Arapça İçerik Kılavuzları

**Genel Kurallar:**
- Sağdan sola metin hizalaması
- Uygun olduğunda Modern Standart Arapça veya yerel lehçe kullan
- Marka iletişimleri için resmi ton
- Metin genişlemesini göz önünde bulundur (Arapça genellikle daha uzun)

**Tipografi:**
- Onaylı Arapça fontları kullan
- Küçük boyutlarda okunabilirliği koru
- Birden fazla cihazda test et

### Fransızca İçerik Kılavuzları (Fas)

**Genel Kurallar:**
- Marka iletişimleri için resmi "vous"
- Fransız moda terminolojisi kabul edilebilir
- Daha geniş erişim için Arapça ile denge kur

### Çok Dilli Paylaşımlar

**Strateji:**
- Yerel dille başla
- İngilizce ikincil olarak (isteğe bağlı)
- Cümle ortasında dil karıştırmaktan kaçın
- Diller arasında satır sonları kullan

---

## Kültürel Değerlendirmeler

### KSA Kültürel Kılavuzları

| Alan | Kılavuz |
|------|---------|
| **Görsellik** | Tesettürlü giyim kodları; aşırı cilt açıklığı yok |
| **Din** | İslami pratiklere saygı; Ramazan hassasiyeti |
| **Cinsiyet** | Cinsiyet temsil normlarını göz önünde bulundur |
| **Tatiller** | İslami tatilleri belirgin şekilde öne çıkar |
| **Değerler** | Aile, gelenek, kalite vurgulanan |

**İçerik Yapılması Gerekenler:**
✓ Aile odaklı içerik öne çıkar
✓ Tesettürlü moda standartlarına saygı göster
✓ İslami vesileri kutla
✓ Özendirici yaşam tarzı görselleri kullan
✓ Kalite ve mirası vurgula

**İçerik Yapılmaması Gerekenler:**
✗ Açık giyim görselleri
✗ Namaz vakitlerinde içerik
✗ Alkol veya uygunsuz referanslar
✗ Politik içerik
✗ İslami değerlerle çelişen içerik

### Fas Kültürel Kılavuzları

| Alan | Kılavuz |
|------|---------|
| **Görsellik** | Yerel tesettür standartlarına saygı |
| **Din** | İslami değerlendirmeler geçerli |
| **Dil** | Arapça-Fransızca dengesi |
| **Tatiller** | İslami ve ulusal tatiller |
| **Stil** | Avrupa etkisi takdir edilen |

### Balkanlar Kültürel Kılavuzları

| Alan | Kılavuz |
|------|---------|
| **Görsellik** | Avrupa moda standartları |
| **Din** | Farklı dini arka planlara saygı |
| **Dil** | Yerel dil önceliği, İngilizce ikincil |
| **Tatiller** | Ortodoks, Katolik ve Müslüman tatillerini göz önünde bulundur |
| **Milliyetçilik** | Politik hassas konulardan kaçın |

---

## Hashtag Stratejisi

### Global Marka Hashtagleri

| Marka | Birincil Hashtag | İkincil Hashtagler |
|-------|------------------|-------------------|
| **U.S. Polo Assn.** | #USPoloAssn | #PlayYourWay #LiveAuthentically |
| **Pierre Cardin** | #PierreCardin | #TimelessElegance #ModernClassic |
| **Cacharel** | #Cacharel | #RomanticStyle #CacharelParis |

### Bölgesel Hashtag Kılavuzları

**KSA:**
- Arapça hashtagler dahil et
- Konum bazlı etiketler kullan (#Riyadh, #Jeddah)
- Sezonluk: #رمضان_كريم #عيد_مبارك

**Fas:**
- Arapça ve Fransızca hashtagler
- Konum etiketleri (#Casablanca, #Marrakech)
- İki dilli marka etiketleri

**Balkanlar:**
- Yerel dil hashtagleri
- Ülkeye özgü etiketler (#Belgrade, #Skopje)
- Daha geniş erişim için İngilizce hashtagler

### Hashtag En İyi Uygulamaları

| Platform | Önerilen Sayı | Yerleşim |
|----------|---------------|----------|
| Instagram | 5-10 alakalı | İlk yorum veya açıklama |
| Facebook | Maksimum 2-3 | Açıklama |
| TikTok | 3-5 trend + marka | Açıklama |
| X (Twitter) | Maksimum 1-2 | Açıklama |

---

## Topluluk Yönetimi

### Yanıt Süresi Standartları

| Mesaj Türü | Hedef Yanıt Süresi |
|------------|-------------------|
| **Şikayetler** | 2 saat içinde |
| **Sorular** | 4 saat içinde |
| **Yorumlar (olumlu)** | 24 saat içinde |
| **Yorumlar (nötr)** | 24 saat içinde |
| **DM'ler** | 4 saat içinde |

### Yanıt Dili Kılavuzları

**Genel Prensipler:**
- Müşterinin kullandığı dilde yanıt ver
- Samimi olurken marka tonunu koru
- Asla halka açık tartışma yapma
- Hassas konuları DM'ye taşı

**Yanıt Şablonları (Dil için uyarla):**

| Durum | Yanıt Şablonu |
|-------|---------------|
| **Teşekkür** | "Güzel sözleriniz için teşekkürler! [Ürünü] sevmenize çok sevindik. 💙" |
| **Soru** | "Harika soru! [Cevap]. Daha fazla bilgi için DM atabilirsiniz!" |
| **Şikayet** | "Bunu duyduğumuz için üzgünüz. Çözmemize yardımcı olması için lütfen detaylarınızı DM olarak gönderin." |
| **Mağaza Sorusu** | "Bizi [konum] adresinde bulabilirsiniz. Bugün ziyaret edin! [Mağaza saatleri]" |

### Eskalasyon Protokolü

```
Seviye 1: Standart Yanıt (Mağaza/Bölge Ekibi)
├── Ürün soruları
├── Mağaza bilgileri
├── Genel iltifatlar
└── Basit şikayetler

Seviye 2: Bölge Müdürü İncelemesi
├── Kalıcı şikayetler
├── Olumsuz yorumlar
├── Influencer etkileşimleri
└── Hassas konular

Seviye 3: Merkez Eskalasyonu
├── Marka itibar sorunları
├── Yasal endişeler
├── PR krizleri
└── Influencer ortaklıkları
```

---

## Süreç Adımları

### İçerik Lokalizasyon Süreci

```
Adım 1: Global İçeriği Al
        ↓
Adım 2: Yerel Alaka için İncele
        ↓
Adım 3: Adaptasyon İhtiyaçlarını Belirle
        └── Dil, görseller, zamanlama, teklifler
        ↓
Adım 4: Çeviri/Transkreasyon Yap
        ↓
Adım 5: Kültürel İnceleme
        ↓
Adım 6: Görsel Adaptasyon (gerekirse)
        ↓
Adım 7: Bölge Müdürü Onayı
        ↓
Adım 8: Planla ve Yayınla
        ↓
Adım 9: İzle ve Etkileşim Kur
        ↓
Adım 10: Performansı Raporla
```

### Yerel İçerik Oluşturma Süreci

```
1. Yerel Fırsatı Belirle
   └── Etkinlik, trend veya mağaza aktivitesi
       ↓
2. İçerik Brifingi Oluştur
   └── Hedef, format, zamanlama
       ↓
3. Bölgesel Onay için Gönder
   └── Marka uyumluluğu kontrolü
       ↓
4. İçerik Oluştur
   └── Onaylı şablonları/varlıkları kullanarak
       ↓
5. Final İnceleme
   └── Dil, görseller, uyumluluk
       ↓
6. Yayınla
       ↓
7. İzle ve Etkileşim Kur
```

---

## Standartlar ve Kurallar

### İçerik Standartları

| Husus | Standart |
|-------|----------|
| **Marka Sesi** | Global kılavuzlarla tutarlı, yerel olarak adapte edilmiş |
| **Görsel Kalite** | Yüksek çözünürlük, iyi kompozisyon |
| **Dil Kalitesi** | Dilbilgisi doğru, uygun ton |
| **Doğruluk** | Tüm bilgiler doğrulanmış, fiyatlar doğru |
| **Zamanlama** | Optimal yerel saatlerde yayınlanmış |

### Yasaklanan İçerik

| Kategori | Örnekler |
|----------|----------|
| **Politik** | Politik açıklamalar, destekler |
| **Dini** | Herhangi bir dini rahatsız edebilecek içerik |
| **Tartışmalı** | Bölücü sosyal konular |
| **Rakip** | Rakiplerden bahsetme veya karşılaştırma |
| **Doğrulanmamış** | Kanıt olmadan iddialar |
| **Kişisel** | Personel kişisel bilgileri |

### Onay Matrisi

| İçerik Türü | Mağaza | Bölgesel | Merkez |
|-------------|--------|----------|--------|
| Global içeriği yeniden paylaş | ✓ | İnceleme | - |
| Yerel promosyonel paylaşım | - | ✓ | İnceleme |
| Influencer içeriği | - | ✓ | ✓ |
| Kriz yanıtı | - | İnceleme | ✓ |
| Yeni içerik türü | - | Talep | ✓ |

---

## KPI'lar

### Sosyal Medya Performans KPI'ları

| KPI | Formül | Hedef | Takip |
|-----|--------|-------|-------|
| **Etkileşim Oranı** | (Beğeni + Yorum + Paylaşım) / Takipçi × 100 | >%3 | Haftalık |
| **Takipçi Büyümesi** | (Yeni Takipçi - Kayıp) / Önceki Toplam × 100 | Aylık >%2 | Aylık |
| **Yanıt Oranı** | Yanıtlar / Toplam Mesajlar × 100 | >%95 | Haftalık |
| **Yanıt Süresi** | Ortalama yanıt süresi | <4 saat | Haftalık |
| **İçerik Performansı** | Paylaşım Etkileşimi / Ortalama × 100 | >%100 | Paylaşım başına |
| **Yerel Hashtag Kullanımı** | Yerel hashtagli paylaşımlar / Toplam × 100 | >%80 | Aylık |

### Bölgesel Kıyaslamalar

| Bölge | Etkileşim Oranı | Takipçi Büyümesi | En İyi Paylaşım Saatleri |
|-------|-----------------|-----------------|-------------------------|
| **KSA** | %3-5 | %3 | 20:00-23:00 AST |
| **Fas** | %3-4 | %2.5 | 19:00-22:00 WET |
| **Balkanlar** | %2.5-4 | %2 | 18:00-21:00 CET |

---

## Sık Karşılaşılan Sorunlar ve Çözümler

### Sorun 1: Lokalize İçerikte Düşük Etkileşim

**Belirti:** Lokalize paylaşımlar ortalamanın altında performans gösteriyor.

**Nedenler:**
- Çeviri rezonans kurmuyor
- Kültürel uyumsuzluk
- Zayıf zamanlama
- Alakasız içerik

**Çözümler:**
1. Transkreasyon kalitesini inceleyin
2. Yerel kitle araştırması yapın
3. Farklı paylaşım saatlerini test edin
4. En iyi performans gösteren yerel içeriği analiz edin
5. Daha fazla UGC dahil edin

---

### Sorun 2: Yerel Dilde Olumsuz Yorumlar

**Belirti:** Yerel dilde şikayetler veya eleştiriler.

**Çözümler:**
1. Çevirin ve sorunu anlayın
2. Aynı dilde hızlıca yanıt verin
3. Çözüm için DM'ye taşıyın
4. Ciddiyse üst seviyeye iletin
5. Kalıp analizi için belgeleyin

---

### Sorun 3: Kültürel Hata

**Belirti:** İçerik kültürel nedenlerle olumsuz geri bildirim alıyor.

**Acil Eylemler:**
1. İçeriği hemen kaldırın
2. Ciddiyeti ve erişimi değerlendirin
3. Uygun yanıt hazırlayın
4. Önemliyse Merkez'e iletin
5. Öğrenimleri belgeleyin

**Önleme:**
- Geliştirilmiş kültürel inceleme süreci
- Yerel ekip danışmanlığı
- Kültürel takvim farkındalığı
- Düzenli ekip eğitimi

---

## Kontrol Listeleri

### Yayın Öncesi Kontrol Listesi

- [ ] İçerik marka kılavuzlarıyla uyumlu
- [ ] Çeviri/transkreasyon incelendi
- [ ] Kültürel uygunluk doğrulandı
- [ ] Görseller yerel pazar için uygun
- [ ] Hashtagler yerel etiketleri içeriyor
- [ ] Fiyatlandırma/teklifler bölge için doğru
- [ ] Paylaşım zamanı bölge için optimal
- [ ] Bağlantılar çalışıyor ve lokalize edilmiş
- [ ] Onay alındı (gerekirse)

### Haftalık Sosyal Medya Denetimi

- [ ] Etkileşim metriklerini incele
- [ ] Bekleyen tüm yorumlara/mesajlara yanıt ver
- [ ] En iyi ve en kötü performans gösteren paylaşımları analiz et
- [ ] Rakip aktivitesini kontrol et
- [ ] UGC fırsatlarını belirle
- [ ] Yaklaşan içeriği planla
- [ ] Bölge Müdürü'ne raporla

---

## Revizyon Geçmişi

| Versiyon | Tarih | Değişiklikler | Hazırlayan |
|----------|-------|---------------|------------|
| 1.0 | 01 Aralık 2025 | İlk versiyon oluşturuldu | Aydınlı Grup Dijital Pazarlama |

---

*Bu belge Aydınlı Grup Dijital Pazarlama Operasyonları tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*


