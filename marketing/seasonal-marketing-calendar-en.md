---
layout: default
title: "Seasonal Marketing Calendar"
---

# Seasonal Marketing Calendar

> **Last Updated:** December 01, 2025 | **Version:** 1.0  
> **Prepared by:** Aydınlı Grup  
> **Brands:** U.S. Polo Assn., Pierre Cardin, Cacharel

---

## Table of Contents

1. [Purpose](#purpose)
2. [Scope](#scope)
3. [Definitions](#definitions)
4. [Responsibilities](#responsibilities)
5. [Annual Calendar Overview](#annual-calendar-overview)
6. [Spring/Summer Season](#springsummer-season)
7. [Fall/Winter Season](#fallwinter-season)
8. [Key Commercial Moments](#key-commercial-moments)
9. [Regional Considerations](#regional-considerations)
10. [Planning Timeline](#planning-timeline)
11. [Campaign Integration](#campaign-integration)
12. [Process Steps](#process-steps)
13. [Standards & Rules](#standards--rules)
14. [KPIs](#kpis)
15. [Common Issues & Solutions](#common-issues--solutions)
16. [Revision History](#revision-history)

---

## Purpose

This document provides the annual marketing calendar framework for all Aydınlı Grup franchise operations. It ensures coordinated marketing activities across all brands and regions, maximizing commercial opportunities while maintaining brand consistency.

### Calendar Objectives

| Objective | Description |
|-----------|-------------|
| **Alignment** | Synchronize marketing activities across all markets |
| **Planning** | Enable advance preparation for all campaigns |
| **Optimization** | Maximize commercial opportunities throughout the year |
| **Consistency** | Ensure unified brand messaging globally |
| **Efficiency** | Reduce last-minute preparations and costs |

---

## Scope

### Geographic Coverage

| Region | Countries | Calendar Adaptations |
|--------|-----------|---------------------|
| **KSA** | Saudi Arabia | Islamic calendar events, local holidays |
| **Morocco** | Morocco | Islamic calendar, French influences |
| **Balkans** | Serbia, Kosovo, N. Macedonia, Albania, Bosnia | Orthodox/Catholic holidays, regional events |

### Brands Covered

All three brands follow the core calendar with brand-specific adaptations:
- U.S. Polo Assn.
- Pierre Cardin
- Cacharel

---

## Definitions

| Term | Definition |
|------|------------|
| **Season** | Major retail period (SS = Spring/Summer, FW = Fall/Winter) |
| **Campaign** | Coordinated marketing initiative with specific objectives |
| **Commercial Moment** | Key sales opportunity (holiday, event, sale period) |
| **Launch Window** | Period when new collection becomes available |
| **Sale Period** | Discounted pricing period for seasonal clearance |
| **Blackout Period** | Time when no promotions are allowed |
| **Peak Week** | Highest traffic/sales week within a period |
| **Lead Time** | Time required before campaign execution |

---

## Responsibilities

### Store Staff

| Task | Description | Frequency |
|------|-------------|-----------|
| Calendar Review | Understand upcoming campaigns | Weekly |
| Implementation | Execute in-store activities on schedule | Per event |
| Customer Communication | Inform customers of upcoming events | Daily |

### Store Manager

| Task | Description | Frequency |
|------|-------------|-----------|
| Calendar Planning | Plan staff and inventory for key dates | Monthly |
| Campaign Preparation | Ensure readiness for each campaign | Per event |
| Performance Tracking | Monitor campaign results | Per campaign |
| Feedback Reporting | Report calendar execution issues | Weekly |

### Regional Manager

| Task | Description | Frequency |
|------|-------------|-----------|
| Regional Adaptation | Adapt calendar for regional needs | Quarterly |
| Resource Allocation | Ensure adequate resources per period | Monthly |
| Coordination | Align stores within region | Ongoing |
| Performance Analysis | Analyze regional calendar performance | Monthly |

### HQ Marketing Team

| Task | Description | Frequency |
|------|-------------|-----------|
| Calendar Development | Create and update annual calendar | Annually |
| Campaign Creation | Develop campaigns for key moments | Per campaign |
| Asset Distribution | Provide marketing materials | Per campaign |
| Performance Review | Review global calendar effectiveness | Quarterly |

---

## Annual Calendar Overview

### 12-Month Marketing Calendar

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        ANNUAL MARKETING CALENDAR                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  Q1 (Jan-Mar)                    │  Q2 (Apr-Jun)                            │
│  ├── Winter Sale (Jan)           │  ├── Ramadan/Eid (Regional)              │
│  ├── Valentine's Day (Feb)       │  ├── Mother's Day (May)                  │
│  └── SS Season Launch (Mar)      │  └── Summer Peak (Jun)                   │
│                                  │                                          │
│  Q3 (Jul-Sep)                    │  Q4 (Oct-Dec)                            │
│  ├── Summer Sale (Jul)           │  ├── Black Friday (Nov)                  │
│  ├── Back to School (Aug)        │  ├── Singles Day (Nov 11)                │
│  └── FW Season Launch (Sep)      │  └── Holiday Season (Dec)                │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Monthly Calendar Detail

| Month | Primary Focus | Secondary Events | Marketing Priority |
|-------|---------------|------------------|-------------------|
| **January** | Winter Sale | New Year | High |
| **February** | Valentine's Day | SS Preview | Medium |
| **March** | SS Season Launch | Women's Day | Very High |
| **April** | Ramadan (Regional) | Spring Break | High |
| **May** | Mother's Day | Eid (Regional) | High |
| **June** | Summer Collection | Father's Day | Medium |
| **July** | Summer Sale | Vacation | High |
| **August** | Back to School | Summer End | Very High |
| **September** | FW Season Launch | Fashion Week | Very High |
| **October** | Mid-Season | Halloween (Select) | Medium |
| **November** | Black Friday | Singles Day | Very High |
| **December** | Holiday Season | Year-End Sale | Very High |

---

## Spring/Summer Season

### SS Season Timeline

```
Week -8 to -4: Collection Preview & Planning
        ↓
Week -4 to -2: Store Setup Preparation
        ↓
Week -2 to 0: Full Window & Store Refresh
        ↓
Week 0: SEASON LAUNCH
        ↓
Week 1-8: Full Price Selling Period
        ↓
Week 9-12: Mid-Season Activity
        ↓
Week 13+: End of Season Sale
```

### SS Campaign Calendar

| Period | Campaign | Duration | Key Activities |
|--------|----------|----------|----------------|
| **Feb W4 - Mar W1** | SS Preview | 2 weeks | VIP access, early bird offers |
| **Mar W2** | SS Launch | 1 week | Full store refresh, launch event |
| **Mar W2 - May W4** | Spring Focus | 10 weeks | New arrivals, trend highlights |
| **Apr** | Ramadan (Regional) | 4 weeks | Special collections, gifting |
| **May W2** | Mother's Day | 1 week | Gift guide, special offers |
| **Jun** | Summer Collection | 4 weeks | Vacation wear, swimwear |
| **Jul W1** | Summer Sale Start | - | 30-50% off seasonal items |

### SS Hero Products by Brand

| Brand | SS Hero Categories | Marketing Focus |
|-------|-------------------|-----------------|
| **U.S. Polo Assn.** | Polos, Shorts, Swim | Active lifestyle, vacation |
| **Pierre Cardin** | Linen suits, Light shirts | Business casual, summer elegance |
| **Cacharel** | Floral dresses, Light fabrics | Romantic summer, garden party |

### SS Visual Themes

| Month | Window Theme | Color Story |
|-------|--------------|-------------|
| March | Spring Awakening | Pastels, Fresh greens |
| April | Bloom | Florals, Bright colors |
| May | Garden Party | Soft pinks, Natural tones |
| June | Summer Escape | Blues, Whites, Nautical |
| July | Beach & Resort | Vibrant colors, Tropical |

---

## Fall/Winter Season

### FW Season Timeline

```
Week -8 to -4: Collection Preview & Planning
        ↓
Week -4 to -2: Store Setup Preparation
        ↓
Week -2 to 0: Full Window & Store Refresh
        ↓
Week 0: SEASON LAUNCH
        ↓
Week 1-8: Full Price Selling Period
        ↓
Week 9-12: Black Friday / Holiday Build
        ↓
Week 13+: Holiday Peak & Year-End Sale
```

### FW Campaign Calendar

| Period | Campaign | Duration | Key Activities |
|--------|----------|----------|----------------|
| **Aug W4 - Sep W1** | FW Preview | 2 weeks | VIP access, trend preview |
| **Sep W2** | FW Launch | 1 week | Full store refresh, fashion event |
| **Sep W2 - Nov W2** | Autumn Focus | 9 weeks | Layering, transitional looks |
| **Nov W3** | Black Friday | 1 week | Major discounts, door busters |
| **Dec W1-3** | Holiday Season | 3 weeks | Gift guide, festive collections |
| **Dec W4** | Year-End Sale | 1 week | Final clearance |

### FW Hero Products by Brand

| Brand | FW Hero Categories | Marketing Focus |
|-------|-------------------|-----------------|
| **U.S. Polo Assn.** | Sweaters, Outerwear, Boots | Campus style, weekend casual |
| **Pierre Cardin** | Suits, Coats, Knitwear | Business elegance, evening wear |
| **Cacharel** | Dresses, Coats, Accessories | Romantic layers, party season |

### FW Visual Themes

| Month | Window Theme | Color Story |
|-------|--------------|-------------|
| September | Back to Basics | Navy, Burgundy, Camel |
| October | Autumn Layers | Earth tones, Rich colors |
| November | Urban Chic | Black, Grey, Bold accents |
| December | Festive Magic | Gold, Red, Holiday sparkle |

---

## Key Commercial Moments

### Tier 1 Commercial Moments (Must Execute)

| Event | Date | Duration | Priority |
|-------|------|----------|----------|
| **SS Season Launch** | Early March | 2 weeks | Critical |
| **Black Friday** | 4th Friday Nov | 4-7 days | Critical |
| **FW Season Launch** | Early September | 2 weeks | Critical |
| **Holiday/Year-End** | Dec 15-31 | 2 weeks | Critical |

### Tier 2 Commercial Moments (Recommended)

| Event | Date | Duration | Priority |
|-------|------|----------|----------|
| **Valentine's Day** | Feb 14 | 1 week | High |
| **Mother's Day** | 2nd Sunday May | 1 week | High |
| **Summer Sale** | Early July | 3-4 weeks | High |
| **Back to School** | Mid-Aug | 3 weeks | High |

### Tier 3 Commercial Moments (Optional/Regional)

| Event | Date | Region | Priority |
|-------|------|--------|----------|
| **Ramadan/Eid** | Varies | KSA, Morocco | Critical (Regional) |
| **Singles Day** | Nov 11 | All | Medium |
| **Father's Day** | 3rd Sunday June | All | Medium |
| **Halloween** | Oct 31 | Balkans | Low |

### Commercial Moment Planning Template

```
┌─────────────────────────────────────────────────────────────────┐
│              COMMERCIAL MOMENT PLANNING                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Event: _______________________                                  │
│  Date(s): _______________________                                │
│  Duration: _______________________                               │
│                                                                  │
│  MARKETING ELEMENTS                                              │
│  □ Window Display Update                                         │
│  □ In-Store POSM                                                 │
│  □ Digital/Social Campaign                                       │
│  □ Email Campaign                                                │
│  □ Special Promotion/Offer                                       │
│  □ Staff Training                                                │
│                                                                  │
│  TARGETS                                                         │
│  Sales Target: $_____________                                    │
│  Traffic Target: ____________                                    │
│  Conversion Target: ________%                                    │
│                                                                  │
│  RESOURCES NEEDED                                                │
│  Budget: $_____________                                          │
│  Staff: _______________                                          │
│  Inventory Focus: _______________________                        │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## Regional Considerations

### KSA (Saudi Arabia)

| Consideration | Details | Impact |
|---------------|---------|--------|
| **Islamic Calendar** | Ramadan, Eid al-Fitr, Eid al-Adha | Major commercial moments |
| **Weekend** | Friday-Saturday | Different traffic patterns |
| **Shopping Hours** | Extended during Ramadan | Store operations |
| **Modest Fashion** | Conservative preferences | Product selection |
| **Summer Heat** | Extreme temperatures Jun-Aug | Indoor mall focus |

**KSA-Specific Calendar Additions:**

| Event | Timing | Marketing Focus |
|-------|--------|-----------------|
| Ramadan | Varies (Lunar) | Evening shopping, family gifting |
| Eid al-Fitr | End of Ramadan | Celebration outfits, gifts |
| Eid al-Adha | 70 days after Eid al-Fitr | Premium items, family |
| National Day | Sep 23 | Patriotic themes |
| Founding Day | Feb 22 | Heritage celebration |

### Morocco

| Consideration | Details | Impact |
|---------------|---------|--------|
| **Islamic Calendar** | Same as KSA | Major commercial moments |
| **French Influence** | European fashion calendar | Dual influences |
| **Tourism Season** | Peak summer | International customers |
| **Local Holidays** | Independence Day, Throne Day | Regional promotions |

**Morocco-Specific Calendar Additions:**

| Event | Timing | Marketing Focus |
|-------|--------|-----------------|
| Ramadan/Eid | Varies | Family celebrations |
| Independence Day | Nov 18 | National pride |
| Throne Day | Jul 30 | Celebration |
| Green March | Nov 6 | Patriotic themes |

### Balkans

| Consideration | Details | Impact |
|---------------|---------|--------|
| **Religious Diversity** | Orthodox, Catholic, Muslim | Multiple holiday periods |
| **EU Alignment** | Following EU calendar | Standard retail calendar |
| **Back to School** | Early September | Key commercial moment |
| **Local Holidays** | Varies by country | Regional adaptation |

**Balkans-Specific Calendar Additions:**

| Event | Timing | Marketing Focus |
|-------|--------|-----------------|
| Orthodox Christmas | Jan 7 | Serbia, N. Macedonia |
| Orthodox Easter | Varies | Spring celebration |
| Catholic Holidays | Standard dates | Select markets |
| Local National Days | Varies | Country-specific |

---

## Planning Timeline

### Annual Planning Cycle

```
October-November (Y-1)
├── Annual calendar draft
├── Budget planning
└── Agency briefings

December (Y-1)
├── Calendar finalization
├── Asset production start
└── Regional adaptation meetings

January
├── Q1 campaign materials ready
├── SS preview preparations
└── Regional calendars confirmed

Monthly (Ongoing)
├── Monthly review meetings
├── Next month preparations
└── Performance tracking
```

### Campaign Lead Times

| Campaign Type | Lead Time | Key Milestones |
|---------------|-----------|----------------|
| **Season Launch** | 12 weeks | Brief → Creative → Production → Delivery |
| **Major Promotion** | 8 weeks | Brief → Approval → Production → Distribution |
| **Standard Campaign** | 6 weeks | Brief → Assets → Store Implementation |
| **Quick Response** | 2 weeks | Approval → Digital Assets Only |

### Monthly Planning Checklist

- [ ] Review next month's calendar activities
- [ ] Confirm POSM delivery dates
- [ ] Brief store teams on upcoming campaigns
- [ ] Verify inventory alignment with promotions
- [ ] Coordinate regional adaptations
- [ ] Set performance targets
- [ ] Schedule review meetings

---

## Campaign Integration

### Cross-Channel Campaign Integration

```
┌─────────────────────────────────────────────────────────────────┐
│                 INTEGRATED CAMPAIGN EXECUTION                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Week -2: TEASE                                                  │
│  ├── Social media hints                                          │
│  ├── Email preview to VIPs                                       │
│  └── Window teaser                                               │
│                                                                  │
│  Week -1: BUILD                                                  │
│  ├── Full social campaign launch                                 │
│  ├── Email to full database                                      │
│  ├── In-store staff preparation                                  │
│  └── POSM installation                                           │
│                                                                  │
│  Week 0: LAUNCH                                                  │
│  ├── Full window display                                         │
│  ├── All channels active                                         │
│  ├── Launch event (if applicable)                                │
│  └── PR/Influencer activation                                    │
│                                                                  │
│  Week 1+: SUSTAIN                                                │
│  ├── Reminder communications                                     │
│  ├── Performance optimization                                    │
│  ├── User-generated content                                      │
│  └── Retargeting campaigns                                       │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### Channel Timing by Campaign Phase

| Phase | Social Media | Email | In-Store | Paid Media |
|-------|--------------|-------|----------|------------|
| **Tease** | Hints, countdown | VIP preview | Subtle updates | Low spend |
| **Build** | Full content | Database send | Staff brief | Medium spend |
| **Launch** | Peak posting | Launch reminder | Full display | Peak spend |
| **Sustain** | Engagement | Reminders | Maintained | Optimized |
| **Close** | Final push | Last chance | Wind down | Final push |

---

## Process Steps

### Annual Calendar Creation Process

```
Step 1: Review Previous Year Performance
        ↓
Step 2: Identify Key Dates and Events
        ↓
Step 3: Draft Calendar Framework
        ↓
Step 4: Regional Input and Adaptation
        ↓
Step 5: Budget Allocation by Period
        ↓
Step 6: Campaign Brief Development
        ↓
Step 7: Asset Production Planning
        ↓
Step 8: Final Calendar Approval
        ↓
Step 9: Distribution to All Stakeholders
        ↓
Step 10: Monthly Review and Adjustment
```

### Campaign Execution Process

```
1. Calendar Alert (4 weeks before)
       ↓
2. Receive Campaign Brief
       ↓
3. Review Assets and Materials
       ↓
4. Plan Local Implementation
       ↓
5. Receive POSM and Materials
       ↓
6. Staff Training and Briefing
       ↓
7. Store Setup (day before)
       ↓
8. Campaign Launch
       ↓
9. Daily Monitoring
       ↓
10. Campaign Close and Reporting
```

---

## Standards & Rules

### Calendar Compliance Rules

1. **Mandatory Campaigns:** All Tier 1 campaigns must be executed
2. **Timing:** Campaign start/end dates are fixed globally
3. **Messaging:** Use only approved campaign messaging
4. **Pricing:** Follow HQ-approved pricing and discounts
5. **Reporting:** Submit performance reports within 48 hours of campaign end

### Blackout Period Rules

| Blackout Type | Description | Duration |
|---------------|-------------|----------|
| **New Season Launch** | No discounts on new arrivals | First 4 weeks |
| **Before Major Sale** | No promotions before official sale | 2 weeks |
| **Brand Events** | Reserved for specific brand activities | As communicated |

### Promotional Calendar Rules

| Rule | Requirement |
|------|-------------|
| Discount Limits | Maximum discount levels per period defined by HQ |
| Stacking | No combining multiple offers unless approved |
| Communication | All promotional messaging must be pre-approved |
| Duration | Promotional periods cannot be extended without approval |
| Coverage | Promotions apply to specified product categories only |

---

## KPIs

### Calendar Performance KPIs

| KPI | Formula | Target | Frequency |
|-----|---------|--------|-----------|
| **Campaign Execution Rate** | Campaigns Executed / Campaigns Planned × 100 | 100% | Monthly |
| **On-Time Launch Rate** | On-Time Launches / Total Launches × 100 | 95% | Monthly |
| **Sales vs. Target** | Actual Sales / Target Sales × 100 | ≥100% | Per Campaign |
| **Traffic vs. LY** | This Year Traffic / Last Year Traffic × 100 | >100% | Weekly |
| **Seasonal Sell-Through** | Units Sold / Units Bought × 100 | >70% | End of Season |

### Commercial Moment Performance

| Metric | Calculation | Benchmark |
|--------|-------------|-----------|
| **Black Friday Sales Lift** | BF Sales / Normal Week Sales | 3x+ |
| **Holiday Season Share** | Dec Sales / Annual Sales | 15%+ |
| **Season Launch Week** | Launch Week / Normal Week | 2x+ |
| **Sale Period Efficiency** | Sale Margin / Regular Margin | >50% |

### Calendar Review Template

```
┌─────────────────────────────────────────────────────────────────┐
│              MONTHLY CALENDAR REVIEW                             │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Month: _______________ Year: ___________                        │
│                                                                  │
│  CAMPAIGNS EXECUTED                                              │
│  Planned: _____ Executed: _____ Success Rate: _____%             │
│                                                                  │
│  SALES PERFORMANCE                                               │
│  Target: $_________ Actual: $_________ Achievement: _____%       │
│                                                                  │
│  KEY LEARNINGS                                                   │
│  What worked: _________________________________                  │
│  What didn't: _________________________________                  │
│  Recommendations: ____________________________                   │
│                                                                  │
│  NEXT MONTH PREVIEW                                              │
│  Key Events: _________________________________                   │
│  Focus Areas: ________________________________                   │
│  Preparation Status: _________________________                   │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## Common Issues & Solutions

### Issue 1: Late Campaign Materials

**Symptom:** POSM and assets arrive after campaign start date.

**Causes:**
- Production delays
- Shipping issues
- Late approval

**Solution:**
1. Order materials 4+ weeks in advance
2. Use digital alternatives for day-one if physical delayed
3. Implement buffer time in planning
4. Have emergency local printing contacts

---

### Issue 2: Regional Calendar Conflicts

**Symptom:** Global campaign timing conflicts with regional events.

**Causes:**
- Islamic calendar variation
- Local holidays not considered
- Regional market differences

**Solution:**
1. Submit regional calendar conflicts early (Q4 previous year)
2. Request timing adaptation from HQ
3. Propose alternative execution dates
4. Document conflicts for future planning

---

### Issue 3: Inventory Misalignment

**Symptom:** Promoted products not available during campaign.

**Causes:**
- Forecasting errors
- Supply chain delays
- Poor communication with merchandising

**Solution:**
1. Share calendar with merchandising team monthly
2. Confirm stock levels 2 weeks before campaign
3. Identify backup products for promotion
4. Communicate stock issues immediately to marketing

---

### Issue 4: Campaign Fatigue

**Symptom:** Declining response to repeated promotional messages.

**Causes:**
- Too many campaigns
- Lack of variety in messaging
- Over-communication

**Solution:**
1. Space campaigns appropriately
2. Vary messaging and creative
3. Segment audience for targeted communications
4. Focus on quality over quantity

---

## Checklists

### Season Launch Preparation Checklist

- [ ] Campaign brief reviewed (8 weeks before)
- [ ] Assets received and approved (4 weeks before)
- [ ] POSM ordered (4 weeks before)
- [ ] Staff training scheduled (2 weeks before)
- [ ] Inventory confirmed (2 weeks before)
- [ ] Store setup planned (1 week before)
- [ ] POSM received (1 week before)
- [ ] Digital assets scheduled (1 week before)
- [ ] Staff trained (3 days before)
- [ ] Store setup complete (day before)
- [ ] Launch day checklist ready

### Monthly Calendar Review Checklist

- [ ] Previous month performance analyzed
- [ ] Next month campaigns confirmed
- [ ] Materials status checked
- [ ] Regional adaptations approved
- [ ] Budget utilization reviewed
- [ ] Staff schedules aligned
- [ ] Inventory alignment confirmed
- [ ] Performance targets set

---

## Revision History

| Version | Date | Changes | Prepared by |
|---------|------|---------|-------------|
| 1.0 | December 01, 2025 | Initial version created | Aydınlı Grup |

---

*This document was prepared by Aydınlı Grup.*  
*© 2025 Aydınlı Group - All rights reserved.*


