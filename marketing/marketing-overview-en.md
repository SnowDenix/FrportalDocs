---
layout: default
title: "Marketing Overview"
parent: Marketing
---

# Marketing Overview

> **Last Updated:** December 01, 2025 | **Version:** 1.0  
> **Prepared by:** Aydınlı Group  
> **Brands:** U.S. Polo Assn., Pierre Cardin, Cacharel

---

## Table of Contents

1. [Purpose](#purpose)
2. [Scope](#scope)
3. [Definitions](#definitions)
4. [Responsibilities](#responsibilities)
5. [Marketing Structure](#marketing-structure)
6. [Brand Marketing Pillars](#brand-marketing-pillars)
7. [Marketing Calendar Framework](#marketing-calendar-framework)
8. [Campaign Types](#campaign-types)
9. [Marketing Channels](#marketing-channels)
10. [Budget Planning](#budget-planning)
11. [Performance Measurement](#performance-measurement)
12. [Process Steps](#process-steps)
13. [Standards & Rules](#standards--rules)
14. [KPIs](#kpis)
15. [Common Issues & Solutions](#common-issues--solutions)
16. [Revision History](#revision-history)

---

## Purpose

This document provides a comprehensive overview of marketing operations for Aydınlı Group franchise partners managing U.S. Polo Assn., Pierre Cardin, and Cacharel brands. It establishes the framework for all marketing activities, ensuring brand consistency, operational efficiency, and measurable results across all franchise territories.

### Marketing Vision

- Build and maintain strong brand equity across all markets
- Drive customer acquisition and retention through targeted campaigns
- Ensure consistent brand messaging across all touchpoints
- Support franchise partners with effective marketing tools and resources
- Maximize return on marketing investment (ROMI)

### Marketing Objectives

| Objective | Description | Target |
|-----------|-------------|--------|
| Brand Awareness | Increase brand recognition in target markets | +15% YoY |
| Customer Acquisition | Drive new customer traffic to stores | +10% YoY |
| Customer Retention | Increase repeat purchase rate | 35%+ |
| Sales Support | Support seasonal and promotional sales targets | Campaign ROI 3:1 |
| Digital Engagement | Grow social media and digital presence | +25% engagement |

---

## Scope

### Geographic Coverage

| Region | Countries | Marketing Lead |
|--------|-----------|----------------|
| **KSA** | Saudi Arabia | Regional Marketing Manager |
| **Morocco** | Morocco | Regional Marketing Manager |
| **Balkans** | Serbia, Kosovo, North Macedonia, Albania, Bosnia | Regional Marketing Manager |

### Who This Document Covers

- **Store Staff:** Understanding of marketing campaigns and customer communication
- **Store Managers:** Campaign execution, local marketing coordination
- **Regional Managers:** Regional marketing strategy and budget management
- **Franchise Owners:** Marketing investment decisions and ROI tracking
- **HQ Marketing Team:** Central campaign development and brand management

### Covered Activities

1. Brand marketing and positioning
2. Seasonal and promotional campaigns
3. Digital and social media marketing
4. In-store marketing and POSM
5. Local market activation
6. Marketing performance measurement

---

## Definitions

### Core Marketing Terms

| Term | Definition |
|------|------------|
| **ATL (Above The Line)** | Mass media advertising including TV, radio, outdoor, and print media |
| **BTL (Below The Line)** | Direct marketing activities including in-store promotions, events, and direct mail |
| **POSM (Point of Sale Materials)** | Marketing materials displayed at the point of purchase including posters, danglers, standees |
| **Campaign** | A coordinated series of marketing activities designed to achieve specific objectives |
| **Hero Product** | Key product featured in marketing communications to drive traffic and sales |
| **Marketing Mix** | The combination of product, price, place, and promotion strategies |
| **Brand Guidelines** | Official rules governing the use of brand assets and messaging |
| **Co-op Marketing** | Shared marketing investment between HQ and franchise partners |
| **ROMI** | Return on Marketing Investment - revenue generated per marketing dollar spent |
| **Reach** | Total number of unique individuals exposed to marketing communication |
| **Frequency** | Average number of times an individual is exposed to marketing communication |
| **Impression** | Single instance of marketing content being displayed |
| **CTR (Click-Through Rate)** | Percentage of people who click on a digital advertisement |
| **Conversion Rate** | Percentage of marketing-influenced visitors who make a purchase |
| **Customer Lifetime Value (CLV)** | Total revenue expected from a customer over their entire relationship |

### Campaign Types

| Type | Definition | Typical Duration |
|------|------------|------------------|
| **Seasonal Campaign** | Major campaign aligned with fashion seasons (SS/FW) | 8-12 weeks |
| **Promotional Campaign** | Short-term discount or offer-based campaign | 1-4 weeks |
| **Brand Campaign** | Image and awareness-focused campaign | 4-8 weeks |
| **Event Campaign** | Campaign tied to specific events or holidays | 1-2 weeks |
| **Always-On** | Continuous background marketing activity | Ongoing |

---

## Responsibilities

### Store Staff

| Task | Description | Frequency |
|------|-------------|-----------|
| POSM Implementation | Display marketing materials correctly | Per campaign |
| Customer Communication | Inform customers about current promotions | Daily |
| Campaign Feedback | Report customer reactions and questions | Weekly |
| Social Media Support | Support local social media content | As requested |
| Data Collection | Capture customer information for CRM | Every transaction |

### Store Manager

| Task | Description | Frequency |
|------|-------------|-----------|
| Campaign Execution | Ensure all campaign elements are implemented | Per campaign |
| Staff Training | Train team on campaign messaging | Per campaign |
| Local Coordination | Coordinate with local media and partners | As needed |
| Performance Tracking | Monitor campaign sales impact | Daily/Weekly |
| Feedback Reporting | Submit campaign performance reports | Weekly |
| Budget Management | Manage local marketing expenses | Monthly |

### Regional Manager

| Task | Description | Frequency |
|------|-------------|-----------|
| Strategy Implementation | Execute regional marketing strategy | Ongoing |
| Budget Allocation | Distribute marketing budget across stores | Quarterly |
| Performance Analysis | Analyze regional marketing performance | Weekly |
| Local Adaptation | Adapt campaigns for regional requirements | Per campaign |
| Agency Management | Coordinate with regional marketing agencies | Ongoing |
| Compliance Audit | Ensure brand guideline compliance | Monthly |

### HQ Marketing Team

| Task | Description | Frequency |
|------|-------------|-----------|
| Campaign Development | Create central marketing campaigns | Seasonal |
| Brand Management | Maintain and evolve brand guidelines | Ongoing |
| Asset Creation | Produce marketing materials and assets | Per campaign |
| Performance Analysis | Analyze global marketing performance | Monthly |
| Budget Planning | Develop annual marketing budget | Annually |
| Partner Support | Provide marketing support to franchisees | Ongoing |

---

## Marketing Structure

### Organizational Hierarchy

```
┌─────────────────────────────────────────────────────────────────┐
│                    HQ MARKETING DEPARTMENT                       │
├─────────────────────────────────────────────────────────────────┤
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐              │
│  │   Brand     │  │   Digital   │  │   Trade     │              │
│  │  Marketing  │  │  Marketing  │  │  Marketing  │              │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘              │
│         │                │                │                      │
│         └────────────────┼────────────────┘                      │
│                          │                                       │
│               ┌──────────┴──────────┐                            │
│               │  Campaign Manager   │                            │
│               └──────────┬──────────┘                            │
└──────────────────────────┼──────────────────────────────────────┘
                           │
        ┌──────────────────┼──────────────────┐
        │                  │                  │
┌───────┴───────┐  ┌───────┴───────┐  ┌───────┴───────┐
│  KSA Region   │  │ Morocco Region│  │ Balkans Region│
│  Marketing    │  │  Marketing    │  │   Marketing   │
└───────┬───────┘  └───────┬───────┘  └───────┬───────┘
        │                  │                  │
   Store Level        Store Level        Store Level
   Execution          Execution          Execution
```

### Communication Flow

```
1. HQ Campaign Briefing
       ↓
2. Regional Adaptation Meeting
       ↓
3. Local Market Planning
       ↓
4. Store Implementation Briefing
       ↓
5. Campaign Execution
       ↓
6. Performance Reporting (Store → Region → HQ)
```

---

## Brand Marketing Pillars

### U.S. Polo Assn.

**Brand Positioning:** Authentic American lifestyle brand with polo heritage

| Pillar | Description | Key Messages |
|--------|-------------|--------------|
| **Heritage** | Authentic connection to polo sport | "The Official Brand of USPA" |
| **Lifestyle** | American casual lifestyle | "Live Authentically" |
| **Quality** | Reliable quality at accessible prices | "Quality You Can Trust" |
| **Versatility** | From sport to street | "Play Your Way" |

**Marketing Tone:**
- Confident and authentic
- Sporty yet sophisticated
- Inclusive and accessible
- Energetic and optimistic

**Visual Identity:**
- Primary Colors: Navy, White, Red
- Secondary Colors: Green, Yellow
- Imagery: Active lifestyle, polo references, American landscapes

---

### Pierre Cardin

**Brand Positioning:** Modern elegance with French sophistication

| Pillar | Description | Key Messages |
|--------|-------------|--------------|
| **Elegance** | Refined French style | "Timeless Elegance" |
| **Innovation** | Fashion-forward design | "Modern Design" |
| **Quality** | Premium materials and craftsmanship | "Superior Quality" |
| **Sophistication** | Understated luxury | "Effortless Style" |

**Marketing Tone:**
- Sophisticated and refined
- Confident and assured
- Modern yet timeless
- Premium but accessible

**Visual Identity:**
- Primary Colors: Black, Grey, Navy
- Secondary Colors: Burgundy, Gold
- Imagery: Urban sophistication, business settings, evening occasions

---

### Cacharel

**Brand Positioning:** Romantic, feminine French bohemian style

| Pillar | Description | Key Messages |
|--------|-------------|--------------|
| **Romance** | Romantic French aesthetic | "Simply Romantic" |
| **Femininity** | Celebrating women | "Modern Femininity" |
| **Nature** | Floral and natural inspirations | "Natural Beauty" |
| **Joy** | Playful and optimistic | "Joyful Living" |

**Marketing Tone:**
- Romantic and dreamy
- Soft and feminine
- Playful and optimistic
- Artistic and creative

**Visual Identity:**
- Primary Colors: Pastels, White, Pink
- Secondary Colors: Floral patterns, Soft blues
- Imagery: Gardens, flowers, romantic settings, artistic expressions

---

## Marketing Calendar Framework

### Annual Planning Cycle

| Month | Planning Activity | Campaign Focus |
|-------|-------------------|----------------|
| January | Q1 Review, SS Campaign Launch | Winter Sale, Valentine's Preview |
| February | Valentine's Campaign | Valentine's Day |
| March | SS Full Launch | Spring/Summer Season Opening |
| April | Ramadan Planning (Regional) | Ramadan Campaign |
| May | Mother's Day, FW Planning Start | Mother's Day |
| June | Summer Campaign Peak | Summer Collection |
| July | Summer Sale, FW Development | End of Season Sale |
| August | Back to School | Back to School Campaign |
| September | FW Campaign Launch | Fall/Winter Season Opening |
| October | Q4 Planning | Halloween (Select Markets) |
| November | Black Friday | Black Friday/Cyber Monday |
| December | Holiday Season | Christmas/New Year |

### Campaign Planning Timeline

```
Week -8: Campaign Brief from HQ
    ↓
Week -6: Regional Adaptation Plans
    ↓
Week -4: Asset Localization Complete
    ↓
Week -2: POSM Production & Distribution
    ↓
Week -1: Store Implementation Briefing
    ↓
Week 0: Campaign Launch
    ↓
Week +1 to +N: Campaign Execution & Optimization
    ↓
Campaign End: Post-Campaign Analysis (within 2 weeks)
```

---

## Campaign Types

### Tier 1: Major Seasonal Campaigns

| Characteristic | Details |
|----------------|---------|
| **Frequency** | 2 per year (SS, FW) |
| **Duration** | 8-12 weeks |
| **Budget Allocation** | 40% of annual marketing budget |
| **Channels** | ATL + BTL + Digital + In-Store |
| **HQ Support** | Full campaign kit, media planning |

### Tier 2: Promotional Campaigns

| Characteristic | Details |
|----------------|---------|
| **Frequency** | 4-6 per year |
| **Duration** | 1-4 weeks |
| **Budget Allocation** | 30% of annual marketing budget |
| **Channels** | BTL + Digital + In-Store |
| **HQ Support** | Campaign guidelines, POSM templates |

### Tier 3: Local Activations

| Characteristic | Details |
|----------------|---------|
| **Frequency** | As needed |
| **Duration** | Event-specific |
| **Budget Allocation** | 20% of annual marketing budget |
| **Channels** | Local media + In-Store |
| **HQ Support** | Brand guidelines, approval process |

### Always-On Activities

| Characteristic | Details |
|----------------|---------|
| **Frequency** | Continuous |
| **Duration** | Year-round |
| **Budget Allocation** | 10% of annual marketing budget |
| **Channels** | Social media, CRM, SEO |
| **HQ Support** | Content calendar, templates |

---

## Marketing Channels

### Channel Overview

| Channel | Primary Use | Budget Share |
|---------|-------------|--------------|
| **Digital/Social Media** | Awareness, engagement, traffic | 35% |
| **In-Store/POSM** | Conversion, promotion communication | 25% |
| **Outdoor/OOH** | Awareness in key locations | 15% |
| **Print Media** | Brand awareness (select markets) | 10% |
| **Events/Activations** | Engagement, brand experience | 10% |
| **CRM/Direct** | Retention, personalized offers | 5% |

### Digital Channel Strategy

| Platform | Purpose | Content Type | Posting Frequency |
|----------|---------|--------------|-------------------|
| **Instagram** | Brand awareness, engagement | Lifestyle imagery, stories | Daily |
| **Facebook** | Community, promotions | Offers, events, community | 3-5x/week |
| **TikTok** | Youth engagement | Trends, challenges | 3-5x/week |
| **Website** | Information, e-commerce | Products, campaigns | Continuous |
| **Email** | CRM, personalized offers | Newsletters, offers | 2-4x/month |
| **WhatsApp** | Direct customer service | Support, notifications | As needed |

---

## Budget Planning

### Annual Budget Structure

| Category | % of Total | Description |
|----------|------------|-------------|
| **Media Buying** | 40% | Paid advertising across channels |
| **Production** | 25% | Content creation, POSM production |
| **Events** | 15% | Activations, sponsorships |
| **Agency Fees** | 10% | Creative, media, PR agencies |
| **Contingency** | 10% | Unexpected opportunities |

### Co-op Marketing Model

```
┌─────────────────────────────────────────────────────────────┐
│              CO-OP MARKETING STRUCTURE                       │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  Central Campaigns (HQ Funded)                               │
│  ├── Global brand assets                                     │
│  ├── Campaign creative development                           │
│  └── Core POSM kit                                           │
│                                                              │
│  Regional Adaptation (50/50 Split)                           │
│  ├── Local language adaptation                               │
│  ├── Regional media buying                                   │
│  └── Additional POSM quantities                              │
│                                                              │
│  Local Activations (Franchise Funded)                        │
│  ├── Store-specific promotions                               │
│  ├── Local events                                            │
│  └── Local partnerships                                      │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### Budget Request Process

1. Submit budget request via Marketing Portal (30 days before)
2. Regional Manager review (within 5 business days)
3. HQ approval for requests > $5,000 (within 10 business days)
4. PO issuance and vendor coordination
5. Post-activity reconciliation and reporting

---

## Performance Measurement

### Marketing Dashboard

```
┌─────────────────────────────────────────────────────────────────┐
│                 MARKETING PERFORMANCE DASHBOARD                  │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  AWARENESS METRICS          │  ENGAGEMENT METRICS                │
│  ├── Reach: ___________     │  ├── Social Engagement: ________   │
│  ├── Impressions: ______    │  ├── Email Open Rate: ____%        │
│  └── Brand Recall: ____%    │  └── CTR: ____%                    │
│                              │                                    │
│  CONVERSION METRICS          │  REVENUE METRICS                   │
│  ├── Traffic Increase: ___%  │  ├── Campaign Sales: $_______      │
│  ├── Conversion Rate: ____%  │  ├── ROMI: ____:1                  │
│  └── New Customers: _______  │  └── Incremental Revenue: $______  │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```

---

## Process Steps

### Campaign Execution Process

```
Step 1: Receive Campaign Brief from HQ
        ↓
Step 2: Review Campaign Objectives and KPIs
        ↓
Step 3: Assess Local Market Requirements
        ↓
Step 4: Submit Localization Requests (if needed)
        ↓
Step 5: Receive Marketing Assets and POSM
        ↓
Step 6: Brief Store Teams on Campaign
        ↓
Step 7: Implement In-Store Setup
        ↓
Step 8: Launch Campaign
        ↓
Step 9: Monitor Performance Daily
        ↓
Step 10: Submit Weekly Performance Reports
        ↓
Step 11: Campaign Close and Final Reporting
```

### Marketing Approval Process

```
Local Initiative Proposal
        ↓
Store Manager Review → Reject → Revise
        ↓ Approve
Regional Manager Review → Reject → Revise
        ↓ Approve
HQ Marketing Review (if > $5,000) → Reject → Revise
        ↓ Approve
Budget Allocation
        ↓
Execution
        ↓
Post-Activity Report
```

---

## Standards & Rules

### Brand Compliance Rules

1. **Logo Usage:** Only use approved logo files from Brand Portal
2. **Color Accuracy:** Use only Pantone/CMYK/RGB values from brand guidelines
3. **Typography:** Only use approved brand fonts
4. **Imagery:** Use only licensed or approved imagery
5. **Messaging:** All copy must align with brand voice guidelines
6. **Approval:** All external communications require HQ approval

### POSM Standards

| Material | Size | Placement | Durability |
|----------|------|-----------|------------|
| Window Poster | A1/A2 | Inside window | Campaign duration |
| Counter Display | A4/A5 | Checkout area | Campaign duration |
| Ceiling Dangler | 30x50cm | Above key areas | Campaign duration |
| Floor Sticker | 50x50cm | Entry/key areas | Campaign duration |
| Shelf Talker | 10x15cm | Shelf edge | Until promotion ends |

### Digital Content Rules

1. **Response Time:** Social media comments within 4 hours
2. **Content Approval:** All posts reviewed before publishing
3. **Hashtag Usage:** Use official brand hashtags only
4. **Influencer Partnership:** HQ approval required
5. **Crisis Response:** Escalate negative comments immediately

---

## KPIs

### Marketing Performance KPIs

| KPI | Formula | Target | Frequency |
|-----|---------|--------|-----------|
| **ROMI** | (Revenue - Marketing Cost) / Marketing Cost | 3:1 | Per Campaign |
| **Cost Per Acquisition** | Total Marketing Cost / New Customers | < $25 | Monthly |
| **Brand Awareness** | Aided Recall % | > 60% | Quarterly |
| **Social Engagement Rate** | (Likes + Comments + Shares) / Followers × 100 | > 3% | Weekly |
| **Email Open Rate** | Opens / Delivered × 100 | > 25% | Per Campaign |
| **CTR** | Clicks / Impressions × 100 | > 2% | Per Campaign |
| **Conversion Rate** | Conversions / Visitors × 100 | > 3% | Weekly |
| **Customer Acquisition Cost** | Marketing Spend / New Customers | < $30 | Monthly |

### Campaign ROI Calculation

```
Campaign ROI = ((Campaign Revenue - Campaign Cost) / Campaign Cost) × 100

Example:
- Campaign Revenue: $150,000
- Campaign Cost: $40,000
- ROI = (($150,000 - $40,000) / $40,000) × 100 = 275%
```

---

## Common Issues & Solutions

### Issue 1: POSM Arrives Late

**Symptom:** Marketing materials arrive after campaign start date.

**Possible Causes:**
- Order placed too late
- Production delays
- Shipping issues

**Solution:**
1. Order POSM minimum 3 weeks before campaign
2. Use digital alternatives if physical materials delayed
3. Report delays immediately to Regional Manager
4. Document for future planning improvements

---

### Issue 2: Low Campaign Awareness

**Symptom:** Customers not aware of ongoing campaign.

**Possible Causes:**
- Insufficient POSM visibility
- Staff not briefed properly
- Marketing reach too limited

**Solution:**
1. Audit POSM placement and visibility
2. Re-brief store staff on campaign details
3. Increase social media promotion
4. Add window display prominence

---

### Issue 3: Brand Guideline Violations

**Symptom:** Unauthorized use of brand assets or messaging.

**Possible Causes:**
- Staff not trained on guidelines
- Local vendors using old assets
- Urgency leading to shortcuts

**Solution:**
1. Conduct brand guidelines training
2. Audit all local vendor files
3. Implement approval workflow
4. Document violations and consequences

---

### Issue 4: Budget Overruns

**Symptom:** Actual marketing spend exceeds budget.

**Possible Causes:**
- Poor planning
- Unplanned activities
- Scope creep

**Solution:**
1. Review all commitments vs. budget monthly
2. Require pre-approval for unplanned activities
3. Build contingency into planning
4. Track spend weekly during campaigns

---

## Checklists

### Campaign Launch Checklist

- [ ] Campaign brief reviewed and understood
- [ ] Local adaptation requirements identified
- [ ] Marketing assets received and verified
- [ ] POSM delivered and quality checked
- [ ] Store teams briefed on campaign
- [ ] In-store setup completed
- [ ] Window displays updated
- [ ] Digital assets scheduled
- [ ] CRM communications prepared
- [ ] Tracking mechanisms in place
- [ ] Launch confirmation sent to HQ

### Monthly Marketing Review Checklist

- [ ] Review campaign performance vs. KPIs
- [ ] Analyze competitor activity
- [ ] Update social media calendar
- [ ] Check brand compliance across stores
- [ ] Review marketing budget utilization
- [ ] Plan upcoming campaign activities
- [ ] Document learnings and recommendations

---

## Related Documents

- [Brand Guidelines](brand-guideline-en.md)
- [Seasonal Marketing Calendar](seasonal-marketing-calendar-en.md)
- [Campaign Execution Guide](campaign-execution-guide-en.md)
- [Digital Assets Usage](digital-assets-usage-en.md)
- [POSM Guidelines](posm-guideline-en.md)
- [Marketing Approval Process](marketing-approval-process-en.md)

---

## Revision History

| Version | Date | Changes | Prepared by |
|---------|------|---------|-------------|
| 1.0 | December 01, 2025 | Initial version created | Aydınlı Group |

---

*This document was prepared by Aydınlı Group.*  
*© 2025 Aydınlı Group - All rights reserved.*


