---
layout: default
title: "Franchise Communication Protocol"
parent: Guidelines
permalink: /guidelines/franchise-communication-protocol-en/
---

# Franchise Communication Protocol

> **Last Updated:** December 01, 2025 | **Version:** 1.0  
> **Prepared by:** Aydınlı Group

---

## Table of Contents

1. [Introduction and Purpose](#introduction-and-purpose)
2. [Organization Structure](#organization-structure)
3. [Communication Channels](#communication-channels)
4. [Reporting Standards](#reporting-standards)
5. [Order and Supply Communication](#order-and-supply-communication)
6. [Escalation Procedures](#escalation-procedures)
7. [Meeting and Conference Protocols](#meeting-and-conference-protocols)
8. [Crisis Communication](#crisis-communication)
9. [Communication Templates](#communication-templates)

---

## Introduction and Purpose

This protocol establishes communication standards between Aydınlı Group and international franchise partners. Its objectives are:

- Ensure consistent and professional communication
- Create clear channels for rapid problem resolution
- Consider regional differences
- Increase operational efficiency

### Core Principles

1. **Transparency:** All communications should be open and honest
2. **Timeliness:** Responses should be provided within specified timeframes
3. **Professionalism:** Corporate language and tone should be used
4. **Documentation:** All important communications should be recorded

---

## Organization Structure

### Aydınlı Group Headquarters Structure

```
┌─────────────────────────────────────┐
│         GENERAL MANAGER             │
└──────────────┬──────────────────────┘
               │
    ┌──────────┼──────────┐
    │          │          │
┌───▼───┐  ┌───▼───┐  ┌───▼───┐
│Sales  │  │Operations│ │Finance│
│Director│  │Director │ │Director│
└───┬───┘  └───┬───┘  └───┬───┘
    │          │          │
┌───▼───┐  ┌───▼───┐  ┌───▼───┐
│Regional│ │Logistics│ │Accounting│
│Manager│  │Manager  │ │Manager  │
└───────┘  └─────────┘  └─────────┘
```

### Franchise Partner Structure

```
┌─────────────────────────────────────┐
│       FRANCHISE OWNER               │
└──────────────┬──────────────────────┘
               │
    ┌──────────┼──────────┐
    │          │          │
┌───▼───┐  ┌───▼───┐  ┌───▼───┐
│Operations│ │Sales  │  │Finance│
│Manager  │  │Manager │  │Manager│
└───┬───┘  └───────┘  └────────┘
    │
┌───▼───┐
│Store  │
│Manager│
└───────┘
```

### Communication Matrix

| Subject | Franchise Representative | HQ Counterpart |
|---------|-------------------------|----------------|
| Daily Sales | Store Manager | Regional Manager |
| Orders | Operations Manager | Order Tracking |
| Shipment | Operations Manager | Logistics Manager |
| Financial | Finance Manager | Accounting Manager |
| VM/Marketing | Sales Manager | Marketing |
| Strategic | Franchise Owner | General Manager |

---

## Communication Channels

### Primary Channels

| Channel | Usage | Response Time |
|---------|-------|---------------|
| **Email** | Official communication, documentation | 24 hours |
| **WhatsApp Business** | Urgent operational matters | 4 hours |
| **ERP Portal** | Orders, inventory, reports | Real-time |
| **Video Conference** | Weekly meetings | Scheduled |
| **Phone** | Emergencies | Immediate |


### Email Standards

**Subject Line Format:**
```
[REGION] - [SUBJECT] - [URGENCY]

Examples:
[REGION] - Order Tracking #PO2025-1234 - Normal
[REGION] - Shipment Delay - Urgent
[REGION] - VM Material Request - Normal
```

**Email Structure:**
```
Dear [Name],

[Greeting and subject summary]

[Detailed explanation]

[Request or action items]

[Closing]

Best regards,
[Name]
[Position]
[Store/Company]
[Contact Information]
```

---

## Reporting Standards

### Daily Reports

**Sales Report (Due by 22:00 daily)**

| Metric | Format |
|--------|--------|
| Total Sales | Currency |
| Transaction Count | Units |
| UPT | Decimal |
| ATV | Currency |
| Visitors | Units |
| Conversion | Percentage |

**Report Template:**
```
DAILY SALES REPORT
Date: [DD/MM/YYYY]
Store: [Store Name]

                Today    Target   Achievement
Sales:          _____    _____    ___%
Transactions:   _____    _____    ___%
UPT:            _____    _____    ___%
ATV:            _____    _____    ___%
Visitors:       _____    _____    ___%
Conversion:     _____    _____    ___%

Notes: ________________________
```

### Weekly Reports

**Stock Status Report (Due Monday 12:00)**
- Current stock value
- Stock cover (weeks)
- Critical stock alerts (cover < 4 weeks)
- Transfer requests

**Performance Report (Due Monday 12:00)**
- Weekly sales summary
- LFL comparison
- Top selling products (Top 10)
- Slow moving products (Bottom 10)

### Monthly Reports

**Comprehensive Performance Report (Due by 5th of each month)**
- Monthly sales performance
- KPI analysis
- Stock analysis
- Store operations summary
- Next month targets

---

## Order and Supply Communication

### Order Process

```
1. Franchise Order Request
   (via ERP)
   ↓
2. HQ Order Approval
   (within 48 hours)
   ↓
3. Proforma Invoice Issuance
   ↓
4. Payment Confirmation
   ↓
5. Production/Preparation
   ↓
6. Shipment Notification
   ↓
7. Delivery and Goods Receipt
```

### Order Communication Format

**Order Request:**
```
Subject: [REGION] - Order Request - [Season/Period]

Dear Order Team,

Please find our order request below:

Partner: [Partner Name]
Partner Code: [Code]
Request Date: [DD/MM/YYYY]
Required Delivery: [DD/MM/YYYY]

PRODUCT LIST:
| SKU | Product | Size | Color | Qty |
|-----|---------|------|-------|-----|
| ... | ...     | ...  | ...   | ... |

Total Quantity: [X]
Estimated Value: [X] USD

Payment Method: [L/C / TT / Other]
Incoterms: [FOB / CIF / CPT / DDP]

Notes: ________________________

Thank you.
```

### Shipment Communication

**Shipment Notification (HQ → Franchise):**
```
Subject: [REGION] - Shipment Notification - [PO#]

Dear [Name],

The following order has been shipped:

PO No: [PO2025-XXXX]
Ship Date: [DD/MM/YYYY]
ETA: [DD/MM/YYYY]

Logistics Details:
- Carrier: [Carrier Name]
- Tracking No: [AWB/B/L No]
- Container No: [If applicable]
- Carton Count: [X]
- Gross Weight: [X] kg

Attachments:
- Packing List
- Commercial Invoice
- Certificate of Origin

Please confirm goods receipt within 24 hours of arrival.

Best regards,
Logistics Team
```

---

## Escalation Procedures

### Escalation Levels

| Level | Definition | Duration | Authority |
|-------|------------|----------|-----------|
| **L1** | Routine matters | 24 hours | Relevant specialist |
| **L2** | Unresolved L1 | 48 hours | Department Manager |
| **L3** | Critical matters | 72 hours | Director |
| **L4** | Crisis situations | Immediate | General Manager |

### Escalation Matrix

| Subject | L1 | L2 | L3 | L4 |
|---------|-----|-----|-----|-----|
| Order delay | Order Specialist | Order Manager | Ops Director | GM |
| Shipment issue | Logistics Specialist | Logistics Manager | Ops Director | GM |
| Quality issue | QC Specialist | QC Manager | Ops Director | GM |
| Payment issue | Accounting | Finance Manager | Finance Director | GM |
| Contract matter | Legal | Legal Manager | General Secretary | GM |

### Escalation Communication Template

```
Subject: [URGENT] - Escalation L[X] - [Subject Summary]

ESCALATION NOTIFICATION

Date/Time: [DD/MM/YYYY HH:MM]
Escalation Level: L[X]
Previous Level Ref: [Ticket no if available]

PROBLEM DESCRIPTION:
[Detailed description]

IMPACT ANALYSIS:
- Financial Impact: [Estimated amount]
- Operational Impact: [Description]
- Customer Impact: [Description]

ACTIONS TAKEN:
1. [Date] - [Action]
2. [Date] - [Action]

REQUESTED ACTION:
[Clear request]

EXPECTED RESOLUTION TIME: [X hours/days]

Contact: [Name, phone, email]
```

---

## Meeting and Conference Protocols

### Weekly Meetings

**Operations Meeting**
- Day: Every Monday
- Time: 14:00 TRT
- Duration: 1 hour
- Platform: Microsoft Teams / Zoom
- Participants: Regional Manager + Franchise Operations Manager

**Agenda:**
1. Last week performance summary (10 min)
2. Stock status (10 min)
3. Operational issues (15 min)
4. Planned activities (10 min)
5. Questions and closing (15 min)

### Monthly Meetings

**Business Review Meeting**
- Day: First week of each month
- Duration: 2 hours
- Participants: Directors + Franchise Owners

**Agenda:**
1. Monthly performance analysis
2. KPI evaluation
3. Marketing activities
4. Stock and supply planning
5. Next period targets

### Meeting Minutes Template

```
MEETING MINUTES

Meeting: [Meeting Name]
Date: [DD/MM/YYYY]
Time: [HH:MM - HH:MM]
Participants: [Names]

AGENDA ITEMS:

1. [Item 1]
   - Discussion: [Summary]
   - Decision: [Decision]
   - Owner: [Name]
   - Due Date: [DD/MM/YYYY]

2. [Item 2]
   ...

ACTION TABLE:
| # | Action | Owner | Due Date | Status |
|---|--------|-------|----------|--------|
| 1 | ...    | ...   | ...      | ...    |

NEXT MEETING: [DD/MM/YYYY, HH:MM]

Prepared by: [Name]
Distribution: [Names]
```

---

## Crisis Communication

### Crisis Types

| Type | Definition | Priority |
|------|------------|----------|
| **Operational Crisis** | Supply disruption, store closure | High |
| **Financial Crisis** | Payment issues, currency crisis | High |
| **Reputation Crisis** | Media, social media issues | Very High |
| **Natural Disaster** | Earthquake, flood, pandemic | Critical |

### Crisis Communication Protocol

```
1. Detect and Assess Crisis
   ↓
2. Activate Crisis Team
   ↓
3. First 1 Hour: Prepare Situation Report
   ↓
4. First 2 Hours: Inform Internal Stakeholders
   ↓
5. First 4 Hours: Create Action Plan
   ↓
6. Ongoing: Status Updates (every 4 hours)
   ↓
7. Post-Crisis: Evaluation and Reporting
```

### Emergency Contact List

```
CRISIS CONTACT LIST


Crisis Hotline (24/7):
Tel: +90 XXX XXX XX XX
WhatsApp: +90 XXX XXX XX XX
```

---

## Communication Templates

### 1. New Store Opening Notification

```
Subject: [REGION] - New Store Opening Notification

Dear Team,

Please find our new store opening details below:

STORE INFORMATION:
Store Name: [Name]
Address: [Full Address]
Area: [X] m²
Opening Date: [DD/MM/YYYY]

STORE MANAGER:
Name: [Full Name]
Tel: [Phone]
Email: [Email]

Initial order and VM material requests will
be submitted separately.

Thank you.
```

### 2. Stock Request

```
Subject: [REGION] - Stock/Transfer Request - [Urgency]

Dear Team,

Please find our stock request below:

| SKU | Product | Size | Qty | Reason |
|-----|---------|------|-----|--------|
| ... | ...     | ...  | ... | ...    |

Request Reason: [High sales / Size break / New season]
Required Delivery: [DD/MM/YYYY]

Thank you.
```

### 3. Issue Report

```
Subject: [REGION] - Issue Report - [Category]

Dear Team,

We would like to report the following issue:

ISSUE DETAILS:
Category: [Shipment / Quality / System / Other]
Date: [DD/MM/YYYY]
Reference: [PO# / Invoice# / Other]

Description:
[Detailed description]

Impact:
[Operational/financial impact]

Expected Resolution:
[Requested action]

Attachments: [If any]

Thank you.
```

---

## Revision History

| Version | Date | Change | Prepared By |
|---------|------|--------|-------------|
| 1.0 | 2025-12-01 | Initial version created | Aydınlı Group |

---

*This document was prepared by Aydınlı Group.*  
*© 2025 Aydınlı Group - All rights reserved.*


