---
layout: default
title: "Mağaza Operasyonları Rehberi"
parent: Guidelines
permalink: /guidelines/store-operations-guideline-tr/
---

# Mağaza Operasyonları Rehberi

> **Son Güncelleme:** 01 Aralık 2025 | **Versiyon:** 1.0  
> **Hazırlayan:** Aydınlı Grup  
> **Kapsam:** Günlük operasyonlar, iade/değişim, hasar, fiyat yönetimi, stok prosedürleri

---

## İçindekiler

1. [Günlük Operasyon Prosedürleri](#günlük-operasyon-prosedürleri)
2. [Mağaza Açılış Prosedürü](#mağaza-açılış-prosedürü)
3. [Mağaza Kapanış Prosedürü](#mağaza-kapanış-prosedürü)
4. [İade ve Değişim Prosedürleri](#iade-ve-değişim-prosedürleri)
5. [Hasar ve Red Prosedürleri](#hasar-ve-red-prosedürleri)
6. [Fiyat Değişimi Prosedürleri](#fiyat-değişimi-prosedürleri)
7. [Stok Yönetimi Prosedürleri](#stok-yönetimi-prosedürleri)
8. [Kasa Yönetimi](#kasa-yönetimi)
9. [Müşteri Hizmetleri Standartları](#müşteri-hizmetleri-standartları)
10. [Acil Durum Prosedürleri](#acil-durum-prosedürleri)

---

## Günlük Operasyon Prosedürleri

### Günlük Operasyon Akışı

```
08:30 - Mağaza Açılış Hazırlığı
09:00 - Mağaza Açılışı
09:00-12:00 - Sabah Dilimi
12:00-13:00 - Öğle Molası (Rotasyonlu)
13:00-18:00 - Öğleden Sonra Dilimi
18:00-21:00 - Akşam Dilimi
21:00-21:30 - Mağaza Kapanış
```

### Vardiya Planlaması

| Vardiya | Saat | Minimum Personel |
|---------|------|------------------|
| Sabah | 08:30-17:00 | 2 kişi |
| Akşam | 13:00-21:30 | 2 kişi |
| Tam Gün | 09:00-21:30 | (Rotasyonlu) |

### Personel Görev Dağılımı

| Pozisyon | Sorumluluklar |
|----------|---------------|
| **Mağaza Müdürü** | Genel yönetim, raporlama, personel |
| **Vardiya Sorumlusu** | Vardiya yönetimi, kasa kontrolü |
| **Satış Danışmanı** | Müşteri hizmeti, satış, düzen |
| **Kasiyer** | Ödeme işlemleri, iade |
| **Depo Sorumlusu** | Mal kabul, stok yönetimi |

---

## Mağaza Açılış Prosedürü

### Açılış Kontrol Listesi (Açılıştan 30 dk önce)

#### 1. Güvenlik Kontrolleri
- [ ] Alarm sistemini devre dışı bırak
- [ ] Tüm giriş/çıkışları kontrol et
- [ ] Gece boyunca anormallik var mı?
- [ ] Güvenlik kameralarını kontrol et

#### 2. Fiziksel Hazırlık
- [ ] Tüm ışıkları aç
- [ ] Klima/ısıtmayı ayarla
- [ ] Zemin temizliğini kontrol et
- [ ] Camları kontrol et

#### 3. Mağaza Düzeni
- [ ] Vitrin kontrolü
- [ ] Fixture düzeni kontrolü
- [ ] Manken kontrolü
- [ ] Signage kontrolü
- [ ] Fiyat etiketleri kontrolü

#### 4. Kasa Hazırlığı
- [ ] Kasa açılış sayımı yap
- [ ] POS cihazlarını aç ve test et
- [ ] Yeterli bozuk para var mı?
- [ ] Fiş rulosu kontrolü

#### 5. Stok Hazırlığı
- [ ] Gece gelen sevkiyat var mı?
- [ ] Eksik beden/renk takviyesi
- [ ] İade edilecek ürün kontrolü

### Açılış Sayım Formu

```
┌─────────────────────────────────────┐
│    KASA AÇILIŞ SAYIM FORMU          │
├─────────────────────────────────────┤
│ Tarih: ___/___/______               │
│ Personel: _________________         │
├─────────────────────────────────────┤
│ Madeni Para                          │
│   0,50 TL × ___ = _____ TL          │
│   1,00 TL × ___ = _____ TL          │
│   5,00 TL × ___ = _____ TL          │
├─────────────────────────────────────┤
│ Kağıt Para                           │
│   10 TL  × ___ = _____ TL           │
│   20 TL  × ___ = _____ TL           │
│   50 TL  × ___ = _____ TL           │
│   100 TL × ___ = _____ TL           │
│   200 TL × ___ = _____ TL           │
├─────────────────────────────────────┤
│ TOPLAM: _____________ TL            │
│ Avans: _____________ TL             │
│ İmza: _______________               │
└─────────────────────────────────────┘
```

---

## Mağaza Kapanış Prosedürü

### Kapanış Kontrol Listesi (Kapanıştan 30 dk önce başla)

#### 1. Müşteri Yönetimi
- [ ] Son müşteriyi bilgilendir (kapanışa 15 dk kala)
- [ ] Kabindeki müşterileri takip et
- [ ] Kapanış anonsu yap (varsa)

#### 2. Kasa Kapanış
- [ ] Z raporu al
- [ ] Kasa sayımı yap
- [ ] Nakit-kredi kartı karşılaştır
- [ ] Kasayı kilitle
- [ ] Günlük rapor hazırla

#### 3. Mağaza Düzeni
- [ ] Tüm ürünleri yerleştir
- [ ] Kabinleri boşalt ve düzenle
- [ ] Askıları topla
- [ ] Zemin temizliği
- [ ] Vitrin kontrolü

#### 4. Güvenlik
- [ ] Arka odaları kontrol et
- [ ] Depo kapısını kilitle
- [ ] Tüm ışıkları kapat (güvenlik ışığı hariç)
- [ ] Klimayı kapat
- [ ] Alarm sistemini aktifleştir
- [ ] Panjur/kepenk kapat

### Kapanış Sayım ve Mutabakat

```
┌─────────────────────────────────────┐
│    GÜNLÜK KASA MUTABAKAT            │
├─────────────────────────────────────┤
│ Tarih: ___/___/______               │
├─────────────────────────────────────┤
│ NAKİT SATIŞLAR                       │
│ POS Nakit Toplam: _______ TL        │
│ Fiziki Nakit Sayım: _______ TL      │
│ Fark: _______ TL                    │
├─────────────────────────────────────┤
│ KREDİ KARTI SATIŞLAR                │
│ POS Kredi Toplam: _______ TL        │
│ Slip Toplam: _______ TL             │
│ Fark: _______ TL                    │
├─────────────────────────────────────┤
│ GÜNLÜK ÖZET                          │
│ Toplam Satış: _______ TL            │
│ İşlem Sayısı: _______               │
│ İade Tutarı: _______ TL             │
├─────────────────────────────────────┤
│ Onaylayan: _______________          │
│ İmza: _______________               │
└─────────────────────────────────────┘
```

---

## İade ve Değişim Prosedürleri

### İade Politikası

| Durum | Süre | Koşul | İşlem |
|-------|------|-------|-------|
| Etiketli, kullanılmamış | 14 gün | Fatura ile | Tam iade |
| Etiketli, kullanılmamış | 14-30 gün | Fatura ile | Mağaza kredisi |
| Etiketsiz/kullanılmış | - | - | İade kabul edilmez |
| Hatalı ürün | 30 gün | Fatura ile | Değişim veya iade |

### İade İşlem Adımları

```
1. Müşteri Karşılama
   ↓
2. Fatura/Fiş Kontrolü
   ↓
3. Ürün Kontrolü (etiket, durum)
   ↓
4. İade Sebebi Belirleme
   ↓
5. Sistem Kaydı (POS)
   ↓
6. İade Formu Doldurma
   ↓
7. Para İadesi / Değişim
   ↓
8. Müşteri İmzası
```

### İade Sebep Kodları

| Kod | Sebep | Açıklama |
|-----|-------|----------|
| R01 | Beden uyumsuzluğu | Müşteri yanlış beden aldı |
| R02 | Renk beğenmeme | Müşteri rengi beğenmedi |
| R03 | Hediye iadesi | Hediye alan kişi iade |
| R04 | Üretim hatası | Ürün kusurlu |
| R05 | Fiyat farkı | Başka yerde ucuz bulma |
| R06 | Diğer | Açıklama gerekli |

### İade Formu

```
┌─────────────────────────────────────┐
│       İADE / DEĞİŞİM FORMU          │
├─────────────────────────────────────┤
│ Tarih: ___/___/______               │
│ Fiş No: _________________           │
│ Orijinal Satış Tarihi: ___/___/___  │
├─────────────────────────────────────┤
│ MÜŞTERİ BİLGİLERİ                   │
│ Ad Soyad: _________________         │
│ Telefon: _________________          │
├─────────────────────────────────────┤
│ ÜRÜN BİLGİLERİ                      │
│ SKU: _________________              │
│ Ürün Adı: _________________         │
│ Beden: ___ Renk: ___                │
│ Tutar: _______ TL                   │
├─────────────────────────────────────┤
│ İADE SEBEBİ: [ ] R01 [ ] R02 [ ] R03│
│              [ ] R04 [ ] R05 [ ] R06│
│ Açıklama: _________________         │
├─────────────────────────────────────┤
│ İŞLEM TİPİ                          │
│ [ ] Nakit İade  [ ] Kredi Kartı İade│
│ [ ] Değişim     [ ] Mağaza Kredisi  │
├─────────────────────────────────────┤
│ Müşteri İmza: _________________     │
│ Personel: _________________         │
└─────────────────────────────────────┘
```

---

## Hasar ve Red Prosedürleri

### Hasar Türleri

| Tür | Tanım | İşlem |
|-----|-------|-------|
| **Üretim Hatası** | Fabrikadan kaynaklı | Red raporu, merkeze iade |
| **Taşıma Hasarı** | Sevkiyat sırasında | Tutanak, sigorta talebi |
| **Mağaza Hasarı** | Mağazada oluşan | Fire kaydı |
| **Müşteri Hasarı** | Müşteri kaynaklı | Fire veya tazmin |

### Hasar Tespit Prosedürü

```
1. Hasarı Tespit Et
   ↓
2. Fotoğraf Çek (min. 3 açı)
   ↓
3. Hasar Formunu Doldur
   ↓
4. Ürünü Ayır (kırmızı etiket)
   ↓
5. Merkeze Bildir (24 saat içinde)
   ↓
6. Onay Bekle
   ↓
7. Fire/Red İşlemi Yap
```

### Hasar Raporu Formu

```
┌─────────────────────────────────────┐
│         HASAR RAPORU                │
├─────────────────────────────────────┤
│ Tarih: ___/___/______               │
│ Mağaza: _________________           │
│ Rapor No: HAR-____-____             │
├─────────────────────────────────────┤
│ ÜRÜN BİLGİLERİ                      │
│ SKU: _________________              │
│ Ürün Adı: _________________         │
│ Beden: ___ Renk: ___ Adet: ___      │
│ Birim Fiyat: _______ TL             │
│ Toplam: _______ TL                  │
├─────────────────────────────────────┤
│ HASAR TİPİ                          │
│ [ ] Üretim Hatası                   │
│ [ ] Taşıma Hasarı                   │
│ [ ] Mağaza Hasarı                   │
│ [ ] Müşteri Hasarı                  │
├─────────────────────────────────────┤
│ HASAR DETAYI                        │
│ ________________________________    │
│ ________________________________    │
├─────────────────────────────────────┤
│ Fotoğraf Ek: [ ] Evet [ ] Hayır     │
│ Hazırlayan: _________________       │
│ Onaylayan: _________________        │
└─────────────────────────────────────┘
```

### Red (Reject) Prosedürü

Sevkiyat reddi durumunda:

1. Teslim anında kontrol yap
2. Hasarlı/yanlış ürünleri tespit et
3. İrsaliye üzerine not düş
4. Kargo yetkilisine imzalat
5. Red tutanağı hazırla
6. 24 saat içinde merkeze bildir
7. Ürünü ayrı muhafaza et

---

## Fiyat Değişimi Prosedürleri

### Fiyat Değişim Türleri

| Tür | Tanım | Yetki |
|-----|-------|-------|
| **Merkezi İndirim** | Genel kampanya | Merkez |
| **Bölgesel İndirim** | Bölge bazlı | Bölge Müdürü |
| **Markdown** | Sezon sonu | Merkez onayı |
| **Fiyat Düzeltme** | Hata düzeltme | Mağaza Müdürü |

### Fiyat Güncelleme Prosedürü

```
1. Merkez Fiyat Değişim Bildirimi
   ↓
2. Fiyat Listesini İndir
   ↓
3. Sistem Güncelle (POS)
   ↓
4. Fiziki Etiket Değişimi
   ↓
5. Kontrol (rastgele 10 ürün)
   ↓
6. Tamamlandı Bildirimi
```

### Etiket Değişim Kuralları

- Eski etiketler tamamen çıkarılmalı
- Üst üste etiket yapıştırılmamalı
- Etiket üstüne el yazısı yazılmamalı
- Tüm bedenler güncellenmeli

### Fiyat Kontrol Formu

```
┌─────────────────────────────────────┐
│      FİYAT DEĞİŞİM KONTROL          │
├─────────────────────────────────────┤
│ Kampanya/Dönem: ________________    │
│ Geçerlilik: ___/___ - ___/___      │
├─────────────────────────────────────┤
│ SKU        Eski      Yeni    ✓     │
│ ________   _____TL   _____TL  [ ]  │
│ ________   _____TL   _____TL  [ ]  │
│ ________   _____TL   _____TL  [ ]  │
├─────────────────────────────────────┤
│ Kontrol Eden: _________________     │
│ Tarih/Saat: _________________       │
└─────────────────────────────────────┘
```

---

## Stok Yönetimi Prosedürleri

### Mal Kabul Prosedürü

```
1. Sevkiyat Gelişi
   ↓
2. İrsaliye Kontrolü
   ↓
3. Koli Sayımı (dış kontrol)
   ↓
4. Koli Açma ve Sayım
   ↓
5. Sistem Girişi (GR)
   ↓
6. Fiziki Yerleştirme
   ↓
7. İrsaliye İmza/Onay
```

### Sayım Prosedürleri

| Sayım Tipi | Sıklık | Kapsam |
|------------|--------|--------|
| Günlük Sayım | Her gün | Kasa önü, hot items |
| Haftalık Sayım | Haftada 1 | Seçili kategoriler |
| Aylık Sayım | Ayda 1 | Tam mağaza |
| Yıllık Envanter | Yılda 2 | Tam kapanış ile |

### Stok Fark Raporu

```
┌─────────────────────────────────────┐
│        STOK FARK RAPORU             │
├─────────────────────────────────────┤
│ Sayım Tarihi: ___/___/______        │
│ Sayım Tipi: [ ] Günlük [ ] Haftalık │
│             [ ] Aylık  [ ] Yıllık   │
├─────────────────────────────────────┤
│ SKU        Sistem  Fiziki  Fark     │
│ ________   _____   _____   _____    │
│ ________   _____   _____   _____    │
├─────────────────────────────────────┤
│ Toplam Eksik: _____ adet            │
│ Toplam Fazla: _____ adet            │
│ Tutar Farkı: _______ TL             │
├─────────────────────────────────────┤
│ Açıklama: _________________         │
│ Sayan: _________________            │
│ Onaylayan: _________________        │
└─────────────────────────────────────┘
```

### Transfer İşlemleri

**Giden Transfer:**
1. Transfer talebi al
2. Ürünleri topla
3. Sayım ve kontrol
4. Sistem çıkışı (GI)
5. Paketleme
6. Kargo teslim

**Gelen Transfer:**
1. Transferi karşıla
2. Sayım ve kontrol
3. Sistem girişi (GR)
4. Fiziki yerleştirme

---

## Kasa Yönetimi

### Ödeme Türleri

| Ödeme | İşlem | Dikkat Edilecekler |
|-------|-------|-------------------|
| Nakit | Sayım, para üstü | Sahte para kontrolü |
| Kredi Kartı | POS işlemi | İmza/PIN kontrolü |
| Banka Kartı | POS işlemi | Bakiye kontrolü |
| Hediye Çeki | Sistem girişi | Geçerlilik kontrolü |
| Taksit | POS taksit | Taksit seçenekleri |

### Kasa Açığı/Fazlası Prosedürü

**Açık Durumunda:**
1. Yeniden sayım yap
2. Gün içi işlemleri kontrol et
3. Kamera kayıtlarını incele
4. Tutanak hazırla
5. Mağaza müdürüne bildir

**Fazla Durumunda:**
1. Yeniden sayım yap
2. İade/iptal işlemlerini kontrol et
3. Müşteri para üstü kontrolü
4. Tutanak hazırla

### Günlük Kasa Limitleri

| Tutar | İşlem |
|-------|-------|
| < 10.000 TL | Normal operasyon |
| 10.000-25.000 TL | Vardiya içi banka |
| > 25.000 TL | Acil banka transferi |

---

## Müşteri Hizmetleri Standartları

### Müşteri Karşılama

```
1. Göz teması kur
2. Gülümse
3. "Hoş geldiniz" de
4. 3 dakika içinde yaklaş
5. "Size nasıl yardımcı olabilirim?" sor
```

### Satış Süreci

```
İhtiyaç Analizi → Ürün Sunumu → Deneme → İtiraz Karşılama → Kapanış
```

### Şikayet Yönetimi

| Adım | Yapılacak |
|------|-----------|
| 1 | Dinle (kesme) |
| 2 | Empati kur |
| 3 | Özür dile |
| 4 | Çözüm sun |
| 5 | Takip et |

### Yasak Davranışlar

- Müşteriyle tartışmak
- Telefonda uzun konuşmak
- Yemek/içmek (satış alanında)
- Kişisel sohbet (müşteri varken)
- Olumsuz beden dili

---

## Acil Durum Prosedürleri

### Yangın

1. Yangın alarmını çalıştır
2. 110'u ara
3. Müşterileri tahliye et
4. Yangın söndürücü kullan (küçük yangın)
5. Acil çıkışları kullan
6. Toplanma alanına git

### Hırsızlık

1. Güvenliği bilgilendir
2. Müdahale etme (tehlike varsa)
3. Kişiyi tanımla
4. Kamera kaydı al
5. Tutanak hazırla
6. Polisi ara (gerekirse)

### Sağlık Acili

1. 112'yi ara
2. İlk yardım uygula (eğitimliysen)
3. Kişiyi hareket ettirme
4. Sakin ol
5. Alanı boşalt

### Acil İletişim Numaraları

```
Yangın: 110
Ambulans: 112
Polis: 155
Şirket Güvenlik: ___________
Bölge Müdürü: ___________
```

---

## Revision History

| Versiyon | Tarih | Değişiklik | Hazırlayan |
|----------|-------|------------|------------|
| 1.0 | 01.12.2025 | İlk sürüm oluşturuldu | Aydınlı Grup Franchise Ops |

---

*Bu doküman Aydınlı Grup Franchise Operasyonları tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*


