---
layout: default
title: "Visual Merchandising Rehberi"
---

# Visual Merchandising Rehberi

> **Son Güncelleme:** 01 Aralık 2025 | **Versiyon:** 1.0  
> **Hazırlayan:** Aydınlı Grup  
> **Markalar:** U.S. Polo Assn., Pierre Cardin, Cacharel

---

## İçindekiler

1. [Giriş ve Amaç](#giriş-ve-amaç)
2. [Temel VM Prensipleri](#temel-vm-prensipleri)
3. [Mağaza Bölgeleri ve Düzeni](#mağaza-bölgeleri-ve-düzeni)
4. [Vitrin Standartları](#vitrin-standartları)
5. [Fixture ve Ürün Yerleşimi](#fixture-ve-ürün-yerleşimi)
6. [Renk ve Tema Yönetimi](#renk-ve-tema-yönetimi)
7. [Signage ve POP Malzemeleri](#signage-ve-pop-malzemeleri)
8. [Marka Bazlı Standartlar](#marka-bazlı-standartlar)
9. [Sezonsal VM Takvimi](#sezonsal-vm-takvimi)
10. [VM Kontrol Listesi](#vm-kontrol-listesi)

---

## Giriş ve Amaç

Visual Merchandising (VM), ürünlerin mağaza içinde görsel olarak en etkili şekilde sergilenmesini sağlayan stratejik bir disiplindir.

### VM'nin Hedefleri

- **Satış Artışı:** Doğru ürün yerleşimi ile impuls satışları artırmak
- **Marka Algısı:** Tutarlı marka imajı oluşturmak
- **Müşteri Deneyimi:** Alışveriş sürecini keyifli hale getirmek
- **Stok Verimliliği:** Ürünlerin dengeli şekilde satışını sağlamak

### VM'nin Önemi

```
Etkili VM = Daha Fazla Ziyaretçi × Yüksek Dönüşüm × Yüksek UPT × Yüksek ATV
```

---

## Temel VM Prensipleri

### 1. Altın Üçgen Kuralı

Müşteri giriş noktasından bakıldığında, en önemli ürünler "Altın Üçgen" bölgesinde sergilenmelidir.

```
        [VİTRİN]
           △
          / \
         /   \
        /     \
       /  ★★★  \    ← Altın Üçgen Bölgesi
      /         \
     /           \
    /_____________\
       [GİRİŞ]
```

### 2. Göz Seviyesi Kuralı

- **Premium Zone (145-170 cm):** En karlı, yeni sezon ürünleri
- **Touch Zone (90-145 cm):** Kolay erişim, deneme ürünleri
- **Lower Zone (0-90 cm):** Hacimli, ağır ürünler
- **Upper Zone (170+ cm):** Stok, görsel dekor

### 3. Sağa Dönüş Prensibi

Müşterilerin %90'ı mağazaya girince sağa döner. Bu nedenle:
- Sağ taraf: Yeni sezon, trend ürünler
- Sol taraf: Temel ürünler, outlet

### 4. Dekompresson Zone

Giriş alanının ilk 1-2 metresi müşterinin mağazaya adapte olduğu bölgedir. Bu alana kritik ürün yerleştirmeyin.

---

## Mağaza Bölgeleri ve Düzeni

### A-B-C Zone Sistemi

| Zone | Konum | Ürün Tipi | Satış Potansiyeli |
|------|-------|-----------|-------------------|
| **A Zone** | Giriş, vitrin önü | Hero items, yeni sezon | En yüksek |
| **B Zone** | Orta alan | Trend, sezonsal | Orta-yüksek |
| **C Zone** | Arka bölge | Temel, basics | Düşük-orta |

### Alan Dağılımı (Örnek 100 m² mağaza)

```
┌─────────────────────────────────────────┐
│              VİTRİN                      │
├─────────────────────────────────────────┤
│  A ZONE   │      B ZONE      │  A ZONE  │
│  (Erkek)  │    (Aksesuar)    │ (Kadın)  │
│   25 m²   │      15 m²       │   25 m²  │
├───────────┼──────────────────┼──────────┤
│           │    C ZONE        │          │
│   KASA    │   (Basics)       │  KABİN   │
│           │     20 m²        │          │
├───────────┴──────────────────┴──────────┤
│            DEPO (15 m²)                  │
└─────────────────────────────────────────┘
```

### Trafik Akışı Planlaması

- **Ana Yollar:** Min. 120 cm genişlik
- **Ara Yollar:** Min. 90 cm genişlik
- **Fixture Arası:** Min. 80 cm

---

## Vitrin Standartları

### Vitrin Değişim Takvimi

| Dönem | Değişim Sıklığı |
|-------|-----------------|
| Yeni Sezon Lansmanı | Anında |
| Kampanya Dönemleri | Kampanya başı |
| Normal Dönem | 2 haftada bir |
| Özel Günler | Etkinlik öncesi |

### Vitrin Kompozisyon Kuralları

#### Piramit Düzeni
```
       ▲
      /█\
     / █ \
    / █ █ \
   / █ █ █ \
  ───────────
```
- Orta nokta en yüksek
- Simetrik veya asimetrik olabilir
- Bakış noktası 145-165 cm

#### Repetition (Tekrar) Düzeni
```
  █   █   █   █   █
  ▼   ▼   ▼   ▼   ▼
```
- Aynı ürünün farklı renkleri
- Güçlü görsel etki

### Vitrin Aydınlatma

- **Genel Aydınlatma:** 800-1000 lux
- **Spot Aydınlatma:** 2000-3000 lux (hero ürünlere)
- **Renk Sıcaklığı:** 3000-4000K (sıcak beyaz)

### Vitrin Manken Kullanımı

| Mağaza Büyüklüğü | Manken Sayısı |
|------------------|---------------|
| < 80 m² | 2-3 |
| 80-150 m² | 4-6 |
| > 150 m² | 6-8 |

---

## Fixture ve Ürün Yerleşimi

### Fixture Türleri

#### 1. Wall Fixtures (Duvar Üniteleri)
- **Mid-Wall:** 150 cm yükseklik, ana ürünler
- **High-Wall:** 220 cm yükseklik, görseller + stok

#### 2. Floor Fixtures (Zemin Üniteleri)
- **Gondol:** Çift taraflı, orta alan
- **4-Way:** Dört yönlü, aksesuar
- **Round Rail:** Dairesel askılık
- **Nesting Table:** İç içe masalar

### Ürün Sunumu Standartları

#### Katlama Standartları

| Ürün Tipi | Katlama Şekli | Yığın Yüksekliği |
|-----------|---------------|------------------|
| T-Shirt | Yarım katlama | Max 8 adet |
| Polo | Yakalı katlama | Max 6 adet |
| Gömlek | Tam katlama + klips | Max 5 adet |
| Pantolon | Yarım katlama | Max 6 adet |

#### Askı Yönlendirmesi

```
Müşteri Bakış Yönü →

   ←───────────────
   Askı açıklığı sola bakmalı
```

#### Beden Sıralaması

- Soldan sağa: XS → S → M → L → XL → XXL
- Yukarıdan aşağıya: Küçük → Büyük

### Kapasite Standartları

| Fixture Tipi | Max Ürün Sayısı |
|--------------|-----------------|
| Straight Rail (120 cm) | 25-30 adet |
| Round Rail | 40-50 adet |
| Shelf (90 cm) | 15-20 adet (katlanmış) |
| 4-Way | 60-80 adet |

---

## Renk ve Tema Yönetimi

### Renk Blokları Oluşturma

Ürünleri renk geçişlerine göre gruplandırın:

```
AÇIK ──────────────────────────────────► KOYU

Beyaz → Krem → Sarı → Turuncu → Kırmızı → Bordo → Lacivert → Siyah
```

### Renk Hikayesi Anlatımı

Her fixture'da maksimum 3-4 renk kombinasyonu kullanın:

**Doğru:**
```
[Lacivert] [Beyaz] [Kırmızı]
```

**Yanlış:**
```
[Yeşil] [Sarı] [Mor] [Turuncu] [Pembe] [Mavi]
```

### Sezonsal Renk Paleti

| Sezon | Ana Renkler | Aksan Renkler |
|-------|-------------|---------------|
| SS (İlkbahar/Yaz) | Beyaz, Mavi, Mint | Coral, Sarı |
| FW (Sonbahar/Kış) | Lacivert, Bordo, Kahve | Hardal, Bakır |

---

## Signage ve POP Malzemeleri

### Signage Türleri

| Tür | Kullanım Alanı | Boyut |
|-----|----------------|-------|
| **Vitrin Pleksi** | Vitrin içi | A2, A1 |
| **Dön Kart** | Tavan, asılı | 30x50 cm |
| **Shelf Talker** | Raf kenarı | 10x15 cm |
| **Price Tag** | Ürün üzeri | Standard |
| **Floor Sticker** | Zemin | 50x50 cm |

### Fiyat Etiketi Standartları

```
┌─────────────────────────┐
│ MARKA LOGOSU            │
├─────────────────────────┤
│                         │
│    299,90 TL           │
│    (Liste Fiyatı)       │
│                         │
│    SKU: 123456789       │
│    Beden: M             │
└─────────────────────────┘
```

### İndirim Signage

```
┌─────────────────────────┐
│         %30             │
│       İNDİRİM           │
├─────────────────────────┤
│   Eski: 399,90 TL       │
│   Yeni: 279,90 TL       │
└─────────────────────────┘
```

---

## Marka Bazlı Standartlar

### U.S. Polo Assn.

**Konsept:** Amerikan yaşam tarzı, polo heritage, sportif şıklık

**Renk Paleti:**
- Ana: Lacivert, Beyaz, Kırmızı
- Aksan: Yeşil, Sarı

**Fixture Stili:**
- Ahşap + metal kombinasyonu
- Klasik Amerikan tarzı

**Vitrin Temaları:**
- Polo sahası
- Campus life
- Weekend casual

---

### Pierre Cardin

**Konsept:** Modern şıklık, Fransız elegansı, sofistike

**Renk Paleti:**
- Ana: Siyah, Gri, Lacivert
- Aksan: Bordo, Altın

**Fixture Stili:**
- Minimalist, metalik
- Premium görünüm

**Vitrin Temaları:**
- Business elegance
- Urban sophistication
- Evening chic

---

### Cacharel

**Konsept:** Modern erkek giyim, Fransız şıklığı, sofistike

**Renk Paleti:**
- Ana: Lacivert, Gri, Siyah
- Aksan: Koyu tonlar, klasik kombinasyonlar

**Fixture Stili:**
- Klasik hatlar
- Modern detaylar

**Vitrin Temaları:**
- Business elegance
- Urban sophistication
- French classic style

---

## Sezonsal VM Takvimi

### Yıllık VM Takvimi

| Ay | Tema | Vitrin Değişimi |
|----|------|-----------------|
| Ocak | Kış İndirimi | Indirim signage |
| Şubat | Sevgililer Günü | 1-14 Şubat arası |
| Mart | İlkbahar Lansmanı | SS sezon açılış |
| Nisan | Ramazan | Bölgesel uygulama |
| Mayıs | Anneler Günü | Mayıs 2. hafta |
| Haziran | Yaz Koleksiyonu | Tam yaz teması |
| Temmuz | Summer Sale | İndirim teması |
| Ağustos | Back to School | Okul teması |
| Eylül | FW Lansmanı | Sonbahar açılış |
| Ekim | Halloween (seçili) | Bölgesel |
| Kasım | Black Friday | Kasım 4. hafta |
| Aralık | Yılbaşı | Aralık 1-31 |

---

## VM Kontrol Listesi

### Günlük Kontrol

- [ ] Tüm ürünler düzgün asılı/katlanmış mı?
- [ ] Beden sıralaması doğru mu?
- [ ] Boş askı/raf var mı?
- [ ] Fiyat etiketleri görünür mü?
- [ ] Aydınlatma çalışıyor mu?
- [ ] Zemin temiz mi?
- [ ] Mankenler düzgün mü?

### Haftalık Kontrol

- [ ] Vitrin ürünleri güncel mi?
- [ ] Renk blokları düzenli mi?
- [ ] Fixture'lar temiz mi?
- [ ] Signage güncel mi?
- [ ] Alt-üst oranı dengeli mi?
- [ ] Kampanya malzemeleri yerinde mi?

### Aylık Kontrol

- [ ] Vitrin teması sezona uygun mu?
- [ ] Fixture yerleşimi optimize mi?
- [ ] Stok dağılımı dengeli mi?
- [ ] Aydınlatma bakımı yapıldı mı?
- [ ] VM performans analizi yapıldı mı?

---

## VM Performans Metrikleri

| Metrik | Formül | Hedef |
|--------|--------|-------|
| Vitrin Dönüşümü | Vitrin Satışı / Ziyaretçi | >5% |
| A Zone Satış Payı | A Zone Satış / Toplam | >40% |
| Manken Satış Etkisi | Manken Ürün Satışı / Toplam | >15% |

---

## Revision History

| Versiyon | Tarih | Değişiklik | Hazırlayan |
|----------|-------|------------|------------|
| 1.0 | 01.12.2025 | İlk sürüm oluşturuldu | Aydınlı Grup Franchise Ops |

---

*Bu doküman Aydınlı Grup Franchise Operasyonları tarafından hazırlanmıştır.*  
*© 2025 Aydınlı Grup - Tüm hakları saklıdır.*


